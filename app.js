...
(() => {
  ...
  function playRoomEvents(next,afterSeq){
    ...
    for(const ev of events){
      ...
      else if(ev.type==='monster-move')later(()=>monsterMoveEventFx(ev.payload),ev.payload?.signature?1500:1150);
      ...
      else if(ev.type==='enemy-attack')later(()=>enemyAttackEventFx(ev.payload),ev.payload?.signature?1500:ev.payload?.heavy?1250:1050);
      ...
      else if(ev.type==='tactic-link'&&ev.payload?.playerId===state.profileId)later(()=>{const t=$(`[data-monster="${CSS.escape(ev.payload?.instanceId||'')}"]`);if(t){attackLabelV40(t,`RIFT LINK +${Math.round(Number(ev.payload?.amp||0)*100)}%`,'ally');namedSkillOverlayV45(t,'mirror',{element:ev.payload?.element||'별',fxVariant:5});}sound('chain');},700);
  ...
  async function playNamedMonsterMoveV45(payload,enemySide=false){
    ...
    const actorDur=signature?980:tempo==='charge'?820:tempo==='multi'?760:650;try{actor.animate(namedActorFramesV47(family,dir,variant),{duration:actorDur,easing:'cubic-bezier(.14,.82,.16,1)'});}catch{}
    const windup=signature?300:tempo==='charge'?250:family==='beam'?220:170;
    setTimeout(async()=>{
      ...
      if(!['heal','guard'].includes(family)&&Number(payload.damage||0)>0)popNumber(target,`-${payload.damage}`,'damage'+(enemySide?' player':''));if(heavy)screenShake(signature?'heavy':'hit');soundElement(payload.element||'공허',heavy);
  ...
  function moveEffectTagsV54(move){
    if(!move)return[];const out=[];if(move.signature)out.push({k:'signature',t:'전용기'});if(move.burn)out.push({k:'burn',t:`화상 ${move.burn}`});if(move.shock)out.push({k:'shock',t:`감전 ${move.shock}`});if(move.weak)out.push({k:'weak',t:`약화 ${move.weak}`});if(move.vulnerable)out.push({k:'vulnerable',t:`취약 ${move.vulnerable}`});if(move.intentSeal)out.push({k:'seal',t:`봉쇄 ${move.intentSeal}`});if(move.stagger)out.push({k:'break',t:`BREAK +${move.stagger}`});if(move.lifesteal)out.push({k:'drain',t:'흡혈'});if(move.splash)out.push({k:'splash',t:'범위'});if(move.shield)out.push({k:'guard',t:`방어 +${move.shield}`});if(move.heal)out.push({k:'heal',t:`회복 +${move.heal}`});if(move.resonance)out.push({k:'res',t:`RES +${move.resonance}`});return out.slice(0,4);
  }
  function enemyDebuffBadgesV54(e){
    if(!e)return'';const d=e.debuffs||{},chips=[];if(Number(d.burn)>0)chips.push(['burn','화상',d.burn]);if(Number(d.shock)>0)chips.push(['shock','감전',d.shock]);if(Number(d.weak)>0)chips.push(['weak','약화',d.weak]);if(Number(d.vulnerable)>0)chips.push(['vulnerable','취약',d.vulnerable]);if(Number(d.intentSeal)>0)chips.push(['seal','봉쇄',d.intentSeal]);if(e.broken)chips.push(['break','BREAK','']);return chips.length?`<div class="enemy-debuffs-v54">${chips.map(([k,n,v])=>`<span class="debuff-${k}" title="${esc(n)}">${esc(n)}${v?` ${v}`:''}</span>`).join('')}</div>`:'';
  }
  function tacticHintV54(card,monster,enemy){
    if(!card||!monster)return'';if(card.element===monster.element)return'공명 추천 · 다음 기술 +20%';const effects=card.effects||[];if(monster.hp/Math.max(1,monster.maxHp)<.5&&effects.some(x=>['heal','monsterHeal','block','monsterShield'].includes(x.op)))return'위기 대응 추천';if(enemy&&effects.some(x=>['weak','vulnerable','burn','shockAll','intentSealAll'].includes(x.op)))return'상태이상 연계';return'다음 기술 +10%';
  }
  ...
  function scheduleRewardAutoAdvanceV54(r,step,continued){
    clearTimeout(state.rewardAutoTimer);state.rewardAutoTimer=0;if(step!=='READY'||continued||state.busy)return;const key=`${r.id}:${r.floor}:${r.seq}`;const delay=r.finalClearPending?3000:3600;
  ...
  function spellSigilLabel(card){
    ...
    if(mode==='attack')return trait==='aoe'?'광역 스펠':'공격 스펠';
    if(mode==='heal')return trait==='heal'?'회복 스펠':'지원 스펠';
    if(mode==='guard')return '방호 스펠';
    return trait==='debuff'?'약화 스펠':'전술 스펠';
  }
  ...
  function stripCardThumbHtml(card){
    if(card?.type!=='spell')return `<img src="${esc(card.art)}" alt=""><span>${esc(card.name)}</span>`;
    const ec=elementClass(card.element),mode=cardFxMode(card),trait=cardFxTrait(card),variant=cardFxVariant(card);
  ...
  async function renderDungeonLobby(){
    setScreen('dungeonLobby',`<section class="page"><div class="page-head"><button class="back-btn" data-action="home">← 홈</button><div><div class="section-label">CO-OP DUNGEON</div><h1>50층 협동 던전</h1><p>내가 잡은 몬스터 + 링크 전술 덱으로 50층을 공략합니다.</p></div></div><div class="lobby-grid"><div class="room-create"><h2>새 원정대 만들기</h2><p>1~4인 · 파티와 링크 전술 덱은 서버에 동기화됩니다.</p>${difficultySelector()}<div class="field"><input id="roomNameInput" maxlength="24" placeholder="원정대 이름 (선택)"><button class="cta mint" data-action="create-dungeon">방 만들기</button></div><div style="height:8px"></div><div class="field"><input id="roomCodeInput" maxlength="6" inputmode="numeric" placeholder="6자리 방 코드"><button class="cta" data-action="join-code">코드 참가</button></div></div><div class="public-rooms"><div class="section-label">OPEN ROOMS</div><h2 style="font-size:13px">공개 원정대</h2><div id="publicRooms"><p class="small-note">방 목록을 불러오는 중...</p></div></div></div></section>`);
  ...
  function renderBattle(r){
    ...
    const spellMenu=`<div class="spell-command-v51 spell-command-v52 tactic-command-v54"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>RIFT LINK TACTIC</small><b>전술을 사용하면 몬스터 행동을 소모하지 않고 다음 기술이 강화됩니다 · ${me.tacticsUsed||0}/${me.tacticLimit||2}</b></div></div><div class="tactic-guide-v54"><span>같은 속성 전술 → 다음 기술 +20% / BREAK +6</span><span>다른 속성 전술 → 다음 기술 +10% / BREAK +3</span></div><div class="spell-list-v51 spell-list-v52 tactic-list-v54">${me.hand.map((cid,i)=>{const c=runCardView(byId(state.meta.cards,cid),run),cost=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(me.buffs.spellDiscount>0?1:0))),can=!me.ended&&b.phase==='players'&&Number(me.tacticsUsed||0)<Number(me.tacticLimit||2)&&me.energy>=cost,hint=tacticHintV54(c,selectedMon,selectedEnemy);return `<button data-action="play-card" data-index="${i}" ${can?'':'disabled'}><em>${i+1}</em><span>${spellSigilIcon(c)}</span><div><b>${esc(c.name)}</b><small>${elementGlyph(c.element)} ${esc(c.element)} · ${esc(cardKeywords(c,2).join(' · ')||'전술 보조')}</small><i>${esc(hint)}</i></div><strong>${cost}E</strong></button>`;}).join('')}</div></div>`;
    ...
    const prompt=menu==='root'?'무엇을 할까?':menu==='moves'?'어떤 기술을 사용할까?':menu==='spells'?'어떤 링크 전술을 사용할까?':menu==='party'?'누구와 교대할까?':menu==='capture'?'어떤 봉인구를 사용할까?':'전투 정보';
  ...
  function rewardChoiceTileV43(o){
    if(o.type==='card'){const c=o.card;return `<button class="reward-choice-v51 rarity-${esc(c.rarity)}" data-action="reward-pick" data-reward-id="${esc(o.id)}"><span><img src="${esc(c.art||'')}" alt=""></span><div><small>${esc(c.element)} · ${c.cost}E</small><b>${esc(c.name)}</b><em>${esc(shortLine(c.text||cardKeywords(c,2).join(' · '),52))}</em></div></button>`;}
  ...
  function renderReward(r){
    ...
    if(skillOffer&&!skillDone){
      ...
      body=`<section class="reward-simple-v51"><header><small>CAMP</small><b>야영지</b></header><div class="three-choice-v51"><button data-action="camp-rest"><span>♥</span><b>휴식</b><small>HP +30%</small></button><button data-action="camp-meditate"><span>◇</span><b>명상</b><small>파편 +2</small></button><button data-action="open-forge" ${uniq.length?'':'disabled'}><span>⚒</span><b>제련</b><small>스펠 강화</small></button></div></section>`;
  ...
  function renderCollectionInner(){
    ...
    const grid=view.map(c=>{const count=state.profile.collection[c.id]||0,inDeck=state.deckDraft.includes(c.id);return `<article class="collection-card-v34 spell-v40 ${count?'owned':'unowned'} ${inDeck?'selected':''} ${c.type==='spell'?'spell-row-v43':''}"><div class="collection-art-v34">${collectionArtHtml(c)}</div><div class="collection-copy-v34"><b class="rarity-${c.rarity}" title="${esc(c.name)}">${esc(c.name)}</b><small>${c.cost}E · ${esc(cardRole(c).label)} · ${esc(c.element)}</small><div>${cardEffectRows(c,3).map(x=>`<span><i>${esc(x.icon)}</i>${esc(x.value||x.label)}</span>`).join('')}</div></div>${count?`<button class="deck-add-v34" data-action="${inDeck?'deck-remove':'deck-add'}" data-card-id="${esc(c.id)}">${inDeck?'제외':'편성'}</button>`:'<em>미획득</em>'}</article>`;}).join('');
    setScreen('collection',`<section class="loadout-page-v34 spellbook-v40"><header class="loadout-head-v34"><button class="back-btn" data-action="${state.prepMode?'prep-return':'home'}">← ${state.prepMode?'파티':'홈'}</button><div><small>SPELL GRIMOIRE</small><h1>링크 전술 덱</h1><p>몬스터는 파티에서 직접 싸우고, 카드는 공격·방어·강화·진화를 만드는 전술입니다.</p></div><div class="loadout-total-v34"><b>${state.deckDraft.length}</b><span>/${max}</span></div></header><div class="spell-deck-strip-v40">${state.deckDraft.map(id=>{const c=byId(state.meta.cards,id);return c?`<button data-action="deck-remove" data-card-id="${esc(id)}">${stripCardThumbHtml(c)}</button>`:'';}).join('')}</div><div class="collection-toolbar-v34"><div class="filter-row">${[['all','전체'],['gene','인자술'],['attack','공격'],['support','지원'],['common','일반'],['rare','희귀'],['ultra','초희귀'],['legendary','전설'],['mythic','신화']].map(([k,n])=>`<button class="filter-btn ${state.collectionFilter===k?'active':''}" data-action="filter" data-filter="${k}">${n}</button>`).join('')}</div><input class="search-input" id="collectionSearch" value="${esc(state.collectionSearch)}" placeholder="전술 검색"></div><div class="collection-grid-v34 spell-grid-v40">${grid}</div><div class="loadout-save-v34"><div><b>${min}~${max}종</b><small>현재 ${state.deckDraft.length}종</small></div><button class="primary" data-action="deck-save" ${state.deckDraft.length<min||state.deckDraft.length>max?'disabled':''}>링크 전술 덱 저장</button></div><div class="pagination"><button class="cta" data-action="page-prev" ${state.collectionPage===0?'disabled':''}>←</button><span>${state.collectionPage+1}/${pages}</span><button class="cta" data-action="page-next" ${state.collectionPage>=pages-1?'disabled':''}>→</button></div></section>`);
  }
  function addDeckCard(cardId){const c=byId(state.meta.cards,cardId);if(!c||c.type!=='spell'||!state.profile.collection?.[cardId])return;if(state.deckDraft.includes(cardId))return;const max=state.meta.spellRules?.max||12;if(state.deckDraft.length>=max)return toast(`링크 전술 덱은 최대 ${max}종입니다.`,'error');state.deckDraft.push(cardId);renderCollectionInner();sound('card');}
  ...
  async function saveDeck(){const min=state.meta.spellRules?.min||8;if(state.deckDraft.length<min)return toast(`링크 전술 덱은 최소 ${min}종이 필요합니다.`,'error');try{const d=await api('/api/deck',{body:{profileId:state.profileId,nickname:state.profile.nickname,deck:state.deckDraft}});state.profile=d.profile;state.deckDraft=[...d.profile.deck];updateTopbar();toast('링크 전술 덱 저장 완료.','good');sound('win');if(state.prepMode)return renderExpeditionPrep(state.prepMode,false);renderCollectionInner();}catch(e){toast(e.message,'error');}}
