...
(() => {
  ...
  function playRoomEvents(next,afterSeq){
    ...
    for(const ev of events){
      ...
      else if(ev.type==='monster-move')later(()=>monsterMoveEventFx(ev.payload),ev.payload?.signature?1850:1450);
      ...
      else if(ev.type==='enemy-attack')later(()=>enemyAttackEventFx(ev.payload),ev.payload?.signature?1850:ev.payload?.heavy?1550:1350);
      ...
      else if(ev.type==='status-tick')later(()=>statusTickFxV55(ev.payload),620);
  ...
  function battleCaptureChanceUi(enemy,seal,run,battle){
    ...
    if(battle?.encounterType==='boss'){if(seal!=='royal')return{chance:0,hpRatio,reason:'로열 봉인구 필요'};if(hpRatio>.25)return{chance:0,hpRatio,reason:'HP 25% 이하 필요'};chance=Math.min(chance,.40);}
  ...
  const STATUS_META_V55={
    burn:{label:'화상',glyph:'🔥'},poison:{label:'독',glyph:'☠'},shock:{label:'감전',glyph:'ϟ'},weak:{label:'공격↓',glyph:'▽'},vulnerable:{label:'방어↓',glyph:'◎'},intentSeal:{label:'행동 봉쇄',glyph:'⊘'},broken:{label:'BREAK',glyph:'✹'},stagger:{label:'BREAK 게이지',glyph:'✦'},guard:{label:'방어',glyph:'⬢'},heal:{label:'회복',glyph:'✚'},buff:{label:'능력 상승',glyph:'▲'}
  };
  ...
  function battleMoveFeedbackV51(payload,enemySide=false){
    ...
    const c=rectCenter(target),el=document.createElement('div'),d=Number(payload.damage||0),miss=payload.hit===false,mv=payload.move||{},element=mv.element||payload.element||'공허',debuffs=(payload.appliedDebuffs||[]).map(effectChipTextV54).filter(Boolean),summary=(payload.effectSummary||[]).filter(Boolean),link=payload.tacticLink;
  ...
  async function playNamedMonsterMoveV45(payload,enemySide=false){
    ...
    if(signature)monsterSignatureFxV532(actor,payload,'cast');
  ...
  function moveStatus(move,u){if(!move)return'LOCK';if(Number(u?.level||1)<Number(move.unlockLevel||1))return`Lv.${move.unlockLevel}`;if(Number(move.cooldownRemaining||0)>0)return`CD ${move.cooldownRemaining}`;let acc=Number(move.accuracy||100);if(move.rewrite?.id==='overclock')acc=Math.max(50,acc-6);const m=moveMasteryUi(move);return `${m.label?`숙련${m.label} · `:''}${move.rewrite?.icon?`${move.rewrite.icon} `:''}${acc}%`;}
  ...
  function showEvolutionMenu(instanceId){
    ...
    modal(`<div class="evolution-menu-v41 evolution-menu-v43"><div class="section-label">MONSTER EVOLUTION</div><div class="evo-hero-v41"><img src="${esc(monsterDisplaySprite(u))}"><div><small>${esc(u.element)} · Lv.${u.level}</small><h2>${esc(u.name)}</h2><p>자연 진화는 Lv.${evoLevel}에 자동으로 발생합니다. GENE을 모으면 더 일찍 진화시킬 수도 있습니다.</p></div></div><div class="evo-grid-v41"><button data-action="monster-evolve" data-mode="evolve" data-monster="${esc(u.instanceId)}" ${evoOk?'':'disabled'}><span>▲</span><b>${naturalReady?'자연 진화':'조기 진화'}</b><small>${naturalReady?`Lv.${evoLevel} 달성`:`Lv.${evoLevel} 자동 · 또는 GENE 3`}</small></button><button data-action="open-fusion" data-monster="${esc(u.instanceId)}" ${u.gene>=2&&(me.units.length+me.bench.length>1)?'':'disabled'}><span>∞</span><b>융합</b><small>양쪽 GENE 2</small></button><button data-action="monster-evolve" data-mode="resonance" data-monster="${esc(u.instanceId)}" ${resOk?'':'disabled'}><span>✦</span><b>공명진화</b><small>RES 4 · 3턴</small></button><button data-action="monster-evolve" data-mode="rift" data-monster="${esc(u.instanceId)}" ${riftOk?'':'disabled'}><span>◆</span><b>균열개화</b><small>HP 60%↓ · RIFT 3</small></button></div><button class="cta" data-action="modal-close">닫기</button></div>`);
  }
  ...
  function featureChips(){return `<div class="feature-strip"><span class="feature-chip">MONSTER HUNT</span><span class="feature-chip">315 CARDS</span><span class="feature-chip">128 ITEMS</span><span class="feature-chip">205 MONSTERS</span><span class="feature-chip">50 FLOORS</span><span class="feature-chip">1–4 CO-OP</span></div>`;}
  ...
  async function renderDungeonLobby(){
    setScreen('dungeonLobby',`<section class="page"><div class="page-head"><button class="back-btn" data-action="home">← 홈</button><div><div class="section-label">CO-OP DUNGEON</div><h1>50층 협동 던전</h1><p>내가 잡은 몬스터 + 공명 지령 덱으로 50층을 공략합니다.</p></div></div><div class="lobby-grid"><div class="room-create"><h2>새 원정대 만들기</h2><p>1~4인 · 파티와 공명 지령 덱은 서버에 동기화됩니다.</p>${difficultySelector()}<div class="field"><input id="roomNameInput" maxlength="24" placeholder="원정대 이름 (선택)"><button class="cta mint" data-action="create-dungeon">방 만들기</button></div><div style="height:8px"></div><div class="field"><input id="roomCodeInput" maxlength="6" inputmode="numeric" placeholder="6자리 방 코드"><button class="cta" data-action="join-code">코드 참가</button></div></div><div class="public-rooms"><div class="section-label">OPEN ROOMS</div><h2 style="font-size:13px">공개 원정대</h2><div id="publicRooms"><p class="small-note">방 목록을 불러오는 중...</p></div></div></div></section>`);
  ...
  function showRunInfo(){
    ...
    modal(`<div class="run-info-modal-v27 run-info-modal-v53"><div class="section-label">RUN STATUS</div><h2>${r.floor}F · ${esc(r.biome.name)}</h2><div class="run-info-grid-v27"><div><small>CORE HP</small><b>${run.hp}/${run.maxHp}</b></div><div class="money-card-v53"><small>RUN MONEY</small><b>${Number(run.gold||0).toLocaleString('ko-KR')}G</b></div><div><small>DECK</small><b>${run.runDeck?.length||0}</b></div><div><small>PRESSURE</small><b>×${(r.threatTokens||[]).length}</b></div></div><h3>몬스터 장착 아이템</h3><p class="held-help-v53">몬스터를 눌러 열매/장착 아이템을 바꿀 수 있습니다. 장착 아이템은 전투 밖에서만 변경됩니다.</p><div class="held-roster-v53">${monsters}</div><h3>유물</h3><div class="simple-chip-list-v27">${relics.length?relics.map(x=>`<span title="${esc(x.text)}">${esc(x.icon||'✦')} ${esc(x.name)}</span>`).join(''):'<span>아직 없음</span>'}</div><h3>런 아이템</h3><div class="simple-chip-list-v27">${items.length?items.map(x=>`<span title="${esc(x.item.text)}">${esc(x.item.icon||'◆')} ${esc(x.item.name)} ×${x.n}</span>`).join(''):'<span>아직 없음</span>'}</div><div class="modal-actions"><button class="cta mint" data-action="modal-close">닫기</button></div></div>`);
  }
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>{const held=heldItemClient(u);return `<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${held?` <i class="battle-held-v53" title="${esc(held.text||'')}">${esc(held.icon||'◆')} ${esc(held.name)}</i>`:''}${Number(u.tacticAmp||0)>0?` <i class="battle-link-v54">LINK +${Math.round(Number(u.tacticAmp||0)*100)}%</i>`:''}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i>${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`;}).join('');
    const enemyActors=alive.map(e=>{const it=intentCompact(e.intent);return `<button class="enemy-mon-v41 enemy-mon-v44 enemy-mon-v51 enemy-mon-v52 ${state.selectedEnemy===e.uid?'selected':''} ${e.tier==='boss'?'is-boss-v50':''}" data-action="select-enemy" data-enemy="${esc(e.uid)}"><div class="enemy-status-v51 enemy-status-v52"><div><b>${esc(e.name)}</b><small>${esc(e.element||'')} ${enemyWeakBadge(e.element)}</small></div><span>${e.hp}/${e.maxHp}</span><i><em style="width:${pct(e.hp,e.maxHp)}%"></em></i>${enemyDebuffBadgesV54(e)}</div><div class="enemy-sprite-v51 enemy-sprite-v52"><i class="enemy-shadow-v41"></i><img class="enemy-sprite" src="${esc(e.sprite)}" alt="${esc(e.name)}" decoding="async"><span class="target-ring-v41"></span></div><div class="enemy-intent-v51 enemy-intent-v52"><small>NEXT</small><b>${esc(it.icon)} ${esc(it.value||'')}</b></div></button>`;}).join('');

    const rootMenu=`<div class="root-command-v51 root-command-v52"><button class="fight" data-action="battle-menu" data-menu="moves" ${selectedMon?.acted?'disabled':''}><span>⚔</span><b>기술</b><small>몬스터 기술</small></button><button class="spell tactic-v54" data-action="battle-menu" data-menu="spells"><span>◇</span><b>공명 지령</b><small>즉시 지원 + 다음 기술 강화 · ${me.tacticsUsed||0}/${me.tacticLimit||2}</small></button><button class="party" data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length||selectedMon?.acted?'disabled':''}><span>↔</span><b>교대</b><small>${(me.bench||[]).length} 대기</small></button><button class="capture" data-action="battle-menu" data-menu="capture" ${!['wild','boss'].includes(b.encounterType)||selectedMon?.acted?'disabled':''}><span>◉</span><b>봉인</b><small>${selectedEnemy?`${Math.round(Number(selectedEnemy.hp||0)/Math.max(1,Number(selectedEnemy.maxHp||1))*100)}% HP`:'대상 없음'}</small></button></div>`;
    const moves=selectedMon?.moves||[];
    const moveMenu=`<div class="move-command-v51 move-command-v52 move-command-v54"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>${esc(selectedMon?.name||'MONSTER')}</small><b>기술을 선택하세요</b></div></div><div class="move-grid-v51 move-grid-v52">${[0,1,2,3].map(i=>{const mv=moves[i];if(!mv)return `<button class="move-btn-v51 move-btn-v52 empty" disabled><em>${i+1}</em><b>비어 있음</b></button>`;const can=moveUsable(mv,selectedMon,me),power=movePowerLine(mv),el=mv.element||selectedMon.element,tags=moveEffectTagsV54(mv);return `<button class="move-btn-v51 move-btn-v52 move-btn-v54 element-${elementClass(el)} ${mv.signature?'signature-move-v54':''}" data-action="move-use" data-move-id="${esc(mv.id)}" ${can?'':'disabled'}><em>${i+1}</em><span>${namedSkillGlyphV47(namedMoveFamily({skill:mv.name,element:el,move:mv}))}</span><div><b>${esc(mv.name)}</b><small class="move-type-line-v54">${elementGlyph(el)} ${esc(el)} · ${esc(moveKindKo(mv))} · ${Number(mv.cooldownRemaining||0)>0?`CD ${mv.cooldownRemaining}`:`명중 ${mv.accuracy||100}%`}</small><small>${esc(power||moveKindKo(mv))}</small><div class="move-effect-tags-v54">${tags.map(x=>`<i class="effect-${x.k}">${esc(x.t)}</i>`).join('')}</div></div><strong>${mv.signature?'★ SIG':moveStatus(mv,selectedMon)}</strong></button>`;}).join('')}</div></div>`;
    const spellMenu=`<div class="spell-command-v51 spell-command-v52 tactic-command-v54 tactic-command-v55"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>RIFT COMMAND</small><b>공명 지령은 행동을 소모하지 않습니다. 즉시 효과 + 다음 기술 증폭을 함께 사용하세요 · ${me.tacticsUsed||0}/${me.tacticLimit||2}</b></div></div><div class="tactic-guide-v54 tactic-guide-v55"><span>같은 속성 → 다음 기술 +20% / BREAK +6</span><span>다른 속성 → 다음 기술 +10% / BREAK +3</span></div><div class="spell-list-v51 spell-list-v52 tactic-list-v54 tactic-list-v55">${me.hand.map((cid,i)=>{const c=runCardView(byId(state.meta.cards,cid),run),cost=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(me.buffs.spellDiscount>0?1:0))),can=!me.ended&&b.phase==='players'&&Number(me.tacticsUsed||0)<Number(me.tacticLimit||2)&&me.energy>=cost,hint=tacticHintV54(c,selectedMon,selectedEnemy),effect=tacticEffectTextV55(c,selectedMon);return `<button data-action="play-card" data-index="${i}" ${can?'':'disabled'}><em>${i+1}</em><span>${spellSigilIcon(c)}</span><div><b>${esc(tacticDisplayNameV55(c))}</b><small>${elementGlyph(c.element)} ${esc(c.element)} · ${esc(hint)}</small><i>${esc(effect)}</i></div><strong>${cost}E</strong></button>`;}).join('')}</div></div>`;
    ...
    const command=menu==='moves'?moveMenu:menu==='capture'?captureMenu:menu==='spells'?spellMenu:menu==='party'?partyMenu:menu==='info'?infoMenu:rootMenu;
  ...
  function fieldServiceHtml(r,run){
    ...
    return `<div class="field-service-v51"><header><small>FIELD SUPPORT</small><b>정비소</b><span>필요할 때만 이용하세요.</span></header>${svc('mend','♥','응급 회복','CORE + 생존 몬스터 회복',damaged)}${svc('revive','✚','부활 캡슐','쓰러진 몬스터 1체 부활',ko)}${svc('charge','✦','링크 충전',`다음 전투 전술 강화 · ${charge}/2`,charge<2)}</div>`;
  }
  ...
  function rewardChoiceTileV43(o){
    if(o.type==='card'){const c=o.card;return `<button class="reward-choice-v51 rarity-${esc(c.rarity)}" data-action="reward-pick" data-reward-id="${esc(o.id)}"><span><img src="${esc(c.art||'')}" alt=""></span><div><small>${esc(c.element)} · ${c.cost}E</small><b>${esc(tacticDisplayNameV55(c))}</b><em>${esc(shortLine(c.text||cardKeywords(c,2).join(' · '),64))}</em></div></button>`;}
  ...
  function renderReward(r){
    ...
    if(skillOffer&&!skillDone){
      ...
      const uniq=[...new Set(run.runDeck||[])].map(id=>({c:byId(state.meta.cards,id),lv:Number(run.upgrades?.[id]||0)})).filter(x=>x.c&&x.lv<2);step='CAMP';
      body=`<section class="reward-simple-v51"><header><small>CAMP</small><b>야영지</b></header><div class="three-choice-v51"><button data-action="camp-rest"><span>♥</span><b>휴식</b><small>HP +30%</small></button><button data-action="camp-meditate"><span>◇</span><b>명상</b><small>파편 +2</small></button><button data-action="open-forge" ${uniq.length?'':'disabled'}><span>⚒</span><b>제련</b><small>지령 강화</small></button></div></section>`;
      ...
      step='SHOP';body=merchantHtmlCompact(r);actions=`<div class="reward-actions-v51"><button data-action="open-purge">덱 정제</button><button class="next-v51" data-action="reward-continue">${esc(nextEncounterPreview(r))}</button></div>`;
    }else{
