...
(() => {
  ...
  function renderRoute(r){
    ...
    if(!claim){
      ...
      decision=`<div class="route-loading-v52 route-loading-v53"><span>✦</span><small>${esc(node?.kicker||'NEXT BATTLE')}</small><b>${esc(node?.label||'전투 준비')}</b><p>${voted?'다른 원정대원의 준비를 기다리고 있습니다.':'준비가 끝났습니다. 버튼을 눌러 전투에 진입하세요.'}</p>${node&&!voted?`<button class="encounter-start-v53" data-action="encounter-start" data-node="${esc(node.id)}">전투 시작</button>`:'<div class="encounter-wait-v53">READY…</div>'}</div>`;
    }
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>{const held=heldItemClient(u);return `<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${held?` <i class="battle-held-v53" title="${esc(held.text||'')}">${esc(held.icon||'◆')} ${esc(held.name)}</i>`:''}${playerStatusBadgeV56(u)}${statStageBadgesV56(u)}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i>${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`;}).join('');
    ...
    const rootMenu=`<div class="root-command-v51 root-command-v52"><button class="fight" data-action="battle-menu" data-menu="moves" ${selectedMon?.acted?'disabled':''}><span>⚔</span><b>기술</b><small>몬스터 기술</small></button><button class="fusion fusion-command-v56" data-action="battle-menu" data-menu="fusion" ${selectedMon?.acted||([...(me.units||[]),...(me.bench||[])]).filter(x=>x.hp>0&&x.instanceId!==selectedMon?.instanceId).length<1?'disabled':''}><span>∞</span><b>융합</b><small>다른 몬스터와 결합 · 행동 1회</small></button><button class="party" data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length||selectedMon?.acted?'disabled':''}><span>↔</span><b>교대</b><small>${(me.bench||[]).length} 대기</small></button><button class="capture" data-action="battle-menu" data-menu="capture" ${!['wild','boss'].includes(b.encounterType)||selectedMon?.acted?'disabled':''}><span>◉</span><b>봉인</b><small>${selectedEnemy?`${Math.round(Number(selectedEnemy.hp||0)/Math.max(1,Number(selectedEnemy.maxHp||1))*100)}% HP`:'대상 없음'}</small></button></div>`;
    ...
    const prompt=menu==='root'?'무엇을 할까?':menu==='moves'?'어떤 기술을 사용할까?':menu==='fusion'?'누구와 융합할까?':menu==='party'?'누구와 교대할까?':menu==='capture'?'어떤 봉인구를 사용할까?':'전투 정보';
  ...
  function renderReward(r){
    ...
    if(skillOffer&&!skillDone){
      ...
      actions=`<div class="reward-actions-v51"><button data-action="skip-move">건너뛰기</button></div>`;
      ...
      step='CHOOSE ONE';body=`<section class="reward-simple-v51"><header><small>${r.mode==='journey'?'FIELD DROP':'RIFT DROP'}</small><b>보상 하나를 선택하세요</b></header><div class="reward-options-v51">${options.map(rewardChoiceTileV43).join('')}</div></section>`;
