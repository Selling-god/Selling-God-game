...
function staticChecks(){
  ...
  console.log('V62_STATIC_OK largeType=yes compactReward=yes liveBattleStage=yes generatedFusionAsset=yes');
}
staticChecks();
if(process.argv.includes('--static'))process.exit(0);

const profileFile=path.join(os.tmpdir(),`fusewild-v62-${process.pid}.json`);fs.writeFileSync(profileFile,'{}');
...
(async()=>{try{
  ...
  for(let round=0;round<7&&room2.status==='battle';round++){
    ...
    while(room2.status==='battle'&&room2.battle?.phase==='players'&&guard++<8){
      ...
      room2=(await req('POST',`/api/room/${room2.id}/move`,{...u,instanceId:actor.instanceId,moveId:mv.id,targetUid:foe.uid})).room;
    }
  ...
  console.log(`V62_RUNTIME_OK hp=${moveEv.payload.targetHpBefore}->${moveEv.payload.targetHpAfter} fusionArt=realPNG fiveTurnSplit=yes`);
}catch(e){console.error('V62_RUNTIME_FAIL',e);process.exitCode=1}finally{proc?.kill('SIGTERM');try{fs.unlinkSync(profileFile)}catch{}}})();
