...
for(const token of ['RIFT LINK TACTIC','move-effect-tags-v54','enemy-debuffs-v54','scheduleRewardAutoAdvanceV54','combatCinematicUntil','sameBattle','waitForCombatCinematicV54'])if(!app.includes(token))throw Error(`client token ${token}`);
...
(async()=>{try{
 const hz=await ready();if(hz.version!=='5.4.0'||hz.deployId!=='RIFT-V540-SIGNATURE-TACTIC-PACING-20260915')throw Error(`health ${hz.version}/${hz.deployId}`);
 ...
 if(hand.length){room=(await req('POST',`/api/room/${room.id}/play`,{...u,handIndex:hand[0].index,targetUid:enemy.uid,targetMonsterId:unit.instanceId})).room;pc=room.battle?.party?.find(x=>x.playerId===u.profileId);unit=pc?.units?.find(x=>x.instanceId===unit.instanceId);if(room.status==='battle'&&Number(unit?.tacticAmp||0)<=0)throw Error('RIFT LINK not charged');}
