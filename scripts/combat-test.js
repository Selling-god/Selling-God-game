...
const proc=cp.spawn(process.execPath,['server.js'],{cwd:root,env:{...process.env,PORT:'3301',PROFILE_FILE:profileFile},stdio:['ignore','ignore','inherit']});
...
(async()=>{try{
 ...
 while(room.status==='battle'&&actions<180){const me=room.battle.party[0];let played=false;for(let i=0;i<me.hand.length;i++){const c=by[me.hand[i]];if(!c)continue;const preview=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(c.type==='spell'&&me.buffs.spellDiscount>0?1:0)));if(preview>me.energy)continue;if(c.type==='unit'&&me.units.length>=3)continue;room=(await req('POST',`/api/room/${room.id}/play`,{...u,handIndex:i,targetUid:room.battle.enemies.find(e=>e.hp>0)?.uid})).room;actions++;played=true;break;}if(room.status!=='battle')break;if(!played){room=(await req('POST',`/api/room/${room.id}/end-turn`,u)).room;actions++;}}
 if(room.status!=='reward')throw Error(`battle did not resolve: ${room.status}`);const opts=room.reward.playerOptions[u.profileId];if(!opts||opts.length<5)throw Error('mixed reward options missing');if(!opts.some(o=>o.type==='card')||!opts.some(o=>o.type==='item'))throw Error('reward must mix card/item');
