...
async function toBattle(room,u){for(let n=0;n<20&&room.status!=='battle';n++){if(room.status==='route'){const node=room.route.find(x=>['combat','elite','boss'].includes(x.kind))||room.route[0];room=(await req('POST',`/api/room/${room.id}/vote`,{...u,nodeId:node.id})).room;}else if(room.status==='event'){room=(await req('POST',`/api/room/${room.id}/event`,{...u,choiceId:'safe'})).room;}else if(room.status==='reward'){const opts=room.reward.playerOptions?.[u.profileId]||[];if(opts.length&&!room.reward.claims?.[u.profileId])room=(await req('POST',`/api/room/${room.id}/reward`,{...u,rewardId:opts[0].id})).room;if(room.reward.relicOptions?.[u.profileId]?.length&&!room.reward.relicClaims?.[u.profileId])room=(await req('POST',`/api/room/${room.id}/relic`,{...u,relicId:room.reward.relicOptions[u.profileId][0].id})).room;if(room.reward.camp&&!room.reward.campBy?.[u.profileId])room=(await req('POST',`/api/room/${room.id}/camp`,{...u,mode:'rest'})).room;room=(await req('POST',`/api/room/${room.id}/continue`,u)).room;}}
 return room;}
(async()=>{try{
  const hz=await ready();if(hz.version!=='4.0.0'||hz.monsters!==205)throw Error(`health ${hz.version}/${hz.monsters}`);
  ...
  if(prof.monsterOwnedCount!==4)throw Error(`fresh starter monster count ${prof.monsterOwnedCount}`);
  const party=catalog.monsterParty.starterIds.slice(0,4);const load=(await req('POST','/api/loadout',{...u,monsterParty:party,deck:prof.deck})).profile;if(load.monsterParty.length!==4||load.monsterPartyCost!==4)throw Error('loadout save failed');
  ...
  let me=room.battle.party.find(x=>x.playerId===u.profileId);if(me.units.length!==3||me.bench.length!==1)throw Error(`active/bench ${me.units.length}/${me.bench.length}`);if(me.hand.some(cid=>meta.cards.find(c=>c.id===cid)?.type!=='spell'))throw Error('non-spell in hand');
  // Standard evolution.
  let a=me.units[0];room=(await req('POST',`/api/room/${room.id}/debug-monster`,{...u,instanceId:a.instanceId,level:3,gene:5})).room;room=(await req('POST',`/api/room/${room.id}/monster-evolve`,{...u,instanceId:a.instanceId,mode:'evolve'})).room;me=room.battle.party[0];a=me.units.find(x=>x.instanceId===a.instanceId);if(!a.evolved||!a.evolutionSprite)throw Error('standard evolution failed');
  // Resonance evolution.
  let b=me.units[1];room=(await req('POST',`/api/room/${room.id}/debug-monster`,{...u,instanceId:b.instanceId,resonance:4})).room;room=(await req('POST',`/api/room/${room.id}/monster-evolve`,{...u,instanceId:b.instanceId,mode:'resonance'})).room;me=room.battle.party[0];b=me.units.find(x=>x.instanceId===b.instanceId);if(!(b.resonanceTurns>0)||!b.resonanceSprite)throw Error('resonance evolution failed');
  // Rift Bloom.
  let c=me.units[2];room=(await req('POST',`/api/room/${room.id}/debug-monster`,{...u,instanceId:c.instanceId,rift:3,hpRatio:.5})).room;room=(await req('POST',`/api/room/${room.id}/monster-evolve`,{...u,instanceId:c.instanceId,mode:'rift'})).room;me=room.battle.party[0];c=me.units.find(x=>x.instanceId===c.instanceId);if(!c.abyssBloom||!c.riftSprite)throw Error('rift bloom failed');
  // Switch active <-> bench and then fuse two monsters.
  const originalActive=a.instanceId,benchId=me.bench[0].instanceId;room=(await req('POST',`/api/room/${room.id}/switch-monster`,{...u,activeId:originalActive,benchId})).room;me=room.battle.party[0];if(!me.units.some(x=>x.instanceId===benchId)||!me.bench.some(x=>x.instanceId===originalActive))throw Error('switch failed');
  ...
  console.log(`V40_MONSTER_EVOLUTION_OK monsters=${monsters.length} forms=${formHashes.size} spells=${catalog.cards.length} geneStyles=${new Set(genes.map(c=>c.geneStyle)).size} party=4P transforms=4 relicArt=${relicHashes.size}`);
