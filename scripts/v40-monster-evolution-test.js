...
async function toBattle(room,u){for(let n=0;n<20&&room.status!=='battle';n++){if(room.status==='route'){const node=room.route.find(x=>['combat','elite','boss'].includes(x.kind))||room.route[0];room=(await req('POST',`/api/room/${room.id}/vote`,{...u,nodeId:node.id})).room;}else if(room.status==='event'){room=(await req('POST',`/api/room/${room.id}/event`,{...u,choiceId:'safe'})).room;}else if(room.status==='reward'){const opts=room.reward.playerOptions?.[u.profileId]||[];if(opts.length&&!room.reward.claims?.[u.profileId])room=(await req('POST',`/api/room/${room.id}/reward`,{...u,rewardId:opts[0].id})).room;if(room.reward.relicOptions?.[u.profileId]?.length&&!room.reward.relicClaims?.[u.profileId])room=(await req('POST',`/api/room/${room.id}/relic`,{...u,relicId:room.reward.relicOptions[u.profileId][0].id})).room;if(room.reward.camp&&!room.reward.campBy?.[u.profileId])room=(await req('POST',`/api/room/${room.id}/camp`,{...u,mode:'rest'})).room;room=(await req('POST',`/api/room/${room.id}/continue`,u)).room;}}
 return room;}
(async()=>{try{
  const hz=await ready();if(!hz.version||hz.monsters!==205)throw Error(`health ${hz.version}/${hz.monsters}`);
  ...
  const primary=me.units[0],secondary=me.bench[0];room=(await req('POST',`/api/room/${room.id}/debug-monster`,{...u,instanceId:primary.instanceId,gene:2})).room;room=(await req('POST',`/api/room/${room.id}/debug-monster`,{...u,instanceId:secondary.instanceId,gene:2})).room;const beforeCount=room.battle.party[0].units.length+room.battle.party[0].bench.length;room=(await req('POST',`/api/room/${room.id}/monster-fuse`,{...u,primaryId:primary.instanceId,secondaryId:secondary.instanceId})).room;me=room.battle.party[0];const fused=[...me.units,...me.bench].find(x=>x.instanceId===primary.instanceId);if(!fused?.fused||!fused.secondaryElement||!fused.secondaryArchetype)throw Error('fusion failed');if(me.units.length+me.bench.length!==beforeCount-1)throw Error('fusion did not consume partner');if(/undefined/.test(fused.name))throw Error(`fusion display name is broken: "${fused.name}" (regression: FUSION_CODENAMES negative-index bug)`);
  // Switch the remaining bench monster into the single active slot.
  // NOTE: the bench monster here (`c`) was deliberately set to hpRatio:.5 above for the
  // Rift Bloom check. Switching consumes the turn and the enemy strikes immediately
  // (single-active mode ends the turn as soon as the lone active unit has acted), so
  // testing the switch itself on a half-HP monster makes this assertion a coin flip on
  // enemy damage rolls rather than a check of switch-monster's own correctness. Top it
  // back up first so this step verifies the switch mechanic, not incoming damage variance.
  if(me.bench.length){const benchMon=me.bench[0];if(benchMon.hp<benchMon.maxHp){room=(await req('POST',`/api/room/${room.id}/debug-monster`,{...u,instanceId:benchMon.instanceId,hpRatio:1})).room;me=room.battle.party[0];}
    const old=me.units[0].instanceId,benchId=me.bench[0].instanceId;room=(await req('POST',`/api/room/${room.id}/switch-monster`,{...u,activeId:old,benchId})).room;me=room.battle.party[0];if(!me.units.some(x=>x.instanceId===benchId))throw Error('single-active switch failed');}
