...
ok(app.includes('root-command-v51'),'missing v51 root command');
ok(app.includes('move-grid-v51'),'missing v51 move grid');
ok(app.includes('battle-resource-v51'),'missing compact battle resource HUD');
...
ok(css.includes('.command-box-v51'),'v51 command css missing');ok(css.includes('.move-btn-v51'),'v51 move css missing');ok(css.includes('@keyframes v47GlyphPop'),'v47 fx animation missing');
...
console.log(`V51_PREMIUM_LOOP_OK monsters=${mons.length} uniqueSprites=${hashes.size} forms=${formFiles.length} moveFamilies=20 battleCapture=on oneScreenUI=on`);
