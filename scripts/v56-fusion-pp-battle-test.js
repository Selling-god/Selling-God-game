...
function staticChecks(){
  ...
  console.log(`V56_STATIC_OK monsters=${monsters.length} signatures=${sigNames.size} fxFamilies=${fxFamilies.size}`);
}
staticChecks();
if(process.argv.includes('--static'))process.exit(0);

const profileFile=path.join(os.tmpdir(),`riftdeck-v56-${process.pid}.json`);fs.writeFileSync(profileFile,'{}');
...
(async()=>{try{
  ...
  let room=(await req('POST','/api/rooms/create',{...u,mode:'journey',difficulty:'normal',name:'V56 FUSION PP'})).room;
  ...
  console.log(`V56_FUSION_PP_BATTLE_OK room=${room.id} floor=${room.floor} fused=${runMon.name} signatures=205 fxFamilies=${staticFxFamilyCount}`);
}catch(e){console.error('V56_FUSION_PP_BATTLE_FAIL',e);process.exitCode=1}finally{proc?.kill('SIGTERM');try{fs.unlinkSync(profileFile)}catch{}}})();
