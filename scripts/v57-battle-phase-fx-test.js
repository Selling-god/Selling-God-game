...
function staticChecks(){
  ...
  for(const token of ['fusionDraft','openFusionBuilderV56','TURN FUSION','PP ${pp}/${max}','function moveSpectacleV56','function playerStatusEventFxV56','HP / PP KEEP','FULL RECOVERY EVERY 10 WAVES / TURN FUSION','battleTimelineUntil','battleExitPending','function nextRerollCost','function battleNarrateV57','function canvasMoveFxV57','function statusCinematicV57','function moveResultNarrationsV57','reward-dialog-v57'])if(!app.includes(token))throw Error(`app token ${token}`);
  ...
  console.log(`V57_STATIC_OK monsters=${monsters.length} signatures=${sigNames.size} fxFamilies=${fxFamilies.size} sleep=${sleepCount} freeze=${freezeCount}`);
}
staticChecks();
if(process.argv.includes('--static'))process.exit(0);

const profileFile=path.join(os.tmpdir(),`riftdeck-v57-${process.pid}.json`);fs.writeFileSync(profileFile,'{}');
...
(async()=>{try{
  proc=spawn();const hz=await ready();if(hz.version!=='5.7.0'||hz.deployId!=='RIFT-V570-BATTLE-PHASE-FX-20260916')throw Error(`health ${hz.version}/${hz.deployId}`);
  const u={profileId:'v57-bot',nickname:'V57BOT'};await req('POST','/api/profile',u);
  ...
  let room=(await req('POST','/api/rooms/create',{...u,mode:'journey',difficulty:'normal',name:'V57 BATTLE PHASE'})).room;
  ...
  console.log(`V57_BATTLE_PHASE_FX_OK room=${room.id} floor=${room.floor} fused=${runMon.name} signatures=205 fxFamilies=${staticFxFamilyCount}`);
}catch(e){console.error('V57_BATTLE_PHASE_FX_FAIL',e);process.exitCode=1}finally{proc?.kill('SIGTERM');try{fs.unlinkSync(profileFile)}catch{}}})();
