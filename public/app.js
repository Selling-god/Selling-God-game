...
(() => {
  ...
  function rewardTokenV64(o){
    if(o.type==='card')return `<button class="afterbattle-token-v64 reward-token-v64 legacy" data-action="reward-salvage" data-reward-focus-v64="1" data-focus-name="균열 파편" data-focus-meta="LEGACY DATA" data-focus-desc="이전 버전 카드 보상을 균열 파편으로 변환합니다."><span>◇</span><b>균열 파편</b><small>FREE</small></button>`;
    const it=o.item,art=it.art||'',visual=art?`<img src="${esc(art)}" alt="">`:`<i>${esc(it.icon||'◆')}</i>`;
    return `<button class="afterbattle-token-v64 reward-token-v64 rarity-${esc(it.rarity)}" data-action="reward-pick" data-reward-id="${esc(o.id)}" data-reward-focus-v64="1" data-focus-name="${esc(it.name)}" data-focus-meta="${esc(it.category||rarityKo(it.rarity))} · 무료 보상" data-focus-desc="${esc(it.text||'')}"><span>${visual}</span><b>${esc(it.name)}</b><small>FREE</small></button>`;
  }
  function merchantTokenV64(r,x){
    const run=r.runState[state.profileId],bought=r.reward?.purchased?.[`${state.profileId}:${x.id}`],obj=x.type==='card'?x.card:x.item,art=obj.art||'',visual=art?`<img src="${esc(art)}" alt="">`:`<i>${esc(obj.icon||'◆')}</i>`,disabled=bought||run.gold<x.price;
    return `<button class="afterbattle-token-v64 shop-token-v64 rarity-${esc(obj.rarity)}" data-action="buy" data-item="${esc(x.id)}" data-type="${esc(x.type)}" data-reward-focus-v64="1" data-focus-name="${esc(obj.name)}" data-focus-meta="${esc(x.type==='card'?'RIFT COMMAND':obj.category||'SHOP')} · ${Number(x.price).toLocaleString('ko-KR')}G" data-focus-desc="${esc(obj.text||'')}" ${disabled?'disabled':''}><span>${visual}</span><b>${esc(obj.name)}</b><small>${bought?'SOLD':`${Number(x.price).toLocaleString('ko-KR')}G`}</small></button>`;
  }
