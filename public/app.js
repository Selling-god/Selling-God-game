...
(() => {
  ...
  const state = {
    ...
    combatCinematicUntil: 0,
    combatRenderTimer: 0,
    rewardAutoTimer: 0,
    battleTimelineUntil: 0,
    battleMessageTimer: 0,
    battleMessageNonce: 0,
    battleMessageText: '',
  ...
  function cardFxVariant(card){const id=String(card?.id||card?.name||'rift');let n=2166136261;for(let i=0;i<id.length;i++){n^=id.charCodeAt(i);n=Math.imul(n,16777619);}return (n>>>0)%12;}
  function cardFxSignature(card){const id=String(card?.id||card?.name||'rift');let n=2166136261;for(let i=0;i<id.length;i++){n^=id.charCodeAt(i);n=Math.imul(n,16777619);}n>>>=0;return{variant:n%12,spokes:5+(n%7),rotation:n%360,distance:52+(n%34),ringScale:.82+((n>>>5)%34)/100,speed:480+((n>>>9)%190),tilt:-24+((n>>>13)%49)};}
  function spellCastSealFx(anchor,card,phase='cast'){if(!fxOn()||!anchor||!card)return;const c=rectCenter(anchor),sig=cardFxSignature(card),el=document.createElement('div');el.className=`spell-cast-seal-v44 sig-${elementClass(card.element)}-v31 mode-${cardFxMode(card)}-v44 trait-${cardFxTrait(card)}-v44 variant-${sig.variant}-v44 ${phase}`;el.style.left=`${c.x}px`;el.style.top=`${c.y}px`;el.style.setProperty('--sig-rot',`${sig.rotation}deg`);el.style.setProperty('--sig-scale',String(sig.ringScale));el.style.setProperty('--sig-speed',`${sig.speed}ms`);el.innerHTML=`<i class="seal-ring-v44 a"></i><i class="seal-ring-v44 b"></i><i class="seal-ring-v44 c"></i><b>${esc(spellSigilIcon(card))}</b>${Array.from({length:sig.spokes},(_,i)=>`<em style="--a:${sig.rotation+(360/sig.spokes)*i}deg;--d:${sig.distance+(i%3)*8}px"></em>`).join('')}`;fxRoot().appendChild(el);setTimeout(()=>el.remove(),sig.speed+260);}
  ...
  function signatureImpactFx(target,card,strength=1){
    if(!fxOn()||!target)return;const c=rectCenter(target),root=fxRoot(),ec=elementClass(card?.element),mode=cardFxMode(card),heavy=cardFxHeavy(card),sig=cardFxSignature(card);const el=document.createElement('div');
    el.className=`signature-impact-v31 sig-${ec}-v31 mode-${mode}-v31 trait-${cardFxTrait(card)}-v31 variant-${sig.variant}-v31 ${heavy?'heavy':''}`;el.style.left=`${c.x}px`;el.style.top=`${c.y}px`;el.style.setProperty('--fx-scale',String(Math.max(.75,Math.min(1.7,strength))));el.style.setProperty('--sig-rot',`${sig.rotation}deg`);el.style.setProperty('--fx-distance',`${sig.distance}px`);el.style.setProperty('--fx-speed',`${sig.speed}ms`);el.style.setProperty('--sig-tilt',`${sig.tilt}deg`);
    el.innerHTML=`<i class="fx-core-v31"></i><i class="fx-ring-v31 r1"></i><i class="fx-ring-v31 r2"></i>${Array.from({length:sig.spokes},(_,i)=>`<b class="fx-bit-v31" style="--a:${sig.rotation+(360/sig.spokes)*i}deg;--delay:${(i%4)*18}ms"></b>`).join('')}`;
    root.appendChild(el);soundElement(card?.element,heavy);setTimeout(()=>el.remove(),heavy?820:680);
  }
  function signatureProjectileFx(fromEl,targetEl,card){
    if(!fxOn()||!fromEl||!targetEl)return Promise.resolve();const a=rectCenter(fromEl),b=rectCenter(targetEl),dx=b.x-a.x,dy=b.y-a.y,root=fxRoot(),heavy=cardFxHeavy(card),ec=elementClass(card?.element),sig=cardFxSignature(card),el=document.createElement('div');
    el.className=`signature-projectile-v31 sig-${ec}-v31 trait-${cardFxTrait(card)}-v31 variant-${sig.variant}-v31 ${heavy?'heavy':''}`;el.style.left=`${a.x}px`;el.style.top=`${a.y}px`;el.style.setProperty('--ang',`${Math.atan2(dy,dx)}rad`);el.style.setProperty('--sig-rot',`${sig.rotation}deg`);el.style.setProperty('--sig-tilt',`${sig.tilt}deg`);el.innerHTML='<i></i><b></b><em></em>';root.appendChild(el);
    const dur=heavy?320:245;try{const anim=el.animate([{transform:'translate3d(0,0,0) scale(.65)',opacity:0},{transform:`translate3d(${dx*.12}px,${dy*.12}px,0) scale(1)`,opacity:1,offset:.16},{transform:`translate3d(${dx}px,${dy}px,0) scale(${heavy?1.25:1})`,opacity:1}],{duration:dur,easing:'cubic-bezier(.16,.78,.18,1)',fill:'forwards'});anim.onfinish=()=>el.remove();}catch{setTimeout(()=>el.remove(),dur+20);}
  ...
  async function cardCastWindup(cardEl,card,targetEl){
    ...
    const from=cardEl.getBoundingClientRect(),root=fxRoot(),clone=cardEl.cloneNode(true);spellCastSealFx(cardEl,card,'origin');clone.classList.add('cast-card-clone','visual-cast-v31','visual-cast-v33');if(cardEl.classList.contains('spell-command-tile-v44'))clone.classList.add('spell-command-cast-v44');clone.style.left=`${from.left}px`;clone.style.top=`${from.top}px`;clone.style.width=`${from.width}px`;clone.style.height=`${from.height}px`;root.appendChild(clone);cardEl.classList.add('card-committed');
    const cx=innerWidth*.5-(from.left+from.width/2),cy=innerHeight*.50-(from.top+from.height/2);try{await clone.animate([{transform:'translate3d(0,0,0) scale(1)',opacity:1},{transform:`translate3d(${cx}px,${cy}px,0) scale(${heavy?1.22:1.12}) rotate(${motion==='melee'?-5:2}deg)`,opacity:1}],{duration:145,easing:'cubic-bezier(.18,.8,.2,1)',fill:'forwards'}).finished;}catch{await wait(115);}spellCastSealFx(clone,card,'center');
    let target=targetEl;
  ...
  function monsterAttackEventFx(payload){
    if(!fxOn()||!payload)return;const actor=$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`)||$('.active-mon-v51.selected,.active-mon-v51'),target=$(`[data-enemy="${CSS.escape(payload.targetUid||'')}"]`)||$('.enemy-mon-v51.selected,.enemy-mon-v51');if(!actor||!target)return;
    const transformed=payload.form&&payload.form!=='BASE',baseStyle=payload.archetype||'beast',style=moveFxStyle(payload,baseStyle),fake=payloadFxCard(payload,transformed||Number(payload?.damage||0)>=18);
    attackLabelV40(actor,payload.skill||'ATTACK','ally');attackAfterimageV40(actor,1);actor.classList.add('attacking-v40');
    try{actor.animate(motionFramesV40(baseStyle,1,transformed),{duration:transformed?690:570,easing:'cubic-bezier(.14,.82,.16,1)'});}catch{}
    setTimeout(async()=>{
      if(style==='heal'){healBloomFx(actor,fake);signatureImpactFx(actor,fake,.9);}
      else if(style==='guard'){shieldGainFx(actor,Math.max(6,Number(payload?.damage||0)));signatureImpactFx(actor,fake,.92);}
      else if(style==='eruption'){await eruptionFx(target,fake);screenShake(transformed?'heavy':'hit');}
      else if(style==='beam'){await beamFx(actor,target,fake);}
      else if(style==='projectile'){await signatureProjectileFx(actor,target,fake);signatureImpactFx(target,fake,transformed?1.45:1.05);}
      else if(style==='wave'){await waveFx(actor,target,fake);}
      else {await meleeArcFx(target,fake,transformed||['assassin','knight','crab'].includes(baseStyle));}
      if(style!=='heal'&&style!=='guard')popNumber(target,`-${payload.damage||0}`,'damage');if(transformed)screenFlash('break');
    },style==='beam'?220:150);
    setTimeout(()=>actor.classList.remove('attacking-v40'),820);
  }
  function enemyAttackEventFx(payload){return playNamedMonsterMoveV45(payload,true);}
  function monsterBurstEventFx(payload){
    if(!payload)return;const actor=$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`)||$('.active-mon-v51.selected,.active-mon-v51'),target=$(`[data-enemy="${CSS.escape(payload.targetUid||'')}"]`)||$('.enemy-mon-v51.selected,.enemy-mon-v51');if(!actor||!target)return;attackLabelV40(actor,payload.skill||'LINK BURST','ally');screenFlash('break');attackAfterimageV40(actor,1);try{actor.animate([{transform:'scale(1)'},{transform:'scale(1.20)',filter:'brightness(1.8)',offset:.25},{transform:'translateX(92px) scale(1.08)',offset:.52},{transform:'scale(1)'}],{duration:620,easing:'cubic-bezier(.12,.86,.16,1)'});}catch{}setTimeout(()=>signatureProjectileFx(actor,target,{element:payload.element||'빛',rarity:'legendary',power:payload.damage||12,effects:[{op:'damage',value:payload.damage||12}]}).then(()=>{signatureImpactFx(target,{element:payload.element||'빛',rarity:'legendary',power:payload.damage||12,effects:[{op:'damage',value:payload.damage||12}]},1.55);popNumber(target,`-${payload.damage||0}`,'damage');screenShake('hit');}),170);
  }

  async function playEvolutionCinematic(payload){
    if(!fxOn())return;const fromSprite=payload?.fromSprite||'',toSprite=payload?.toSprite||fromSprite,fromName=payload?.fromName||'MONSTER',toName=payload?.toName||fromName;
    const root=document.createElement('div');root.className='evolution-cinematic-v43';root.innerHTML=`<div class="evo-space-v43"><div class="evo-stars-v43">${Array.from({length:28},(_,i)=>`<i style="--i:${i}"></i>`).join('')}</div><div class="evo-ring-v43 r1"></div><div class="evo-ring-v43 r2"></div><div class="evo-ring-v43 r3"></div><div class="evo-beam-v43"></div><div class="evo-mon-v43"><img class="old" src="${esc(fromSprite)}" alt=""><img class="new" src="${esc(toSprite)}" alt=""></div><div class="evo-copy-v43"><small>${payload?.natural?'LEVEL EVOLUTION':'RIFT EVOLUTION'}</small><b>${esc(fromName)}</b><span>변화의 파동이 응축되고 있다…</span><strong>${esc(toName)}</strong></div></div>`;document.body.appendChild(root);sound('overdrive');requestAnimationFrame(()=>root.classList.add('charge'));await wait(1050);root.classList.add('cycle');screenShake('soft');await wait(1450);root.classList.add('reveal');screenFlash('victory');screenShake('heavy');sound('win');await wait(1350);root.classList.add('finish');await wait(420);root.remove();
  }
  async function playLevelUpCinematic(payload){
    if(!fxOn()||!payload)return;const root=document.createElement('div'),gain=payload.statGain||{},unlocked=payload.unlocked||[];root.className='levelup-cinematic-v44';root.innerHTML=`<div class="levelup-panel-v44"><div class="levelup-sprite-v44"><img src="${esc(payload.sprite||'')}" alt=""><i></i></div><div class="levelup-copy-v44"><small>LEVEL UP</small><b>${esc(payload.monsterName||'MONSTER')}</b><strong><em>Lv.${payload.fromLevel||Math.max(1,(payload.level||1)-1)}</em><span>→</span>Lv.${payload.level||''}</strong><div><span>HP <b>+${Number(gain.hp||0)}</b></span><span>ATK <b>+${Number(gain.power||0)}</b></span>${payload.levelCap?`<span>LEVEL CAP <b>${payload.levelCap}</b></span>`:''}</div>${unlocked.length?`<p>NEW MOVE · ${esc(unlocked.join(' / '))}</p>`:'<p>전투 능력이 상승했습니다.</p>'}</div></div>`;document.body.appendChild(root);sound('reward');requestAnimationFrame(()=>root.classList.add('show'));await wait(1080);root.classList.add('out');await wait(220);root.remove();
  }
  async function playMoveMasteryCinematic(payload){
    if(!fxOn()||!payload)return;const root=document.createElement('div');root.className=`move-evolve-cinematic-v44 element-${elementClass(payload.element||'')}`;root.innerHTML=`<div><small>SKILL MASTERY</small><span>${moveKindIcon({kind:payload.kind})}</span><b>${esc(payload.moveName||'MOVE')} ${esc(payload.label||'')}</b><em>사용 ${payload.uses||0}회 · 기술의 위력과 안정성이 진화했습니다.</em></div>`;document.body.appendChild(root);sound('overdrive');requestAnimationFrame(()=>root.classList.add('show'));screenFlash('break');await wait(1050);root.classList.add('out');await wait(220);root.remove();
  }

  function monsterTransformFx(payload){
    if(payload?.mode==='evolve'&&payload?.toSprite){playEvolutionCinematic(payload);return;}
  ...
  const BATTLE_PACE_V57={turn:320,announce:560,move:820,signature:1180,result:420,status:560,blocked:620,win:900,minor:360};
  function battleNarrateV57(text,{tone='normal',tag='BATTLE',sub='',instant=false}={}){
    ...
    if(instant||chars.length<2){b.textContent=chars.join('');return;}let i=0;const speed=Math.max(14,Math.min(30,Math.round(700/Math.max(1,chars.length))));
  ...
  function statusNarrationV57(key,name,mode='apply',damage=0){
    ...
    if(mode==='blocked'){if(key==='paralysis'||key==='shock')return `${n}은(는) 마비되어 움직일 수 없다!`;if(key==='sleep')return `${n}은(는) 깊이 잠들어 있다…`;if(key==='freeze')return `${n}은(는) 얼어붙어 움직일 수 없다!`;return `${n}은(는) 움직일 수 없다!`;}
    if(mode==='cure'){if(key==='sleep')return `${n}은(는) 잠에서 깨어났다!`;if(key==='freeze')return `${n}의 얼음이 녹았다!`;if(key==='paralysis'||key==='shock')return `${n}의 마비가 풀렸다!`;return `${n}의 ${label} 상태가 풀렸다!`;}
    if(key==='burn')return `${n}은(는) 화상을 입었다!`;if(key==='poison')return `${n}은(는) 중독되었다!`;if(key==='paralysis'||key==='shock')return `${n}은(는) 마비되었다!`;if(key==='sleep')return `${n}은(는) 잠들어 버렸다!`;if(key==='freeze')return `${n}은(는) 꽁꽁 얼어붙었다!`;if(key==='atkDown'||key==='weak')return `${n}의 공격력이 떨어졌다!`;if(key==='defDown'||key==='vulnerable')return `${n}의 방어력이 떨어졌다!`;if(key==='atkUp')return `${n}의 공격력이 올랐다!`;if(key==='defUp')return `${n}의 방어력이 올랐다!`;if(key==='intentSeal')return `${n}의 행동이 봉쇄되었다!`;if(key==='broken')return `${n}의 자세가 무너졌다!`;return `${n}에게 ${label} 효과가 생겼다!`;
  }
  function fxColorV57(element){return({'화염':['#ff684c','#ffd467'],'물':['#46cfff','#d4fbff'],'자연':['#5fe78c','#e8ffc8'],'빛':['#fff08d','#ffffff'],'그림자':['#925cff','#24113f'],'강철':['#d7e1e8','#73859b'],'바람':['#a8fff1','#5bd1dc'],'번개':['#fff36e','#67dfff'],'별':['#ff92ee','#8d8cff'],'시간':['#74f0da','#eac8ff'],'공허':['#db6cff','#290f45'],'수정':['#9ceeff','#e9fdff']})[element]||['#9ff1df','#ffffff'];}
  function seededV57(seed){let h=2166136261;for(const ch of String(seed||'')){h^=ch.charCodeAt(0);h=Math.imul(h,16777619);}return()=>{h+=0x6D2B79F5;let t=h;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);return((t^t>>>14)>>>0)/4294967296;};}
  function canvasMoveFxV57(actor,target,family,payload,enemySide=false){
    if(!fxOn()||!actor||!target)return;const dpr=Math.min(1.25,window.devicePixelRatio||1),cv=document.createElement('canvas'),ctx=cv.getContext('2d');if(!ctx)return;cv.className=`combat-fx-canvas-v57 fx-${family} ${payload?.signature?'signature':''}`;cv.width=Math.round(innerWidth*dpr);cv.height=Math.round(innerHeight*dpr);cv.style.width=`${innerWidth}px`;cv.style.height=`${innerHeight}px`;document.body.appendChild(cv);ctx.scale(dpr,dpr);
    const A=rectCenter(actor),T=rectCenter(target),[c1,c2]=fxColorV57(payload?.element||payload?.move?.element||'공허'),rng=seededV57(payload?.fxSeed||`${family}:${payload?.skill}`),sig=!!payload?.signature||!!payload?.move?.signature,dur=sig?1150:900,variant=Number(payload?.fxVariant||0)%8,parts=Array.from({length:sig?18:12},()=>({r:rng(),a:rng()*Math.PI*2,s:.5+rng()*1.2,x:rng(),y:rng()}));
    const line=(x1,y1,x2,y2,w,col,alpha=1)=>{ctx.globalAlpha=alpha;ctx.strokeStyle=col;ctx.lineWidth=w;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x1,y1);ctx.lineTo(x2,y2);ctx.stroke();};
    const glow=()=>{ctx.shadowBlur=sig?28:18;ctx.shadowColor=c1;};const start=performance.now();
    function draw(now){const p=Math.min(1,(now-start)/dur),fade=Math.max(0,1-p),dx=T.x-A.x,dy=T.y-A.y;ctx.clearRect(0,0,innerWidth,innerHeight);ctx.save();ctx.globalCompositeOperation='lighter';glow();
      if(family==='beam'){const q=Math.min(1,p*1.8),ex=A.x+dx*q,ey=A.y+dy*q;line(A.x,A.y,ex,ey,sig?18:11,c1,.32+fade*.5);line(A.x,A.y,ex,ey,sig?7:4,c2,.95);ctx.fillStyle=c2;ctx.beginPath();ctx.arc(A.x,A.y,8+18*Math.min(1,p*3),0,Math.PI*2);ctx.fill();}
      else if(family==='slash'||family==='claw'){const n=family==='claw'?3:2;for(let i=0;i<n;i++){const off=(i-(n-1)/2)*22,ang=(variant%2?1:-1)*.72;const len=80+(sig?55:25);const q=Math.min(1,Math.max(0,(p-i*.08)*2.3));const cx=T.x+off*.35,cy=T.y+off;line(cx-len*.5,cy-len*.5*ang,cx+len*(q-.5),cy+len*(q-.5)*ang,sig?9:6,i%2?c2:c1,fade);}}
      else if(family==='bite'){const open=(1-Math.min(1,p*2))*45;ctx.strokeStyle=c1;ctx.lineWidth=sig?10:7;ctx.beginPath();ctx.arc(T.x,T.y-open,62+(sig?24:0),.15*Math.PI,.85*Math.PI);ctx.stroke();ctx.beginPath();ctx.arc(T.x,T.y+open,62+(sig?24:0),1.15*Math.PI,1.85*Math.PI);ctx.stroke();}
      else if(family==='rush'){for(let i=0;i<14;i++){const k=(p+i/14)%1,yy=A.y+(T.y-A.y)*k+(i-7)*7;line(A.x+(T.x-A.x)*k-90,yy,A.x+(T.x-A.x)*k+18,yy,2+(i%3),i%2?c1:c2,fade*.7);}ctx.fillStyle=c2;ctx.beginPath();ctx.arc(A.x+dx*Math.min(1,p*1.7),A.y+dy*Math.min(1,p*1.7),sig?22:14,0,Math.PI*2);ctx.fill();}
      else if(family==='bullet'){for(let i=0;i<(sig?7:4);i++){const q=Math.max(0,Math.min(1,p*1.7-i*.10)),x=A.x+dx*q,y=A.y+dy*q+Math.sin((q+i)*8)*8;ctx.fillStyle=i%2?c2:c1;ctx.beginPath();ctx.arc(x,y,sig?8:5,0,Math.PI*2);ctx.fill();}}
      else if(family==='thunder'){for(let j=0;j<(sig?3:2);j++){ctx.strokeStyle=j?c1:c2;ctx.lineWidth=sig?7-j*2:5-j;ctx.beginPath();ctx.moveTo(T.x+(j-1)*25,T.y-150);for(let i=1;i<=9;i++)ctx.lineTo(T.x+(rng()-.5)*80+(j-1)*18,T.y-150+i*34);ctx.stroke();}ctx.fillStyle='rgba(255,255,255,.35)';ctx.fillRect(0,0,innerWidth,innerHeight);}
      else if(family==='flame'){for(const [i,v] of parts.entries()){const q=Math.min(1,p*1.45+v.r*.22),x=A.x+dx*q+(v.x-.5)*70,y=A.y+dy*q-(Math.sin(q*Math.PI)*70)-(v.y*55);ctx.fillStyle=i%2?c1:c2;ctx.globalAlpha=fade*(.35+v.r*.65);ctx.beginPath();ctx.arc(x,y,4+v.s*6,0,Math.PI*2);ctx.fill();}}
      else if(family==='ice'){for(const [i,v] of parts.slice(0,sig?18:11).entries()){const q=Math.min(1,p*1.8),rad=(25+v.r*(sig?90:65))*q,ang=v.a;ctx.save();ctx.translate(T.x+Math.cos(ang)*rad,T.y+Math.sin(ang)*rad);ctx.rotate(ang+p*2);ctx.fillStyle=i%2?c1:c2;ctx.beginPath();ctx.moveTo(0,-14*v.s);ctx.lineTo(6*v.s,7*v.s);ctx.lineTo(-6*v.s,7*v.s);ctx.closePath();ctx.fill();ctx.restore();}}
      else if(family==='wave'){for(let j=0;j<4;j++){ctx.strokeStyle=j%2?c1:c2;ctx.lineWidth=4+j;ctx.globalAlpha=fade;ctx.beginPath();for(let i=0;i<=24;i++){const q=i/24,x=A.x+dx*Math.min(1,p*1.2)*q,y=A.y+dy*q+Math.sin(q*12+p*8+j)*18+(j-1.5)*14;i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.stroke();}}
      else if(family==='cyclone'){for(let i=0;i<(sig?36:24);i++){const q=Math.max(0,p-i*.006),ang=i*.75+p*11,rad=(18+i*2.3)*(1-q*.35),x=T.x+Math.cos(ang)*rad,y=T.y+Math.sin(ang)*rad*.45;ctx.fillStyle=i%2?c1:c2;ctx.globalAlpha=fade;ctx.fillRect(x,y,5,3);}}
      else if(family==='root'){for(let j=0;j<(sig?7:5);j++){const x=T.x+(j-3)*26,top=T.y-30-(j%2)*25,q=Math.min(1,p*1.8);ctx.strokeStyle=j%2?c1:c2;ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(x,T.y+100);for(let i=1;i<=5;i++)ctx.lineTo(x+Math.sin(i+j)*15,T.y+100-(T.y+100-top)*q*i/5);ctx.stroke();}}
      else if(family==='spore'||family==='toxin'){for(const [i,v] of parts.entries()){const q=Math.min(1,p*1.3+v.r*.18),x=T.x+(v.x-.5)*(sig?190:130),y=T.y+80-(q*(130+v.y*80));ctx.fillStyle=family==='toxin'?(i%2?'#d26cff':'#70f398'):(i%2?c1:c2);ctx.globalAlpha=fade*(.45+v.r*.5);ctx.beginPath();ctx.arc(x,y,4+v.s*5,0,Math.PI*2);ctx.fill();}}
      else if(family==='shadow'){for(let i=0;i<7;i++){ctx.strokeStyle=i%2?c1:c2;ctx.lineWidth=3+i*.6;ctx.globalAlpha=fade*.8;ctx.beginPath();ctx.arc(T.x,T.y,18+i*12+p*26,Math.PI*(p*2+i*.15),Math.PI*(p*2+i*.15+1.25));ctx.stroke();}}
      else if(family==='meteor'){for(let i=0;i<(sig?9:6);i++){const delay=i*.07,q=Math.max(0,Math.min(1,(p-delay)*1.7)),x=T.x+(i-3)*36+(variant-3)*3,y=T.y-220+230*q;line(x-70,y-100,x,y,5,i%2?c1:c2,fade);ctx.fillStyle=i%2?c1:c2;ctx.beginPath();ctx.arc(x,y,8+(sig?4:0),0,Math.PI*2);ctx.fill();}}
      else if(family==='quake'){ctx.strokeStyle=c1;ctx.lineWidth=5;for(let j=0;j<8;j++){ctx.beginPath();ctx.moveTo(T.x,T.y+35);let x=T.x,y=T.y+35;for(let i=1;i<6;i++){const ang=(j/8)*Math.PI*2+(rng()-.5)*.35,len=(20+i*9)*Math.min(1,p*2);x+=Math.cos(ang)*len/5;y+=Math.sin(ang)*len/5;ctx.lineTo(x,y);}ctx.stroke();}ctx.strokeStyle=c2;ctx.lineWidth=4;ctx.beginPath();ctx.arc(T.x,T.y+30,(40+120*p),0,Math.PI*2);ctx.stroke();}
      else if(family==='time'||family==='mirror'){for(let i=0;i<5;i++){ctx.strokeStyle=i%2?c1:c2;ctx.lineWidth=2+i*.6;ctx.globalAlpha=fade;ctx.beginPath();ctx.arc(T.x,T.y,28+i*19+p*18,0,Math.PI*2);ctx.stroke();}if(family==='time'){line(T.x,T.y,T.x+Math.cos(p*8)*70,T.y+Math.sin(p*8)*70,5,c2,fade);}else{for(let i=0;i<8;i++){ctx.save();ctx.translate(T.x+Math.cos(i*.78)*(45+p*55),T.y+Math.sin(i*.78)*(45+p*55));ctx.rotate(i+p*2);ctx.fillStyle=i%2?c1:c2;ctx.fillRect(-4,-18,8,36);ctx.restore();}}}
      else if(family==='music'){ctx.font=`${sig?34:25}px sans-serif`;for(let i=0;i<10;i++){const q=(p+i*.1)%1;ctx.fillStyle=i%2?c1:c2;ctx.globalAlpha=fade;ctx.fillText(i%2?'♪':'♫',T.x+(i-5)*22,T.y+70-q*150-Math.sin(i)*20);}}
      else if(family==='heal'){for(let i=0;i<14;i++){const q=(p+i*.055)%1,x=A.x+(i-7)*12,y=A.y+70-q*130;ctx.strokeStyle=i%2?c1:c2;ctx.lineWidth=4;line(x-7,y,x+7,y,4,i%2?c1:c2,fade);line(x,y-7,x,y+7,4,i%2?c1:c2,fade);}}
      else if(family==='guard'){ctx.strokeStyle=c1;ctx.lineWidth=sig?9:6;ctx.globalAlpha=fade;for(let i=0;i<3;i++){const rad=40+i*18+p*12;ctx.beginPath();for(let k=0;k<=6;k++){const a=k*Math.PI/3-.5,x=A.x+Math.cos(a)*rad,y=A.y+Math.sin(a)*rad;k?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.stroke();}}
      else {line(A.x,A.y,T.x,T.y,sig?10:6,c1,fade);}
      ctx.restore();if(p<1)requestAnimationFrame(draw);else cv.remove();
    }requestAnimationFrame(draw);setTimeout(()=>cv.remove(),dur+180);
  }
  function statusCinematicV57(target,key){
    if(!fxOn()||!target)return;const c=rectCenter(target),el=document.createElement('div'),safe=String(key||'status').replace(/[^a-zA-Z0-9_-]/g,''),glyph=(STATUS_META_V55[key]||{}).glyph||'◆';el.className=`status-cinematic-v57 status-${safe}`;el.style.left=`${c.x}px`;el.style.top=`${c.y}px`;
    const mark=key==='sleep'?'Z':key==='freeze'?'❄':(key==='paralysis'||key==='shock')?'ϟ':key==='poison'?'●':key==='burn'?'▲':(key==='atkDown'||key==='defDown'||key==='weak'||key==='vulnerable')?'▼':(key==='atkUp'||key==='defUp')?'▲':glyph;
    el.innerHTML=`<b>${glyph}</b>${Array.from({length:10},(_,i)=>`<i style="--i:${i}">${mark}</i>`).join('')}<span></span>`;fxRoot().appendChild(el);target.classList.add(`status-pulse-${safe}-v57`);setTimeout(()=>{target.classList.remove(`status-pulse-${safe}-v57`);el.remove();},1550);
  }
  function moveResultNarrationsV57(payload,enemySide=false){
    const out=[],mv=payload?.move||{},targetName=enemySide?(payload?.targetMonsterName||'아군 몬스터'):(payload?.targetName||'상대'),actorName=enemySide?(payload?.monsterName||'상대'):(payload?.monsterName||'몬스터');
    if(payload?.hit===false){out.push({text:'그러나 기술이 빗나갔다!',tone:'miss',tag:'MISS',fx:'miss'});return out;}
    if(Number(payload?.damage||0)>0)out.push({text:`${targetName}에게 ${Number(payload.damage)} 데미지!`,tone:enemySide?'bad':'move',tag:'DAMAGE'});
    if(payload?.critical)out.push({text:'급소에 맞았다!',tone:'critical',tag:'CRITICAL'});
    const eff=Number(payload?.effectiveness??1);if(eff>1.01)out.push({text:'효과가 굉장했다!',tone:'good',tag:'EFFECTIVE'});else if(eff<.99&&Number(payload?.damage||0)>0)out.push({text:'효과가 별로인 듯하다…',tone:'bad',tag:'RESIST'});
    const statuses=payload?.appliedDebuffs||payload?.appliedStatuses||[];for(const s of statuses)out.push({text:statusNarrationV57(s.key,targetName,'apply'),tone:'status',tag:'STATUS',status:s});
    if(mv.kind==='heal'&&mv.heal)out.push({text:`${actorName}의 체력이 회복되었다!`,tone:'good',tag:'RECOVER'});if(mv.kind==='guard'&&mv.shield)out.push({text:`${actorName}은(는) 방어 태세를 갖췄다!`,tone:'good',tag:'GUARD'});return out;
  }
  ...
  function playRoomEvents(next,afterSeq){
    ...
    const later=(fn,step=0)=>{const at=combatDelay;setTimeout(()=>{try{fn();}catch(err){console.warn('[RIFT] V57 phase skipped',err);}},at);combatDelay+=Math.max(0,Number(step||0));};
    for(const ev of events){const p=ev.payload||{};
      if(ev.type==='battle-start')later(()=>{battleNarrateV57(p.tier==='boss'?`${p.name||'강력한 적'}이(가) 나타났다!`:'야생 몬스터가 나타났다!',{tone:p.tier==='boss'?'enemy':'normal',tag:p.tier==='boss'?'BOSS':'ENCOUNTER'});if(p.tier==='boss'){screenShake('heavy');screenFlash('boss');sound('boss');}},BATTLE_PACE_V57.announce);
        ...
        later(()=>battleNarrateV57(`${name}의 ${skill}!`,{tone:'move',tag:sig?'SIGNATURE MOVE':'MOVE',sub:p.element||p.move?.element||''}),sig?520:360);
        ...
        later(()=>battleNarrateV57(`${name}의 ${skill}!`,{tone:'enemy',tag:sig?'ENEMY SIGNATURE':'ENEMY MOVE',sub:p.element||p.move?.element||''}),sig?520:380);
      ...
      else if(ev.type==='monster-fuse')later(()=>{battleNarrateV57(`${p.toName||'몬스터'} — 5턴 융합!`,{tag:'TURN FUSION',tone:'victory'});playFusionCinematicV56(p);},1450);
      else if(ev.type==='monster-unfuse')later(()=>{battleNarrateV57(p.reason==='battle-end'?'전투 종료 — 융합이 해제되었다!':p.reason==='ko'?'융합이 붕괴되어 원래 모습으로 돌아왔다!':'5턴이 지나 융합이 해제되었다!',{tag:'FUSION END',tone:'good'});monsterTransformFx({instanceId:p.instanceId,mode:'fusion'});},520);
      ...
      else if(ev.type==='move-rewrite'&&p.playerId===state.profileId)later(()=>{battleNarrateV57(`${p.moveName||'기술'}이 변화했다!`,{tag:'RIFT REWRITE',tone:'good'});screenFlash('break');screenShake('medium');sound('overdrive');},BATTLE_PACE_V57.status);
      else if(ev.type==='status-tick'||ev.type==='monster-status'||ev.type==='monster-status-turn'||ev.type==='monster-status-cure'||ev.type==='monster-status-tick'||ev.type==='monster-stat'||ev.type==='enemy-status'||ev.type==='enemy-status-turn'||ev.type==='enemy-status-cure')later(()=>statusEventNarrationV57(ev),p.blocked?BATTLE_PACE_V57.blocked:BATTLE_PACE_V57.status);
      else if(ev.type==='enemy-break')later(()=>{const t=p.enemyUid?$(`[data-enemy="${CSS.escape(p.enemyUid)}"]`):null;battleNarrateV57(`${p.enemyName||'상대'}의 자세가 무너졌다!`,{tag:'BREAK',tone:'good'});if(t){t.classList.add('rift-broken-v26');statusCinematicV57(t,'broken');}screenFlash('break');screenShake('hit');sound('overdrive');},BATTLE_PACE_V57.status);
      else if(ev.type==='boss-shield-break')later(()=>{battleNarrateV57(`${p.enemyName||'보스'}의 보호막이 깨졌다!`,{tag:'BOSS SHIELD',tone:'critical'});const t=p.enemyUid?$(`[data-enemy="${CSS.escape(p.enemyUid)}"]`):null;if(t)signatureImpactFx(t,{element:'수정',rarity:'legendary',power:22,effects:[{op:'damage',value:22}]},1.55);screenFlash('break');screenShake('heavy');},BATTLE_PACE_V57.status);
      else if(ev.type==='held-item')later(()=>battleNarrateV57(ev.message||'장착 아이템이 발동했다!',{tag:'HELD ITEM',tone:'good'}),BATTLE_PACE_V57.minor);
  ...
  function sceneTransitionLabel(next){
    const map={route:'PATH SELECT',battle:next?.battle?.tier==='boss'?'BOSS ENCOUNTER':next?.battle?.tier==='elite'?'ELITE ENCOUNTER':next?.battle?.tier==='rival'?'RIFT HUNTER':'ENCOUNTER',reward:'CLEAR',event:'UNKNOWN SIGNAL',cleared:'ABYSS CLEAR',ended:'EXPEDITION END'};
  ...
  function acceptRoomUpdate(next,{initial=false}={}){
    ...
    if(leavingBattle){
      ...
      const delay=fxOn()?Math.min(45000,Math.max(2200,Number(choreography||0)+650)):160;
      const finish=()=>{if(!state.room||Number(state.room.seq||0)<Number(next.seq||0))return;state.battleExitPending=false;try{renderRoom();playSceneCurtain(state.room);}catch(err){console.error('[RIFT] post-battle render recovery',err);renderRoom();}};
      ...
      state.battleExitFallbackTimer=setTimeout(()=>{if(state.current==='battle'&&state.room?.status!=='battle')finish();},48000);
      return;
    }
    ...
    if(sameBattle&&fxOn()){
      ...
      let choreography=0;try{choreography=playRoomEvents(next,after)||0;}catch(err){console.warn('[RIFT] combat choreography skipped',err);}
      if(choreography>0){
  ...
  function clearTimers(){clearInterval(state.bannerTimer);clearInterval(state.roomsTimer);clearInterval(state.battleMessageTimer);clearTimeout(state.rewardAutoTimer);clearTimeout(state.combatRenderTimer);state.bannerTimer=0;state.roomsTimer=0;state.battleMessageTimer=0;state.rewardAutoTimer=0;state.combatRenderTimer=0;if(state.current!=='battle')state.battleTimelineUntil=0;document.body.classList.remove('combat-resolving-v54');}
  ...
  function effectUi(op,value=0){
    const m={
      ...
      vulnerable:['◎','방어↓','debuff'],vulnerableAll:['◎','전체 방어↓','debuff'],weak:['▽','공격↓','debuff'],burn:['🔥','화상','burn'],poison:['☠','독','poison'],poisonAll:['☠','전체 독','poison'],shockAll:['ϟ','감전','debuff'],intentSealAll:['⊘','행동 봉인','debuff'],
    ...
    if(Number(value)>0){
      ...
      else if(['vulnerable','vulnerableAll','weak','burn','poison','poisonAll','shockAll','intentSealAll','debuffImmune','cleanseWeak'].includes(op))val=`${value}`;
  ...
  function enemyAttackStyle(enemy){
    const a=enemy?.archetype||enemyArchetypeName(enemy?.name||'');if(['beast','assassin'].includes(a))return'leap';if(['wing','spirit','wraith'].includes(a))return'cast';if(['golem','tyrant'].includes(a))return'slam';if(['insect','mimic','slime'].includes(a))return'snap';if(['priest','watcher','construct'].includes(a))return'beam';return'thrust';
  }
  ...
  function nextRerollCost(r){const run=r?.runState?.[state.profileId]||{},count=Number(r?.reward?.rerolls?.[state.profileId]||0),discount=Math.min(.70,Math.max(0,runMod(run,'rerollDiscount'))),base=42+Number(r?.floor||0)*3;return Math.max(20,Math.round(base*Math.pow(1.55,count)*(1-discount)));}
  ...
  function battleCaptureChanceUi(enemy,seal,run,battle){
    ...
    let chance=Number(enemy.captureBase||.25)*mult*hpFactor+statusBonus+relic+runMod(run,'captureBonus');const cap=enemy.tier==='boss'?.40:enemy.tier==='ultra'?.78:enemy.tier==='rare'?.93:.985;
    if(battle?.encounterType==='boss'){if(Number(enemy?.hpSegments||1)>1)return{chance:0,hpRatio,reason:'HP 보호막을 마지막 1칸까지 파괴'};if(seal!=='royal')return{chance:0,hpRatio,reason:'로열 봉인구 필요'};if(hpRatio>.25)return{chance:0,hpRatio,reason:'HP 25% 이하 필요'};chance=Math.min(chance,.40);}
    if(!['wild','boss'].includes(battle?.encounterType||''))return{chance:0,hpRatio,reason:'봉인 불가 전투'};
    return{chance:Math.max(.01,Math.min(cap,chance)),hpRatio,reason:''};
  }
  function battleCaptureMenuV45(enemy,run,battle,me,selectedMon){
    const hp=Math.round((Number(enemy?.hp||0)/Math.max(1,Number(enemy?.maxHp||1)))*100),disabled=selectedMon?.acted||me?.ended;
    return `<div class="capture-command-v45"><div class="subcommand-head-v41"><button data-action="battle-menu" data-menu="root">←</button><b>몬스터 봉인</b><span>${enemy?`${esc(enemy.name)} · HP ${hp}%`:'대상을 선택하세요'}</span></div><div class="capture-guide-v45"><b>${hp<=25?'봉인 최적 구간':hp<=50?'봉인 확률 상승 중':'HP를 더 낮추면 성공률이 크게 올라갑니다.'}</b><small>BREAK · 화상 · 약화 상태도 성공률을 조금 높여 줍니다.</small></div><div class="capture-orbs-v45">${[['basic','◇','기본'],['silver','◈','실버'],['royal','◆','로열']].map(([seal,glyph,label])=>{const c=battleCaptureChanceUi(enemy,seal,run,battle),count=Number(state.profile?.seals?.[seal]||0),can=!!enemy&&!disabled&&count>0&&c.chance>0;return `<button data-action="battle-capture" data-seal="${seal}" ${can?'':'disabled'}><span>${glyph}</span><div><b>${label} 봉인구</b><small>${c.reason||`성공률 ${Math.round(c.chance*100)}%`}</small></div><strong>${count}</strong><i style="--capture:${Math.round(c.chance*100)}%"></i></button>`;}).join('')}</div></div>`;
  }
  function skillHash(text=''){let n=2166136261;for(let i=0;i<text.length;i++){n^=text.charCodeAt(i);n=Math.imul(n,16777619);}return n>>>0;}
  function namedSkillGlyphV47(family){
    return ({slash:'✧',claw:'≋',bite:'◇',rush:'➤',beam:'━',bullet:'•',thunder:'ϟ',flame:'♨',ice:'✦',wave:'≈',cyclone:'↻',root:'⌁',spore:'✺',toxin:'☗',shadow:'◐',meteor:'✶',quake:'⬡',time:'◷',music:'♫',mirror:'◇',heal:'✚',guard:'⬢'})[family]||'◆';
  }
  function namedActorFramesV47(family,dir=1,variant=0){
    const x=n=>`${Math.round(n*dir)}px`;
    if(family==='rush')return[{transform:'translate3d(0,0,0) scale(1)'},{transform:`translate3d(${x(-18)},6px,0) scale(.96)`,offset:.25},{transform:`translate3d(${x(132)},-4px,0) scale(1.12)`,offset:.58},{transform:'translate3d(0,0,0) scale(1)'}];
    if(['slash','claw','bite'].includes(family))return[{transform:'translate3d(0,0,0) rotate(0)'},{transform:`translate3d(${x(-10)},-5px,0) rotate(${dir*-5}deg)`,offset:.28},{transform:`translate3d(${x(82+variant*8)},2px,0) rotate(${dir*(family==='claw'?8:4)}deg) scale(1.06)`,offset:.55},{transform:'translate3d(0,0,0) rotate(0)'}];
    if(['beam','bullet','thunder'].includes(family))return[{transform:'translate3d(0,0,0) scale(1)'},{transform:`translate3d(${x(-8)},0,0) scale(1.08)`,offset:.42},{transform:`translate3d(${x(5)},0,0) scale(.98)`,offset:.64},{transform:'translate3d(0,0,0) scale(1)'}];
    if(['wave','cyclone','music'].includes(family))return[{transform:'translate3d(0,0,0) rotate(0)'},{transform:`translate3d(0,-10px,0) rotate(${dir*(variant%2?4:-4)}deg)`,offset:.42},{transform:`translate3d(${x(8)},-4px,0) rotate(${dir*(variant%2?-3:3)}deg)`,offset:.7},{transform:'translate3d(0,0,0) rotate(0)'}];
    if(['meteor','quake','root','flame','ice'].includes(family))return[{transform:'translate3d(0,0,0) scale(1)'},{transform:'translate3d(0,-14px,0) scale(1.08)',offset:.4},{transform:`translate3d(${x(5)},4px,0) scale(1.03)`,offset:.66},{transform:'translate3d(0,0,0) scale(1)'}];
    if(['shadow','time','mirror'].includes(family))return[{transform:'translate3d(0,0,0) scale(1)',opacity:1},{transform:`translate3d(${x(-22)},-4px,0) scale(.92)`,opacity:.4,offset:.3},{transform:`translate3d(${x(38+variant*5)},-2px,0) scale(1.05)`,opacity:1,offset:.62},{transform:'translate3d(0,0,0) scale(1)',opacity:1}];
    return[{transform:'translate3d(0,0,0) scale(1)'},{transform:'translate3d(0,-8px,0) scale(1.07)',offset:.45},{transform:'translate3d(0,0,0) scale(1)'}];
  }
  function moveDetailBadgesV47(move){
    if(!move)return'';const out=[];
    if(['attack','burst'].includes(move.kind))out.push(`<span>POW <b>${Math.max(1,Math.round(Number(move.power||0)+Number(move.ratio||0)*10))}</b></span>`);
    if(move.stagger)out.push(`<span>BREAK <b>+${move.stagger}</b></span>`);
    if(move.heal)out.push(`<span>HEAL <b>${move.heal}</b></span>`);
    if(move.shield)out.push(`<span>GUARD <b>${move.shield}</b></span>`);
    out.push(`<span>ACC <b>${move.accuracy||100}%</b></span>`);
    if(Number(move.cooldown||0)>0)out.push(`<span>CD <b>${move.cooldown}</b></span>`);
    return out.slice(0,4).join('');
  }
  function namedMoveFamily(payload){
    const explicit=payload?.fxFamily||payload?.move?.fxFamily;if(explicit)return String(explicit);
    const name=String(payload?.skill||payload?.move?.name||''),e=String(payload?.element||payload?.move?.element||''),low=name.toLowerCase();
    if(/치유|회복|축복|재생|heal|cure|bless/.test(low))return'heal';
    if(/방벽|수호|결계|장갑|갑주|shield|guard|barrier|armor/.test(low))return'guard';
    if(/송곳니|깨물|물기|fang|bite|jaw/.test(low))return'bite';
    if(/발톱|할퀴|claw|scratch|rend/.test(low))return'claw';
    if(/참격|베기|절단|검무|slash|blade|cut/.test(low))return'slash';
    if(/돌진|쇄도|추격|강습|도약|rush|charge|dash|leap/.test(low))return'rush';
    if(/광선|레이저|포톤|심판|beam|laser|ray/.test(low))return'beam';
    if(/탄환|수탄|포탄|투사|연사|shot|bullet|cannon|missile/.test(low))return'bullet';
    if(/낙뢰|전격|번개|천둥|방전|bolt|thunder|lightning/.test(low))return'thunder';
    if(/화염|불꽃|용광|홍련|폭염|작열|flame|fire|ember|blaze/.test(low))return'flame';
    if(/빙정|서리|얼음|영도|설화|ice|frost|crystal|snow/.test(low))return'ice';
    if(/해일|격류|파도|물결|수압|wave|tide|surge/.test(low))return'wave';
    if(/돌풍|회오리|폭풍|바람|풍압|gale|wind|storm|cyclone/.test(low))return'cyclone';
    if(/덩굴|뿌리|수목|가시|씨앗|root|vine|thorn|seed/.test(low))return'root';
    if(/균사|포자|spore|pollen/.test(low))return'spore';
    if(/독|맹독|toxin|poison|venom/.test(low))return'toxin';
    if(/그림자|심연|악령|흡혼|월식|shadow|abyss|soul|void/.test(low))return'shadow';
    if(/성좌|별|유성|천체|혜성|초신성|star|meteor|nova|comet/.test(low))return'meteor';
    if(/지진|대지|분쇄|강타|충돌|quake|smash|crush/.test(low))return'quake';
    if(/시간|영겁|시계|초침|윤회|time|chrono|clock/.test(low))return'time';
    if(/음률|노래|선율|공명음|music|song|melody|chime/.test(low))return'music';
    if(/거울|반사|프리즘|mirror|reflect|prism/.test(low))return'mirror';
    if(e==='화염')return'flame';if(e==='물')return'wave';if(e==='바람')return'cyclone';if(e==='번개')return'thunder';if(e==='자연')return'root';if(e==='수정')return'ice';if(e==='그림자'||e==='공허')return'shadow';if(e==='별')return'meteor';if(e==='시간')return'time';if(e==='빛')return'beam';if(e==='강철')return'quake';return'slash';
  }
  function namedSkillOverlayV45(target,family,payload){
    if(!fxOn()||!target)return;const c=rectCenter(target),root=fxRoot(),el=document.createElement('div'),variant=Number(payload?.fxVariant??payload?.move?.fxVariant??(skillHash(String(payload?.skill||''))%8)),ec=elementClass(payload?.element||'공허');el.className=`named-skill-fx-v45 named-skill-fx-v47 family-${family} variant-${variant} sig-${ec}-v31 ${payload?.signature||payload?.move?.signature?'signature-v54':''}`;el.style.left=`${c.x}px`;el.style.top=`${c.y}px`;el.innerHTML=`<i></i><i></i><i></i><b></b><b></b><b></b><em></em><strong class="skill-glyph-v47">${namedSkillGlyphV47(family)}</strong>`;root.appendChild(el);setTimeout(()=>el.remove(),payload?.signature||payload?.move?.signature?1180:900);
  }
  ...
  const STATUS_META_V55={
    burn:{label:'화상',glyph:'♨'},poison:{label:'중독',glyph:'☠'},shock:{label:'감전',glyph:'ϟ'},paralysis:{label:'마비',glyph:'ϟ'},sleep:{label:'수면',glyph:'Z'},freeze:{label:'빙결',glyph:'❄'},atkDown:{label:'공격↓',glyph:'▼'},defDown:{label:'방어↓',glyph:'▼'},atkUp:{label:'공격↑',glyph:'▲'},defUp:{label:'방어↑',glyph:'▲'},weak:{label:'공격↓',glyph:'▽'},vulnerable:{label:'방어↓',glyph:'◎'},intentSeal:{label:'행동 봉쇄',glyph:'⊘'},broken:{label:'BREAK',glyph:'✹'},stagger:{label:'BREAK 게이지',glyph:'✦'},guard:{label:'방어',glyph:'⬢'},heal:{label:'회복',glyph:'✚'},buff:{label:'능력 상승',glyph:'▲'}
  };
  function statusEffectFxV55(target,key,label='',value=0){
    if(!state.fx||!target)return;const meta=STATUS_META_V55[key]||{label:label||String(key||'STATUS'),glyph:'◆'},c=rectCenter(target),el=document.createElement('div');
    el.className=`status-effect-fx-v55 status-${String(key||'status').replace(/[^a-zA-Z0-9_-]/g,'')}`;el.style.left=`${c.x}px`;el.style.top=`${c.y}px`;
    el.innerHTML=`<strong>${meta.glyph}</strong><b>${esc(label||meta.label)}${Number(value)>0?` ${value}`:''}</b><span></span>${Array.from({length:12},(_,i)=>`<i style="--i:${i}"></i>`).join('')}`;fxRoot().appendChild(el);setTimeout(()=>el.remove(),1480);
  }
  function playAppliedStatusFxV55(target,list=[]){
    if(!state.fx||!target||!Array.isArray(list))return;list.slice(0,5).forEach((x,i)=>setTimeout(()=>statusEffectFxV55(target,x.key,x.label,x.value),170+i*250));
  }
  function tacticDisplayNameV55(card){
    const raw=String(card?.name||'공명 지령').replace(/^인자술\s*[·:]?\s*/,'').replace(/^링크\s*전술\s*[·:]?\s*/,'');return `공명 지령 · ${raw}`;
  }
  function tacticEffectTextV55(card,monster){
    const rows=cardEffectRows(card,4).map(x=>`${x.icon} ${x.label}${x.value?` ${x.value}`:''}`);const affinity=card?.element&&monster?.element&&card.element===monster.element;
    rows.push(affinity?'✦ 다음 기술 +20% · BREAK +6':'✦ 다음 기술 +10% · BREAK +3');return rows.join(' · ');
  }
  function tacticCommandFxV55(payload){
    if(!state.fx||!payload)return;const card=byId(state.meta.cards,payload.cardId)||{id:payload.cardId||payload.cardName,name:payload.cardName||'공명 지령',element:payload.element||'별',effects:payload.effects||[],rarity:'rare'};
    const actor=$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`)||$('.player-field-v51'),target=payload.targetUid?$(`[data-enemy="${CSS.escape(payload.targetUid)}"]`):null;if(actor){spellCastSealFx(actor,card,'cast');namedSkillOverlayV45(actor,'mirror',{element:card.element,fxVariant:cardFxSignature(card).variant});}
    const banner=document.createElement('div');banner.className=`tactic-cast-v55 element-${elementClass(card.element)}`;banner.innerHTML=`<small>RIFT COMMAND</small><b>${esc(tacticDisplayNameV55(card))}</b><span>${esc(tacticEffectTextV55(card,{element:payload.monsterElement||''}))}</span><strong>${payload.affinity?'공명 일치 · +20%':'지령 연결 · +10%'}</strong>`;fxRoot().appendChild(banner);setTimeout(()=>banner.remove(),1500);
    const ops=payload.effects||[];ops.forEach((e,i)=>{const map={burn:'burn',poison:'poison',poisonAll:'poison',shockAll:'shock',weak:'weak',weakAll:'weak',vulnerable:'vulnerable',vulnerableAll:'vulnerable',intentSealAll:'intentSeal',block:'guard',blockAllies:'guard',heal:'heal',healAllies:'heal'};const key=map[e.op];if(!key)return;setTimeout(()=>statusEffectFxV55(['guard','heal'].includes(key)?actor:(target||actor),key,'',e.value),320+i*180);});sound('chain');
  }
  function statusTickFxV55(payload){const target=payload?.enemyUid?$(`[data-enemy="${CSS.escape(payload.enemyUid)}"]`):null;if(!target)return;statusEffectFxV55(target,payload.status,payload.label,payload.remaining);if(Number(payload.damage)>0)popNumber(target,`-${payload.damage}`,'damage');}
  function effectChipTextV54(x){if(!x)return'';return x.value&&x.key!=='broken'?`${x.label} +${x.value}`:x.label||'';}
  function battleMoveFeedbackV51(payload,enemySide=false){
    if(!payload)return;const target=(enemySide?(payload.targetMonsterId?$(`[data-monster="${CSS.escape(payload.targetMonsterId)}"]`):$('.player-field-v51')):$(`[data-enemy="${CSS.escape(payload.targetUid||'')}"]`))||(enemySide?$('.active-mon-v51.selected,.active-mon-v51'):$('.enemy-mon-v51.selected,.enemy-mon-v51'))||$('.battle-field-v52'),actor=(enemySide?$(`[data-enemy="${CSS.escape(payload.enemyUid||'')}"]`):$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`))||(enemySide?$('.enemy-mon-v51.selected,.enemy-mon-v51'):$('.active-mon-v51.selected,.active-mon-v51'));if(!target)return;
    const c=rectCenter(target||$('.battle-field-v52')||document.body),el=document.createElement('div'),d=Number(payload.damage||0),miss=payload.hit===false,mv=payload.move||{},element=mv.element||payload.element||'공허',debuffs=(payload.appliedDebuffs||payload.appliedStatuses||[]).map(effectChipTextV54).filter(Boolean),summary=(payload.effectSummary||[]).filter(Boolean),link=null,crit=!!payload.critical,eff=Number(payload.effectiveness??1);
    el.className=`move-feedback-v51 move-feedback-v54 ${enemySide?'enemy':''} ${miss?'miss':''} ${payload.signature||mv.signature?'signature':''}`;el.style.left=`${c.x}px`;el.style.top=`${Math.max(82,c.y-76)}px`;
    el.innerHTML=`<span class="move-feedback-type-v54 element-${elementClass(element)}">${elementGlyph(element)} ${esc(element)}${payload.signature||mv.signature?' · 전용기':''}</span><b>${esc(payload.skill||mv.name||'SKILL')}</b><small>${miss?'MISS':d>0?`${d} DAMAGE`:mv.heal?`HEAL ${mv.heal}`:mv.shield?`GUARD ${mv.shield}`:'ACTION'}</small>${debuffs.length?`<em>${debuffs.map(x=>esc(x)).join(' · ')}</em>`:summary.length?`<em>${summary.slice(0,3).map(x=>esc(x)).join(' · ')}</em>`:''}${crit?'<strong>급소에 맞았다!</strong>':eff>1?'<strong>효과가 굉장했다!</strong>':eff<1?'<strong>효과가 약했다…</strong>':''}`;
    fxRoot().appendChild(el);target.classList.remove('hit-pulse-v51');void target.offsetWidth;target.classList.add('hit-pulse-v51');setTimeout(()=>target.classList.remove('hit-pulse-v51'),620);setTimeout(()=>el.remove(),payload.signature||mv.signature?1700:1450);if(actor){actor.classList.add('move-focus-v51');setTimeout(()=>actor.classList.remove('move-focus-v51'),900);}
  }

  async function playNamedMonsterMoveV45(payload,enemySide=false){
    if(!payload)return;battleMoveFeedbackV51(payload,enemySide);const actor=(enemySide?$(`[data-enemy="${CSS.escape(payload.enemyUid||'')}"]`):$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`))||(enemySide?$('.enemy-mon-v51.selected,.enemy-mon-v51'):$('.active-mon-v51.selected,.active-mon-v51')),target=(enemySide?(payload.targetMonsterId?$(`[data-monster="${CSS.escape(payload.targetMonsterId)}"]`):$('.player-field-v44')):$(`[data-enemy="${CSS.escape(payload.targetUid||'')}"]`))||(enemySide?$('.active-mon-v51.selected,.active-mon-v51'):$('.enemy-mon-v51.selected,.enemy-mon-v51'));if(!target)return;
    ...
    const lowFx=Number(navigator.deviceMemory||8)<=4||Number(navigator.hardwareConcurrency||8)<=4;moveSpectacleV56(actor||target,target,family,payload,enemySide);if((signature||heavy)&&!lowFx&&actor)canvasMoveFxV57(actor,target,family,payload,enemySide);if(signature&&actor)monsterSignatureFxV532(actor,payload,'cast');
    if(actor){attackLabelV40(actor,payload.skill||'SKILL',enemySide?'enemy':'ally');attackAfterimageV40(actor,dir);actor.classList.add('attacking-v40');}
    const actorDur=signature?720:tempo==='charge'?620:tempo==='multi'?560:480;if(actor)try{actor.animate(namedActorFramesV47(family,dir,variant),{duration:actorDur,easing:'cubic-bezier(.14,.82,.16,1)'});}catch{}
    const windup=signature?220:tempo==='charge'?190:family==='beam'?170:130;
    setTimeout(async()=>{
      if(signature)monsterSignatureFxV532(['heal','guard'].includes(family)?(actor||target):target,payload,'impact');
      if(family==='heal'){healBloomFx(actor||target,card);namedSkillOverlayV45(actor||target,family,payload);}
      else if(family==='guard'){shieldGainFx(actor||target,Number(mv.shield||8));namedSkillOverlayV45(actor||target,family,payload);}
      else if(family==='beam'||family==='mirror'||family==='time'){await beamFx(actor,target,card);namedSkillOverlayV45(target,family,payload);}
      else if(family==='wave'||family==='cyclone'||family==='music'||family==='spore'){await waveFx(actor,target,card);namedSkillOverlayV45(target,family,payload);}
      else if(['flame','meteor','root','quake','ice'].includes(family)){await eruptionFx(target,card);namedSkillOverlayV45(target,family,payload);}
      else if(family==='thunder'){await beamFx(actor,target,card);namedSkillOverlayV45(target,family,payload);setTimeout(()=>namedSkillOverlayV45(target,'thunder',{...payload,fxVariant:(variant+3)%8}),90);}
      else if(family==='bullet'){for(let i=0;i<Math.min(4,hits+1);i++)setTimeout(()=>signatureProjectileFx(actor,target,card).then(()=>signatureImpactFx(target,card,.72)),i*95);namedSkillOverlayV45(target,family,payload);}
      else if(family==='toxin'||family==='shadow'){await signatureProjectileFx(actor,target,card);namedSkillOverlayV45(target,family,payload);signatureImpactFx(target,card,heavy?1.28:.92);}
      else if(['slash','claw','bite','rush'].includes(family)){const n=family==='claw'?Math.max(3,hits):family==='bite'?Math.max(2,hits):Math.max(1,hits);for(let i=0;i<Math.min(4,n);i++)setTimeout(()=>slashAt(target,payload.element||'강철',heavy&&i===0),i*92);namedSkillOverlayV45(target,family,payload);}
      else{await signatureProjectileFx(actor,target,card);namedSkillOverlayV45(target,family,payload);signatureImpactFx(target,card,heavy?1.35:1);}
      if(!['heal','guard'].includes(family)&&Number(payload.damage||0)>0)popNumber(target,`-${payload.damage}`,'damage'+(enemySide?' player':''));
      if(Array.isArray(payload.appliedDebuffs)&&payload.appliedDebuffs.length)setTimeout(()=>playAppliedStatusFxV55(target,payload.appliedDebuffs),170);
      if(heavy)screenShake(signature?'heavy':'hit');soundElement(payload.element||'공허',heavy);
    },windup);if(actor)setTimeout(()=>actor.classList.remove('attacking-v40'),actorDur+120);
  }

  function moveSpectacleV56(actor,target,family,payload,enemySide=false){
    if(!fxOn()||!actor||!target)return;const a=rectCenter(actor),t=rectCenter(target),root=fxRoot(),el=document.createElement('div'),variant=Number(payload?.fxVariant||0)%8;el.className=`move-spectacle-v56 family-${family} variant-${variant} ${payload?.signature?'signature':''}`;el.style.setProperty('--sx',`${a.x}px`);el.style.setProperty('--sy',`${a.y}px`);el.style.setProperty('--tx',`${t.x}px`);el.style.setProperty('--ty',`${t.y}px`);el.style.setProperty('--dx',`${t.x-a.x}px`);el.style.setProperty('--dy',`${t.y-a.y}px`);
    const glyph={slash:'╱',claw:'≋',bite:'◖ ◗',rush:'➤',beam:'━',bullet:'•••',thunder:'ϟ',flame:'♨',ice:'❄',wave:'≈',cyclone:'◎',root:'♜',spore:'✺',toxin:'☠',shadow:'◉',meteor:'✦',quake:'⌁',time:'◷',music:'♫',mirror:'◇',heal:'✚',guard:'⬢'}[family]||'✦';
    el.innerHTML=`<span class="fx-glyph-v56">${glyph}</span>${Array.from({length:family==='bullet'?7:family==='meteor'?6:family==='spore'?8:5},(_,i)=>`<i style="--i:${i}"></i>`).join('')}<b></b><em></em>`;root.appendChild(el);setTimeout(()=>el.classList.add('go'),10);setTimeout(()=>el.remove(),payload?.signature?1900:1450);
  }
  async function playFusionCinematicV56(payload){
    if(!fxOn()){monsterTransformFx({...payload,mode:'fusion'});return;}const root=document.createElement('div');root.className='fusion-cinematic-v56';root.innerHTML=`<div class="fusion-space-v56"><div class="fusion-core-v56">∞</div><div class="fusion-mon-v56 a"><img src="${esc(payload?.fromSpriteA||'')}"><b>${esc(payload?.fromA||'')}</b></div><div class="fusion-mon-v56 b"><img src="${esc(payload?.fromSpriteB||'')}"><b>${esc(payload?.fromB||'')}</b></div><div class="fusion-result-v56"><img src="${esc(payload?.toSprite||payload?.fromSpriteA||'')}"><b>${esc(payload?.toName||'FUSION')}</b><small>${(payload?.elements||[]).map(x=>esc(x)).join(' / ')}</small></div><div class="fusion-lines-v56">${Array.from({length:16},(_,i)=>`<i style="--i:${i}"></i>`).join('')}</div></div>`;document.body.appendChild(root);sound('overdrive');requestAnimationFrame(()=>root.classList.add('charge'));await wait(700);root.classList.add('merge');screenShake('medium');await wait(850);root.classList.add('reveal');screenFlash('victory');sound('win');await wait(650);root.remove();
  }
  function playerStatusEventFxV56(ev){
    const p=ev?.payload||{},target=p.instanceId?$(`[data-monster="${CSS.escape(p.instanceId)}"]`):null;if(!target)return;const key=p.status||(p.stat?`${p.stat}${Number(p.delta)<0?'Down':'Up'}`:'buff'),label=p.label||(p.stat?`${p.label||p.stat}${Number(p.delta)<0?'↓':'↑'}`:'STATUS');statusEffectFxV55(target,key,label,Number(p.delta||p.damage||0));if(Number(p.damage)>0)popNumber(target,`-${p.damage}`,'damage player');
  }

  async function startEncounterV53(button,nodeId){
    if(state.encounterStarting)return;
    state.encounterStarting=true;
    const oldText=button?.textContent||'전투 시작';
    if(button){button.disabled=true;button.classList.add('loading');button.textContent='전투 준비 중…';}
    try{
      let room=state.room;
      if(room?.status==='battle'){state.battleMenu='root';return renderRoom();}
      if(room?.status!=='route'){
        const fresh=await api(`/api/room/${state.roomId}`);acceptRoomUpdate(fresh.room,{initial:true});return;
      }
      const node=(room.route||[]).find(n=>n.id===nodeId)||(room.route||[])[0];
      if(!node)throw new Error('전투 경로를 찾을 수 없습니다.');
      const d=await roomPost('vote',{nodeId:node.id});
      if(d.room?.status==='battle'){state.battleMenu='root';return;}
      if(d.room?.status==='route'){
        await wait(160);
        const fresh=await api(`/api/room/${state.roomId}`);acceptRoomUpdate(fresh.room,{initial:true});
      }
    }finally{
      state.encounterStarting=false;
      if(button&&document.body.contains(button)){button.disabled=false;button.classList.remove('loading');button.textContent=oldText;}
    }
  }

  function moveMasteryUi(move){const stage=Math.max(0,Math.min(2,Number(move?.masteryStage||0))),uses=Number(move?.masteryUses||0),th=state.meta?.battleRules?.moveMasteryThresholds||[0,5,13],next=stage>=2?th[2]:th[stage+1],base=stage?th[stage]:0,p=stage>=2?100:Math.max(0,Math.min(100,Math.round((uses-base)/Math.max(1,next-base)*100)));return{stage,uses,next,p,label:stage>=2?'++':stage===1?'+':''};}
  function spellCommandTileV44(card,index,can,cost){const sig=cardFxSignature(card),rows=cardEffectRows(card,2);return `<button class="spell-command-tile-v44 mode-${cardFxMode(card)} trait-${cardFxTrait(card)} variant-${sig.variant}" data-action="play-card" data-index="${index}" data-card-id="${esc(card.id)}" ${can?'':'disabled'} title="${esc(card.name)} · ${esc(card.text||'')}"><span class="spell-command-sigil-v44 element-${elementClass(card.element)}"><i></i><b>${spellSigilIcon(card)}</b></span><span class="spell-command-copy-v44"><b>${esc(card.name)}</b><small>${esc(spellSigilLabel(card))} · ${esc(card.element)}</small><em>${rows.map(x=>`${esc(x.icon)} ${esc(x.value||x.label)}`).join(' · ')}</em></span><strong>${cost}E</strong></button>`;}
  function routeGraphic(kind){return `<span class="route-glyph-v31 kind-${esc(kind)}"><i></i><i></i><i></i><i></i><i></i></span>`;}
  function waveTracker(r,label='WAVE'){
    const wave=((Number(r?.floor||1)-1)%10)+1;
    return `<div class="wave-track-v42"><small>${esc(label)}</small><div>${Array.from({length:10},(_,i)=>{const n=i+1,cls=n===wave?'current':n<wave?'cleared':n===5?'warden':n===10?'boss':'';return `<i class="${cls}" title="${n} 웨이브"><b>${n===5?'W':n===10?'B':''}</b></i>`;}).join('')}</div><strong>${String(wave).padStart(2,'0')} / 10</strong></div>`;
  }
  ...
  function queueAutoEncounter(r,node){
    ...
    if(!claim||!node||node.biomeChoice||(r.route||[]).length!==1||node.votes?.includes(state.profileId))return;
    const key=`${r.id}:${r.floor}:${node.id}`;
    if(state.autoEncounterKey===key)return;
    state.autoEncounterKey=key;
    setTimeout(async()=>{
      const now=state.room,current=now?.route?.find(n=>n.id===node.id);
      if(!now||now.id!==r.id||now.floor!==r.floor||now.status!=='route'||!current||current.votes?.includes(state.profileId))return;
      if(state.busy){state.autoEncounterKey='';return queueAutoEncounter(now,current);}
      try{await roomPost('vote',{nodeId:current.id});}catch{state.autoEncounterKey='';}
    },1100);
  }
  ...
  function moveEffectivenessV58(element,enemy){if(!element||!enemy?.element)return{mult:1,label:'보통'};if(elementStrongAgainst(element).includes(enemy.element))return{mult:1.35,label:'효과 굉장'};if(elementStrongAgainst(enemy.element).includes(element))return{mult:.75,label:'효과 약함'};return{mult:1,label:'보통'};}
  ...
  function fusionLineageV58(u){return [...new Set((u?.fusionLineage?.length?u.fusionLineage:[u?.speciesId,u?.fusionWith]).filter(Boolean))].slice(0,4);}
  function fusionSpriteUrlV58(...units){const ids=[...new Set(units.flatMap(u=>fusionLineageV58(u)))].filter(Boolean).slice(0,4);return ids.length>=2?`/api/fusion-sprite?ids=${encodeURIComponent(ids.join(','))}&v=590`:'';}
  function monsterDisplaySprite(u){if(!u)return'';if(u.abyssBloom)return u.riftSprite||u.sprite;if(Number(u.resonanceTurns||0)>0)return u.resonanceSprite||u.sprite;if(u.fused)return fusionSpriteUrlV58(u)||u.fusionSprite||u.sprite;if(u.evolved)return u.evolutionSprite||u.sprite;return u.sprite||u.baseSprite||'';}
  ...
  function movePowerLine(move){
    if(!move)return'';const bits=[],stage=Math.max(0,Math.min(2,Number(move.masteryStage||0))),scale=stage===1?1.10:stage>=2?1.22:1,sv=v=>Math.max(1,Math.round(Number(v||0)*scale));
    if(['attack','burst'].includes(move.kind))bits.push(`위력 ${Math.round((Number(move.power||0)+Number(move.ratio||0)*10)*scale)}`);
    if(move.shield)bits.push(`방어 ${sv(move.shield)}`);if(move.heal)bits.push(`회복 ${sv(move.heal)}`);if(move.boost)bits.push(`ATK +${sv(move.boost)}`);if(move.draw)bits.push(`드로우 ${move.draw}`);if(move.stagger)bits.push(`BREAK +${sv(move.stagger)}`);
    if(move.rewrite)bits.push(`${move.rewrite.icon||'◆'} ${move.rewrite.name}`);
    return bits.slice(0,2).join(' · ')||moveKindKo(move);
  }
  function moveEffectTagsV54(move){
    if(!move)return[];const out=[];if(move.signature)out.push({k:'signature',t:'전용기'});if(move.burn)out.push({k:'burn',t:`화상 ${move.burn}`});if(move.poison)out.push({k:'poison',t:`독 ${move.poison}`});if(move.shock)out.push({k:'shock',t:`마비 가능`});if(move.sleep)out.push({k:'sleep',t:`수면`});if(move.freeze)out.push({k:'freeze',t:`빙결`});if(move.weak)out.push({k:'weak',t:`공격↓ ${move.weak}`});if(move.vulnerable)out.push({k:'vulnerable',t:`방어↓ ${move.vulnerable}`});if(move.intentSeal)out.push({k:'seal',t:`봉쇄 ${move.intentSeal}`});if(move.stagger)out.push({k:'break',t:`BREAK +${move.stagger}`});if(move.lifesteal)out.push({k:'drain',t:'흡혈'});if(move.splash)out.push({k:'splash',t:'범위'});if(move.shield)out.push({k:'guard',t:`방어 +${move.shield}`});if(move.heal)out.push({k:'heal',t:`회복 +${move.heal}`});if(move.resonance)out.push({k:'res',t:`RES +${move.resonance}`});return out.slice(0,6);
  }
  function enemyDebuffBadgesV54(e){
    if(!e)return'';const d=e.debuffs||{},chips=[];if(e.majorStatus){const row=PLAYER_STATUS_META_V56[e.majorStatus]||[e.majorStatus,'◆'];chips.push([e.majorStatus,row[0],'']);}if(Number(d.burn)>0)chips.push(['burn','화상',d.burn]);if(Number(d.poison)>0)chips.push(['poison','독',d.poison]);if(Number(d.shock)>0)chips.push(['shock','감전',d.shock]);if(Number(d.weak)>0)chips.push(['weak','공격↓',d.weak]);if(Number(d.vulnerable)>0)chips.push(['vulnerable','방어↓',d.vulnerable]);if(Number(d.intentSeal)>0)chips.push(['seal','봉쇄',d.intentSeal]);if(e.broken)chips.push(['break','BREAK','']);return chips.length?`<div class="enemy-debuffs-v54 enemy-debuffs-v55">${chips.map(([k,n,v])=>`<span class="debuff-${k}" title="${esc(n)}">${esc(n)}${v?` ${v}`:''}</span>`).join('')}</div>`:'';
  }
  function tacticHintV54(card,monster,enemy){
    if(!card||!monster)return'';if(card.element===monster.element)return'공명 추천 · 다음 기술 +20%';const effects=card.effects||[];if(monster.hp/Math.max(1,monster.maxHp)<.5&&effects.some(x=>['heal','monsterHeal','block','monsterShield'].includes(x.op)))return'위기 대응 추천';if(enemy&&effects.some(x=>['weak','vulnerable','burn','poison','poisonAll','shockAll','intentSealAll'].includes(x.op)))return'상태이상 연계';return'다음 기술 +10%';
  }
  function waitForCombatCinematicV54(){const ms=Math.max(0,Number(state.combatCinematicUntil||0)-Date.now());return ms>0?wait(Math.min(ms,45000)):Promise.resolve();}
  function scheduleRewardAutoAdvanceV54(r,step,continued){
    clearTimeout(state.rewardAutoTimer);state.rewardAutoTimer=0;if(!['READY','SHOP'].includes(step)||continued||state.busy)return;const key=`${r.id}:${r.floor}:${r.seq}`;const delay=step==='SHOP'?14000:(r.finalClearPending?5200:6500);
    state.rewardAutoTimer=setTimeout(async()=>{if(state.current!=='reward'||!state.room||`${state.room.id}:${state.room.floor}:${state.room.seq}`!==key||state.busy)return;try{await roomPost('continue',{});}catch{}},delay);
  }
  function moveStatus(move,u){
    if(!move)return'LOCK';if(Number(u?.level||1)<Number(move.unlockLevel||1))return`Lv.${move.unlockLevel}`;
    const pp=Math.max(0,Number(move.pp??move.maxPp??0)),max=Math.max(1,Number(move.maxPp||move.ppMax||pp||1));const m=moveMasteryUi(move);return `${m.label?`숙련${m.label} · `:''}PP ${pp}/${max}`;
  }
  function moveUsable(move,u,me){return !!move&&!u?.acted&&!me?.ended&&state.room?.battle?.phase==='players'&&Number(u?.level||1)>=Number(move.unlockLevel||1)&&Number(move.pp??move.maxPp??1)>0;}
  function monsterMoveEventFx(payload){
    if(!payload)return;const mv=payload.move||{},mapped={...payload,skill:mv.name||payload.skill||'MOVE',element:mv.element||payload.element,move:mv};
    if(['guard','heal','status'].includes(mv.kind)&&!['guard','heal'].includes(namedMoveFamily(mapped))){
      const actor=$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`);if(!actor)return;attackLabelV40(actor,mv.name||'SKILL','ally');namedSkillOverlayV45(actor,mv.kind==='heal'?'heal':mv.kind==='guard'?'guard':'time',mapped);
      if(mv.kind==='guard'){shieldGainFx(actor,Number(mv.shield||6));sound('block');}else if(mv.kind==='heal'){healBloomFx(actor,{element:payload.element||'빛',rarity:'rare',effects:[{op:'heal',value:Number(mv.heal||6)}]});popNumber(actor,`+${Number(mv.heal||6)}`,'heal');sound('heal');}else{screenFlash('break');sound('chain');}return;
    }
    return playNamedMonsterMoveV45(mapped,false);
  }
  const PLAYER_STATUS_META_V56={burn:['화상','♨'],poison:['중독','☠'],paralysis:['마비','ϟ'],sleep:['수면','Z'],freeze:['빙결','❄']};
  function playerStatusBadgeV56(u){const row=PLAYER_STATUS_META_V56[u?.majorStatus];if(!row)return'';return `<i class="player-major-status-v56 status-${esc(u.majorStatus)}" title="${esc(row[0])}">${esc(row[1])} ${esc(row[0])}</i>`;}
  function statStageBadgesV56(u){const st=u?.statStages||{},rows=[['atk','ATK'],['def','DEF'],['speed','SPD']].filter(([k])=>Number(st[k]||0)!==0);return rows.length?`<span class="stat-stage-row-v56">${rows.map(([k,n])=>`<i class="${Number(st[k])>0?'up':'down'}">${n}${Number(st[k])>0?'↑':'↓'}${Math.abs(Number(st[k]))}</i>`).join('')}</span>`:'';}
  function allMyBattleMonstersV56(){const me=state.room?.battle?.party?.find(x=>x.playerId===state.profileId);return [...(me?.units||[]),...(me?.bench||[])].filter(x=>x.hp>0);}
  function fusionMovePoolV56(a,b){const map=new Map();for(const mv of [...(a?.moves||[]),...(b?.moves||[])])if(mv?.id&&!map.has(mv.id))map.set(mv.id,mv);return [...map.values()];}
  function fusionPreviewTypesV56(a,b){return [...new Set([a?.element,a?.secondaryElement,b?.element,b?.secondaryElement].filter(Boolean))].slice(0,2);}
  function openFusionBuilderV56(primaryId,secondaryId){
    const all=allMyBattleMonstersV56(),a=all.find(x=>x.instanceId===primaryId),b=all.find(x=>x.instanceId===secondaryId);if(!a||!b||a.instanceId===b.instanceId||a.fused||b.fused)return;
    const pool=fusionMovePoolV56(a,b),need=Math.min(4,pool.length);state.fusionDraft={primaryId:a.instanceId,secondaryId:b.instanceId,moves:pool.slice(0,need).map(x=>x.id)};renderFusionBuilderV56();
  }
  function renderFusionBuilderV56(){
    ...
    modal(`<div class="fusion-builder-v56"><header><small>TURN FUSION · ACTION COST 1</small><h2>${esc(a.name)} <em>×</em> ${esc(b.name)}</h2><p>이번 행동을 소비해 두 몬스터를 하나로 합칩니다. 융합은 5턴 동안 유지되며 전투가 끝나면 즉시 해제됩니다.</p></header><section class="fusion-pair-v56"><article><img src="${esc(monsterDisplaySprite(a))}"><b>${esc(a.name)}</b><small>${esc(a.element)} · HP ${a.hp}/${a.maxHp}</small></article><strong>×</strong><article><img src="${esc(monsterDisplaySprite(b))}"><b>${esc(b.name)}</b><small>${esc(b.element)} · HP ${b.hp}/${b.maxHp}</small></article><i>→</i><article class="result"><div class="fusion-preview-wrap-v58"><img class="fusion-preview-v58" src="${esc(fusionSpriteUrlV58(a,b))}" alt="융합 미리보기"><i>∞</i></div><b>${esc(a.name)} × ${esc(b.baseName||b.name)}</b><small>${types.map(x=>esc(x)).join(' / ')} · 예상 HP ${hp} · ATK ${atk}</small></article></section><div class="fusion-rule-v56"><span>행동 1회 소비</span><span>기술 ${need}개 선택</span><span>정확히 5턴 유지</span><span>전투 종료 시 자동 해제</span></div><div class="fusion-moves-v56">${pool.map((mv,i)=>{const on=selected.has(mv.id),pp=Number(mv.pp??mv.maxPp??0),max=Number(mv.maxPp||pp||1);return `<button data-action="fusion-toggle-move" data-move-id="${esc(mv.id)}" class="${on?'selected':''} element-${elementClass(mv.element||a.element)}"><em>${on?'✓':String(i+1).padStart(2,'0')}</em><div><b>${esc(mv.name)}</b><small>${esc(mv.element||a.element)} · ${esc(moveKindKo(mv))} · PP ${pp}/${max}</small><span>${esc(movePowerLine(mv))}</span></div></button>`;}).join('')}</div><footer><small>${selected.size}/${need} 선택 · 정확히 ${need}개를 선택하세요.</small><button data-action="modal-close">취소</button><button class="cta" data-action="fusion-confirm" ${selected.size===need?'':'disabled'}>1턴 사용 · 융합</button></footer></div>`);
  }
  function showFusionMenu(instanceId){
    const all=allMyBattleMonstersV56(),a=all.find(x=>x.instanceId===instanceId)||all.find(x=>!x.fused)||all[0];if(!a||a.fused)return;const partners=all.filter(x=>x.instanceId!==a.instanceId&&!x.fused);
    modal(`<div class="fusion-partner-v56"><header><small>TURN FUSION</small><h2>${esc(a.name)}와 융합할 몬스터</h2><p>전투 중 한 턴을 소비합니다. 5턴 동안 융합한 뒤 자동으로 원래 두 몬스터로 돌아옵니다.</p></header><div>${partners.map(b=>`<button data-action="fusion-partner" data-primary="${esc(a.instanceId)}" data-secondary="${esc(b.instanceId)}"><img src="${esc(monsterDisplaySprite(b))}"><span><b>${esc(b.name)}</b><small>${esc(b.element)} · Lv.${b.level} · HP ${b.hp}/${b.maxHp}</small></span><strong>선택</strong></button>`).join('')||'<p>융합 가능한 파트너가 없습니다.</p>'}</div><button data-action="modal-close">닫기</button></div>`);
  }
  ...
  function showEvolutionMenu(instanceId){
    ...
    const evoLevel=Number(u.evolutionLevel||18),naturalReady=Number(u.level||1)>=evoLevel,earlyReady=Number(u.level||1)>=3&&Number(u.gene||0)>=3,evoOk=!u.evolved&&(naturalReady||earlyReady),resOk=Number(u.resonanceTurns||0)<=0&&u.resonance>=4,riftOk=!u.abyssBloom&&u.rift>=3&&u.hp/u.maxHp<=.6;
    modal(`<div class="evolution-menu-v41 evolution-menu-v43"><div class="section-label">MONSTER EVOLUTION</div><div class="evo-hero-v41"><img src="${esc(monsterDisplaySprite(u))}"><div><small>${esc(u.element)} · Lv.${u.level}</small><h2>${esc(u.name)}</h2><p>자연 진화는 Lv.${evoLevel}에 자동으로 발생합니다. GENE을 모으면 더 일찍 진화시킬 수도 있습니다.</p></div></div><div class="evo-grid-v41"><button data-action="monster-evolve" data-mode="evolve" data-monster="${esc(u.instanceId)}" ${evoOk?'':'disabled'}><span>▲</span><b>${naturalReady?'자연 진화':'조기 진화'}</b><small>${naturalReady?`Lv.${evoLevel} 달성`:`Lv.${evoLevel} 자동 · 또는 GENE 3`}</small></button><button data-action="open-fusion" data-monster="${esc(u.instanceId)}" ${(me.units.length+me.bench.length>1)&&!u.acted&&!u.fused?'':'disabled'}><span>∞</span><b>턴 융합</b><small>5턴 유지 · 전투 종료 시 해제</small></button><button data-action="monster-evolve" data-mode="resonance" data-monster="${esc(u.instanceId)}" ${resOk?'':'disabled'}><span>✦</span><b>공명진화</b><small>RES 4 · 3턴</small></button><button data-action="monster-evolve" data-mode="rift" data-monster="${esc(u.instanceId)}" ${riftOk?'':'disabled'}><span>◆</span><b>균열개화</b><small>HP 60%↓ · RIFT 3</small></button></div><button class="cta" data-action="modal-close">닫기</button></div>`);
  }
  function showMoveTeachMenu(offer,moveId){
    const move=(offer?.moves||[]).find(x=>x.id===moveId);if(!offer||!move)return;const current=offer.currentMoves||[],isLevel=offer.source==='level';
    modal(`<div class="move-teach-modal-v41 move-teach-modal-v43 move-teach-modal-v44"><div class="section-label">${isLevel?'LEVEL-UP MOVE':'TECH DISC'}</div><header class="teach-head-v44"><img src="${esc(offer.monsterSprite||'')}"><div><small>${esc(offer.reason||'NEW MOVE')}</small><h2>${esc(offer.monsterName)}</h2><p>${isLevel?'레벨이 오르며 새로운 기술을 떠올렸습니다.':'획득한 기술 디스크에서 새 기술을 배울 수 있습니다.'}</p></div></header><section class="teach-new-v44 element-${elementClass(move.element||'')}"><span>${moveKindIcon(move)}</span><div><small>NEW MOVE</small><b>${esc(move.name)}</b><em>${esc(movePowerLine(move))} · 명중 ${move.accuracy||100}% · PP ${move.maxPp||move.pp||'-'}</em><p>${esc(move.summary||moveKindKo(move))}</p></div></section><div class="teach-question-v44"><b>어떤 기술과 교체할까요?</b><small>기존 기술을 하나 선택하면 즉시 교체됩니다.</small></div><div class="replace-moves-v41 replace-moves-v44">${[0,1,2,3].map(i=>{const old=current[i],m=moveMasteryUi(old);return `<button data-action="learn-move" data-move-id="${esc(move.id)}" data-replace-index="${i}"><em>0${i+1}</em><div><b>${old?esc(old.name):'EMPTY'}</b><small>${old?esc(movePowerLine(old)):'빈 기술 슬롯'}</small>${old?`<span>${esc(moveKindKo(old))} · ${old.accuracy||100}% · PP ${old.maxPp||old.pp||'-'}${m.label?` · 숙련${m.label}`:''}</span>`:''}</div><strong>교체</strong></button>`;}).join('')}</div><div class="modal-actions"><button class="cta" data-action="modal-close">다시 보기</button></div></div>`);
  }


  function captureThrowFx(button,card,seal='basic'){
    if(!fxOn()||!button)return Promise.resolve();const r=button.getBoundingClientRect(),root=fxRoot(),orb=document.createElement('div');orb.className=`capture-orb-flight-v34 seal-${seal}`;orb.style.left=`${r.left+r.width/2}px`;orb.style.top=`${r.top+r.height/2}px`;orb.innerHTML='<i></i><b></b>';root.appendChild(orb);const target=(state.current==='battle'&&state.selectedEnemy)?$(`[data-enemy=\"${CSS.escape(state.selectedEnemy)}\"]`):($('.hunt-monster-v40')||$('.hunt-card-v32')),t=target?rectCenter(target):{x:innerWidth/2,y:innerHeight/2};const dx=t.x-(r.left+r.width/2),dy=t.y-(r.top+r.height/2);orb.style.setProperty('--dx',`${dx}px`);orb.style.setProperty('--dy',`${dy}px`);sound('card');setTimeout(()=>{target?.classList.add('capture-lock-v34');screenShake('soft');},300);setTimeout(()=>{target?.classList.remove('capture-lock-v34');orb.remove();},720);return wait(520);
  }
  ...
  function spellSigilLabel(card){
    const mode=cardFxMode(card),trait=cardFxTrait(card);
    if(mode==='attack')return trait==='aoe'?'광역 공명 지령':'공격 공명 지령';
    if(mode==='heal')return trait==='heal'?'회복 공명 지령':'지원 공명 지령';
    if(mode==='guard')return '방호 공명 지령';
    return trait==='debuff'?'약화 공명 지령':'전술 공명 지령';
  }
  function spellSigilIcon(card){
    const mode=cardFxMode(card),trait=cardFxTrait(card),ops=new Set((card?.effects||[]).map(e=>e.op));
    if(mode==='attack'&&trait==='aoe')return '✹';
    if(mode==='attack'&&ops.has('burn'))return '✦';
    if(mode==='attack'&&trait==='debuff')return '✶';
    if(mode==='heal')return '✚';
    if(mode==='guard')return '⬡';
    if(ops.has('draw')||ops.has('discover'))return '◎';
    if(ops.has('energy')||ops.has('energyNext'))return '◇';
    return '◈';
  }
  function cardVisualArtHtml(card){
    const role=cardRole(card);
    if(card?.type!=='spell')return `<img src="${esc(card.art)}" loading="lazy" alt=""><span class="role-glyph-v33 role-${role.key}" title="${esc(role.label)}">${role.icon}</span><span class="element-mark-v31 element-${elementClass(card.element)}">${esc(elementGlyph(card.element))}</span>`;
    const ec=elementClass(card.element),mode=cardFxMode(card),trait=cardFxTrait(card),variant=cardFxVariant(card),icon=spellSigilIcon(card),label=spellSigilLabel(card);
    return `<div class="spell-art-shell-v43 mode-${mode} trait-${trait} variant-${variant} element-${ec}">${card.art?`<img class="spell-ghost-v43" src="${esc(card.art)}" loading="lazy" alt="">`:''}<div class="spell-sigil-v43"><i class="spell-ring-v43 outer"></i><i class="spell-ring-v43 mid"></i><i class="spell-ring-v43 inner"></i><b class="spell-core-v43">${icon}</b><span class="spell-label-v43">${esc(label)}</span></div><span class="role-glyph-v33 role-${role.key}" title="${esc(role.label)}">${role.icon}</span><span class="element-mark-v31 element-${ec}">${esc(elementGlyph(card.element))}</span></div>`;
  }
  function collectionArtHtml(card){
    if(card?.type!=='spell')return `<img src="${esc(card.art)}" loading="lazy" alt=""><span>${esc(elementGlyph(card.element))}</span>`;
    const ec=elementClass(card.element),mode=cardFxMode(card),trait=cardFxTrait(card),variant=cardFxVariant(card);
    return `<div class="collection-spell-art-v43 mode-${mode} trait-${trait} variant-${variant} element-${ec}">${card.art?`<img class="spell-ghost-v43" src="${esc(card.art)}" loading="lazy" alt="">`:''}<i class="outer"></i><i class="mid"></i><i class="inner"></i><b>${spellSigilIcon(card)}</b><small>${esc(spellSigilLabel(card))}</small><span>${esc(elementGlyph(card.element))}</span></div>`;
  }
  function stripCardThumbHtml(card){
    if(card?.type!=='spell')return `<img src="${esc(card.art)}" alt=""><span>${esc(tacticDisplayNameV55(card))}</span>`;
    const ec=elementClass(card.element),mode=cardFxMode(card),trait=cardFxTrait(card),variant=cardFxVariant(card);
    return `<div class="strip-spell-thumb-v43 mode-${mode} trait-${trait} variant-${variant} element-${ec}"><i class="outer"></i><i class="inner"></i><b>${spellSigilIcon(card)}</b></div><span>${esc(card.name)}</span>`;
  }
  function moveFxStyle(payload,fallback='beast'){
    const skill=String(payload?.skill||'').toLowerCase(),element=String(payload?.element||'');
    if(/beam|ray|laser|포톤|광선|심판|정화|lance/.test(skill))return 'beam';
    if(/heal|cure|회복|치유|regener|bless/.test(skill))return 'heal';
    if(/shield|guard|barrier|보호|방패|수호/.test(skill))return 'guard';
    if(/wave|surge|tidal|storm|파동|해일|격류|돌풍/.test(skill)||['물','바람'].includes(element))return 'wave';
    if(/meteor|eruption|quake|nova|폭발|분화|낙뢰|심판|충돌/.test(skill)||['화염','번개','강철'].includes(element))return 'eruption';
    if(/shadow|mist|spore|orb|shot|arrow|bolt|환영|독|그림자/.test(skill)||['그림자','공허','자연'].includes(element))return 'projectile';
    if(/rush|slash|fang|claw|strike|베기|연격|참격|물기|발톱/.test(skill))return 'melee';
    const arche=String(fallback||'');
    if(['watcher','priest','drone','construct'].includes(arche))return 'beam';
    if(['spirit','wraith','phoenix','mushroom'].includes(arche))return 'projectile';
    if(['wing','serpent'].includes(arche))return 'wave';
    if(['golem','tyrant','leviathan'].includes(arche))return 'eruption';
    return ['assassin','knight','crab','beast','insect'].includes(arche)?'melee':'projectile';
  }
  function payloadFxCard(payload,heavy=false){
    const skill=String(payload?.skill||''),style=moveFxStyle(payload,payload?.archetype||payload?.style||'beast'),damage=Number(payload?.damage||0),element=payload?.element||'강철';
    const fake={element,rarity:heavy?'legendary':'rare',power:damage||8,effects:[]};
    if(style==='heal')fake.effects=[{op:'heal',value:Math.max(8,damage||12)}];
    else if(style==='guard')fake.effects=[{op:'block',value:Math.max(6,damage||10)}];
    else if(style==='wave' && /all|광역|폭풍|storm|nova/i.test(skill))fake.effects=[{op:'damageAll',value:Math.max(8,damage||10)}];
    else if(/burn|화상|ember|flame/i.test(skill))fake.effects=[{op:'burn',value:2},{op:'damage',value:Math.max(4,damage||8)}];
    else if(/weak|쇠약|vulner/i.test(skill))fake.effects=[{op:'vulnerable',value:1},{op:'damage',value:Math.max(4,damage||8)}];
    else fake.effects=[{op:'damage',value:Math.max(4,damage||8)}];
    return fake;
  }
  ...
  function cardHtml(card,opt={}){
    ...
    const visual=!!opt.visual,role=cardRole(card),spell=card.type==='spell';
    const cls=`game-card${opt.compact?' mini-card':''}${opt.claimed?' claimed':''}${card.upgradeLevel?' upgraded':''}${visual?' visual-card-v31 visual-card-v33':''} role-${role.key}${spell?' spell-card-v43':''}`;
    const attrs=[`data-rarity="${esc(card.rarity)}"`,`data-card-id="${esc(card.id)}"`,`title="${esc(card.name)} · ${esc(role.label)} · 우클릭/길게 누르기: 상세 능력"`];
    ...
    const stats=card.type==='unit'?`<span>HP ${card.hp}</span><span>ATK ${card.power}</span>`:`${card.power?`<span>DMG ${card.power}</span>`:''}${card.block?`<span>DEF ${card.block}</span>`:''}${card.type==='spell'&&!card.power&&!card.block?`<span>TACTIC</span>`:''}`;
    ...
    return `<${tag} class="${cls}" ${attrs.join(' ')}>${upgrade}${mastery}${synergy}<div class="rarity-ribbon rarity-${card.rarity}">${esc(rarityKo(card.rarity))}${card.limited?' · LIMITED':''}</div><div class="card-top"><span class="cost">${card.cost}</span><span class="card-name" title="${esc(card.name)}">${esc(card.name)}</span></div><div class="card-art">${cardVisualArtHtml(card)}</div><div class="card-type"><span>${spell?spellSigilLabel(card):role.label}</span><span>${esc(card.element)}</span></div>${body}<div class="card-stats">${stats}</div></${tag}>`;
  }
  ...
  function featureChips(){return `<div class="feature-strip"><span class="feature-chip">MONSTER HUNT</span><span class="feature-chip">205 SIGNATURES</span><span class="feature-chip">128 ITEMS</span><span class="feature-chip">205 MONSTERS</span><span class="feature-chip">50 FLOORS</span><span class="feature-chip">TURN FUSION</span></div>`;}
  ...
  async function savePrepAndProceed(){
    ...
    try{const d=await api('/api/loadout',{body:{profileId:state.profileId,nickname:state.profile.nickname,monsterParty:state.monsterDraft}});state.profile=d.profile;state.monsterDraft=[...d.profile.monsterParty];state.deckDraft=[...(d.profile.deck||[])];updateTopbar();const mode=state.prepMode;if(mode==='manage'){toast('파티 편성을 저장했습니다.','good');state.prepMode='';return renderHome();}if(mode==='journey')return createJourney();return renderDungeonLobby();}catch(e){toast(e.message,'error');}
  }
  async function renderDungeonLobby(){
    setScreen('dungeonLobby',`<section class="page"><div class="page-head"><button class="back-btn" data-action="home">← 홈</button><div><div class="section-label">CO-OP DUNGEON</div><h1>50층 협동 던전</h1><p>MONSTER-ONLY 50-WAVE RUN / HP, PP AND MAJOR STATUS PERSIST BETWEEN WAVES.</p></div></div><div class="lobby-grid"><div class="room-create"><h2>새 원정대 만들기</h2><p>1-4 PLAYER / PARTY, HP, PP, STATUS AND FUSION ARE SERVER-SYNCED.</p>${difficultySelector()}<div class="field"><input id="roomNameInput" maxlength="24" placeholder="원정대 이름 (선택)"><button class="cta mint" data-action="create-dungeon">방 만들기</button></div><div style="height:8px"></div><div class="field"><input id="roomCodeInput" maxlength="6" inputmode="numeric" placeholder="6자리 방 코드"><button class="cta" data-action="join-code">코드 참가</button></div></div><div class="public-rooms"><div class="section-label">OPEN ROOMS</div><h2 style="font-size:13px">공개 원정대</h2><div id="publicRooms"><p class="small-note">방 목록을 불러오는 중...</p></div></div></div></section>`);
  ...
  function contractPanel(r){
    const claim=r.contractClaims?.[state.profileId],offers=r.contractOffers?.[state.profileId]||[];
    if(claim){const c=offers.find(x=>x.id===claim);return `<div class="starter-picked-v51 starter-picked-v52"><span>${esc(c?.icon||'◇')}</span><div><small>원정 보너스</small><b>${esc(c?.name||claim)}</b></div></div>`;}
    return `<section class="starter-choice-v51 starter-choice-v52"><header><small>EXPEDITION BONUS</small><b>이번 원정의 보너스를 하나 고르세요</b><span>한 번만 선택합니다. 선택하면 전투 준비 화면으로 이어집니다.</span></header><div class="starter-grid-v51 starter-grid-v52">${offers.slice(0,3).map((c,i)=>`<button data-action="contract-pick" data-contract-id="${esc(c.id)}"><span>${esc(c.icon||'◇')}</span><div><b>${esc(c.name)}</b><small>${esc(c.detail||c.desc||'원정 보너스')}</small></div></button>`).join('')}</div></section>`;
  }

  function briefingHtml(r){const nextBoss=r.mode==='dungeon'?Math.min(r.maxFloor||50,Math.ceil(r.floor/10)*10):null;return `<div class="briefing-panel"><div><div class="section-label">EXPEDITION BRIEFING</div><h3>${esc(r.biome.name)} · ${r.floor}F</h3><p>HP / PP / STATUS MANAGEMENT, SWITCHING, HELD ITEMS AND TURN FUSION DEFINE THE RUN.</p></div><div class="briefing-meta"><span>${r.mode==='dungeon'?`다음 지역 보스 ${nextBoss}F`:'끝없는 여행 모드'}</span><span>전투 보상은 필요 없으면 분해 가능</span><span>정예/보물/보스 → 추가 유물 선택권</span></div></div>`;}
  ...
  function partyStrip(r){return `<div class="party-strip">${r.players.map(p=>{const run=r.runState[p.id];return `<div class="party-chip"><b>${esc(p.nickname)}${p.id===state.profileId?' · YOU':''}</b><small>HP ${run?.hp||0}/${run?.maxHp||0} · G ${run?.gold||0} · PARTY ${(run?.monsters||[]).length} · 아이템 ${Object.keys(run?.items||{}).length}</small><div class="hpbar"><i style="width:${pct(run?.hp,run?.maxHp)}%"></i></div></div>`;}).join('')}</div>`;}
  function compactRunHud(r,run){
    const scene=sceneName(r),gold=Number(run?.gold||0).toLocaleString('ko-KR');
    return `<div class="run-hud-v27 run-hud-v53"><div class="run-floor-v27"><strong>${r.floor}</strong><span>F</span><div><b>${esc(scene)}</b><small>${r.mode==='dungeon'?difficultyKo(r.difficulty):`MONSTER HUNT · ${state.profile.monsterOwnedCount}/${state.profile.totalMonsters}`}</small></div></div><div class="run-money-v53"><small>RUN MONEY</small><b>G ${gold}</b></div><div class="run-pills-v27"><span>◇ ${run.fragments||0}</span><span>✦ ${(run.relics||[]).length}</span><button data-action="run-info">RUN / 장착</button></div></div>`;
  }
  function heldItemClient(mon){return mon?.heldItemId?byId(state.meta?.items,mon.heldItemId):null;}
  function heldLabelV53(mon){const item=heldItemClient(mon);return item?`<i class="held-inline-v53" title="${esc(item.text||'')}">${esc(item.icon||'◆')} ${esc(item.name)}</i>`:'<i class="held-inline-v53 empty">장착 없음</i>';}
  function showHeldItemMenuV53(instanceId){
    const r=state.room,run=r?.runState?.[state.profileId],mon=(run?.monsters||[]).find(x=>x.instanceId===instanceId);if(!r||!run||!mon)return;
    const current=heldItemClient(mon),equipped={};for(const m of run.monsters||[]){if(m.instanceId!==mon.instanceId&&m.heldItemId)equipped[m.heldItemId]=(equipped[m.heldItemId]||0)+1;}
    const held=Object.entries(run.items||{}).map(([id,n])=>({item:byId(state.meta.items,id),n:Number(n||0)})).filter(x=>x.item?.held);
    const choices=held.map(({item,n})=>{const free=Math.max(0,n-Number(equipped[item.id]||0)),on=current?.id===item.id;return `<button class="held-choice-v53 ${on?'on':''}" data-action="equip-held-item" data-monster="${esc(mon.instanceId)}" data-item="${esc(item.id)}" ${free<1&&!on?'disabled':''}><span>${esc(item.icon||'◆')}</span><div><b>${esc(item.name)}</b><small>${esc(item.text)}</small></div><strong>${on?'장착 중':`사용 가능 ${free}`}</strong></button>`;}).join('');
    modal(`<div class="held-modal-v53"><div class="section-label">HELD ITEM</div><header><img src="${esc(monsterDisplaySprite(mon))}" alt=""><div><small>${esc(mon.element)} · Lv.${mon.level}</small><h2>${esc(mon.name)}</h2><p>몬스터 한 마리당 열매 또는 장착 아이템 1개를 들 수 있습니다.</p></div></header><div class="held-current-v53"><small>현재 장착</small><b>${current?`${esc(current.icon||'◆')} ${esc(current.name)}`:'없음'}</b></div><div class="held-choice-list-v53">${choices||'<p class="held-empty-v53">보유한 장착형 아이템이 없습니다. 전투 보상이나 상점에서 획득할 수 있습니다.</p>'}</div><div class="modal-actions">${current?`<button class="cta" data-action="equip-held-item" data-monster="${esc(mon.instanceId)}" data-item="">장착 해제</button>`:''}<button class="cta mint" data-action="run-info">← RUN 정보</button><button class="cta" data-action="modal-close">닫기</button></div></div>`);
  }
  ...
  function showRunInfo(){
    ...
    const canEquip=['route','reward','event'].includes(r.status);const monsters=(run.monsters||[]).map(mon=>`<button class="held-mon-row-v53" data-action="open-held-item" data-monster="${esc(mon.instanceId)}" ${canEquip?'':'disabled'}><img src="${esc(monsterDisplaySprite(mon))}" alt=""><div><b>${esc(mon.name)}</b><small>Lv.${mon.level} · HP ${mon.hp}/${mon.maxHp}</small>${heldLabelV53(mon)}</div><strong>${canEquip?'변경 ›':'전투 중'}</strong></button>`).join('');
    modal(`<div class="run-info-modal-v27 run-info-modal-v53"><div class="section-label">RUN STATUS</div><h2>${r.floor}F · ${esc(r.biome.name)}</h2><div class="run-info-grid-v27"><div><small>CORE HP</small><b>${run.hp}/${run.maxHp}</b></div><div class="money-card-v53"><small>RUN MONEY</small><b>${Number(run.gold||0).toLocaleString('ko-KR')}G</b></div><div><small>PARTY</small><b>${(run.monsters||[]).filter(m=>m.hp>0).length}/${(run.monsters||[]).length}</b></div><div><small>PRESSURE</small><b>×${(r.threatTokens||[]).length}</b></div></div><h3>몬스터 장착 아이템</h3><p class="held-help-v53">몬스터를 눌러 열매/장착 아이템을 바꿀 수 있습니다. 장착 아이템은 전투 밖에서만 변경됩니다.</p><div class="held-roster-v53">${monsters}</div><h3>유물</h3><div class="simple-chip-list-v27">${relics.length?relics.map(x=>`<span title="${esc(x.text)}">${esc(x.icon||'✦')} ${esc(x.name)}</span>`).join(''):'<span>아직 없음</span>'}</div><h3>런 아이템</h3><div class="simple-chip-list-v27">${items.length?items.map(x=>`<span title="${esc(x.item.text)}">${esc(x.item.icon||'◆')} ${esc(x.item.name)} ×${x.n}</span>`).join(''):'<span>아직 없음</span>'}</div><div class="modal-actions"><button class="cta mint" data-action="modal-close">닫기</button></div></div>`);
  }
  function showForgeMenu(){
    ...
    modal(`<div class="forge-modal-v27"><div class="section-label">COMMAND FORGE</div><h2>강화할 카드 1장을 선택하세요.</h2><p>카드 수치가 강화되고, ++ 단계에서는 비용 2 이상 카드의 에너지가 1 감소합니다.</p><div class="forge-grid-v27">${uniq.map(x=>`<button data-action="camp-upgrade" data-card-id="${esc(x.c.id)}" ${done?'disabled':''}><img src="${esc(x.c.art)}" alt=""><b>${esc(x.c.name)}</b><small>${x.lv===0?'→ +':'+ → ++'}</small></button>`).join('')}</div><div class="modal-actions"><button class="cta" data-action="modal-close">취소</button></div></div>`);
  }
  ...
  function rewardStageHeader(r,run,step){return `<header class="reward-hud-v27"><div><small>${esc(step)}</small><b>${r.floor}F · ${esc(r.biome.name)}</b></div><div><span class="reward-money-v53">G ${Number(run.gold||0).toLocaleString('ko-KR')}</span><span>◇ ${run.fragments||0}</span><button data-action="run-info">RUN / 장착</button></div></header>`;}
  // V42 compatibility tokens retained for regression checks: 다음 10웨이브를 고르세요 · LEVEL CAP
  function renderRoute(r){
    const run=r.runState[state.profileId],nodes=r.route||[],claim=r.contractClaims?.[state.profileId];
    const fork=nodes.length>1||nodes.some(n=>n.biomeChoice),votedNode=nodes.find(n=>n.votes?.includes(state.profileId));
    let decision='';
    if(!claim){
      decision=contractPanel(r);
    }else if(fork){
      ...
      const node=nodes[0],voted=node?.votes?.includes(state.profileId);
      const eventNode=node?.kind==='event';
      decision=`<div class="route-loading-v52 route-loading-v53 route-loading-v58 ${eventNode?'is-mystery':''}"><span>${eventNode?'?':'✦'}</span><small>${esc(node?.kicker||'NEXT BATTLE')}</small><b>${esc(node?.label||'전투 준비')}</b><p>${voted?'다른 원정대원의 준비를 기다리고 있습니다.':eventNode?'특별한 사건이 감지되었습니다. 잠시 후 선택 화면으로 자동 이동합니다.':'잠시 후 야생 조우가 자동으로 시작됩니다.'}</p>${node&&!voted?`<button class="encounter-start-v53" data-action="encounter-start" data-node="${esc(node.id)}">${eventNode?'바로 확인':'바로 진행'}</button><small class="auto-count-v58">AUTO · 약 1초</small>`:'<div class="encounter-wait-v53">READY…</div>'}</div>`;
    }
    setScreen('route',`<section class="run-stage-v31 route-stage-v31 route-v42 route-v51 route-v52"><div class="run-bg-v31" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="run-shade-v31"></div>${compactRunHud(r,run)}<main class="decision-center-v31 encounter-decision-v51 encounter-decision-v52">${decision}</main><div class="floor-track-v31"><i style="width:${r.maxFloor?pct(r.floor,r.maxFloor):Math.min(100,(r.floor%50)*2)}%"></i><span>${r.floor}/${r.maxFloor||'∞'}</span></div></section>`);
    if(!fork&&claim&&nodes[0]&&!nodes[0].votes?.includes(state.profileId))queueAutoEncounter(r,nodes[0]);
  }
  ...
  function renderBattle(r){
    ...
    const mode=b.battleMode||((b.activeSlots||1)>1?'double':'single'),menu=state.battleMenu||'root';
    const monActors=me.units.map(u=>{const held=heldItemClient(u);return `<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${held?` <i class="battle-held-v53" title="${esc(held.text||'')}">${esc(held.icon||'◆')} ${esc(held.name)}</i>`:''}${playerStatusBadgeV56(u)}${statStageBadgesV56(u)}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i><img class="${u.fused?'fused-visual-v58':''}" src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async"><span class="mon-ring-v41"></span>${u.fused?`<strong class="fusion-turn-badge-v59">∞ ${Number(u.fusionTurnsRemaining||5)}T</strong>`:''}</div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`;}).join('');
    const enemyActors=alive.map(e=>{const it=intentCompact(e.intent);return `<button class="enemy-mon-v41 enemy-mon-v44 enemy-mon-v51 enemy-mon-v52 ${state.selectedEnemy===e.uid?'selected':''} ${e.tier==='boss'?'is-boss-v50':''}" data-action="select-enemy" data-enemy="${esc(e.uid)}"><div class="enemy-status-v51 enemy-status-v52"><div><b>${esc(e.name)}</b><small>${esc(e.element||'')} ${enemyWeakBadge(e.element)}</small></div><span>${e.hp}/${e.maxHp}</span><i><em style="width:${pct(e.hp,e.maxHp)}%"></em></i>${Number(e.hpSegmentsMax||1)>1?`<div class="boss-hp-segments-v56">${Array.from({length:Number(e.hpSegmentsMax||1)},(_,si)=>`<i class="${si<Number(e.hpSegments||1)?'on':''}"></i>`).join('')}<small>SHIELD ${Number(e.hpSegments||1)}/${Number(e.hpSegmentsMax||1)}</small></div>`:''}${enemyDebuffBadgesV54(e)}</div><div class="enemy-sprite-v51 enemy-sprite-v52"><i class="enemy-shadow-v41"></i><img class="enemy-sprite" src="${esc(e.sprite)}" alt="${esc(e.name)}" decoding="async"><span class="target-ring-v41"></span></div></button>`;}).join('');

    const canRun=r.mode==='journey'&&b.encounterType==='wild'&&b.tier==='combat'&&r.players?.length===1&&!selectedMon?.acted;
    const rootMenu=`<div class="root-command-v51 root-command-v52 root-command-v58"><button class="fight" data-action="battle-menu" data-menu="moves" ${selectedMon?.acted?'disabled':''}><span>⚔</span><b>싸운다</b><small>기술 선택</small></button><button class="capture" data-action="battle-menu" data-menu="capture" ${!['wild','boss'].includes(b.encounterType)||selectedMon?.acted?'disabled':''}><span>◉</span><b>봉인</b><small>${selectedEnemy?`${Math.round(Number(selectedEnemy.hp||0)/Math.max(1,Number(selectedEnemy.maxHp||1))*100)}% HP`:'대상 없음'}</small></button><button class="party" data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length||selectedMon?.acted?'disabled':''}><span>↔</span><b>파티</b><small>${(me.bench||[]).length} 대기</small></button><button class="run" data-action="battle-run" ${canRun?'':'disabled'}><span>➜</span><b>도주</b><small>${canRun?'야생 전투 이탈':'사용 불가'}</small></button><button class="fusion fusion-command-v56" data-action="battle-menu" data-menu="fusion" ${selectedMon?.acted||selectedMon?.fused||([...(me.units||[]),...(me.bench||[])]).filter(x=>x.hp>0&&!x.fused&&x.instanceId!==selectedMon?.instanceId).length<1?'disabled':''}><span>∞</span><b>융합</b><small>${selectedMon?.fused?`융합 중 · ${Number(selectedMon.fusionTurnsRemaining||0)}턴`:'5턴 유지 · 행동 1회'}</small></button></div>`;
    const moves=selectedMon?.moves||[];
    const moveMenu=`<div class="move-command-v51 move-command-v52 move-command-v54"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>${esc(selectedMon?.name||'MONSTER')}</small><b>기술을 선택하세요</b></div></div><div class="move-grid-v51 move-grid-v52">${[0,1,2,3].map(i=>{const mv=moves[i];if(!mv)return `<button class="move-btn-v51 move-btn-v52 empty" disabled><em>${i+1}</em><b>비어 있음</b></button>`;const can=moveUsable(mv,selectedMon,me),power=movePowerLine(mv),el=mv.element||selectedMon.element,tags=moveEffectTagsV54(mv),eff=moveEffectivenessV58(el,selectedEnemy);return `<button class="move-btn-v51 move-btn-v52 move-btn-v54 element-${elementClass(el)} ${mv.signature?'signature-move-v54':''}" data-action="move-use" data-move-id="${esc(mv.id)}" ${can?'':'disabled'}><em>${i+1}</em><span>${namedSkillGlyphV47(namedMoveFamily({skill:mv.name,element:el,move:mv}))}</span><div><b>${esc(mv.name)}</b><small class="move-type-line-v54">${elementGlyph(el)} ${esc(el)} · ${esc(moveKindKo(mv))} · 명중 ${mv.accuracy||100}% · PP ${Number(mv.pp??mv.maxPp??0)}/${Number(mv.maxPp||mv.pp||1)}</small><small>${esc(power||moveKindKo(mv))}</small><div class="move-effect-tags-v54">${tags.map(x=>`<i class="effect-${x.k}">${esc(x.t)}</i>`).join('')}<i class="effectiveness-v58 eff-${eff.mult>1?'up':eff.mult<1?'down':'neutral'}">${esc(eff.label)}</i></div></div><strong>${mv.signature?'★ SIG':moveStatus(mv,selectedMon)}</strong></button>`;}).join('')}</div></div>`;
    const fusionPartners=[...(me.units||[]),...(me.bench||[])].filter(x=>x.hp>0&&!x.fused&&x.instanceId!==selectedMon?.instanceId);
    const fusionMenu=`<div class="fusion-command-panel-v56"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>TURN FUSION</small><b>${esc(selectedMon?.name||'몬스터')}와 융합할 파트너를 선택하세요</b></div></div><div class="fusion-explain-v56"><b>∞ 우리 게임의 핵심 전술</b><span>행동 1회를 사용해 두 몬스터를 합칩니다. 정확히 5턴 유지되며 전투가 끝나면 남은 턴과 관계없이 즉시 원래 파티로 돌아옵니다.</span></div><div class="fusion-partner-grid-v56">${fusionPartners.map(u=>`<button data-action="fusion-partner" data-primary="${esc(selectedMon?.instanceId||'')}" data-secondary="${esc(u.instanceId)}"><img src="${esc(monsterDisplaySprite(u))}" alt=""><div><b>${esc(u.name)}</b><small>${esc(u.element)}${u.secondaryElement?` / ${esc(u.secondaryElement)}`:''} · Lv.${u.level}</small><span>HP ${u.hp}/${u.maxHp} · ${u.fused?'융합체':'기본체'}</span></div><strong>융합</strong></button>`).join('')||'<span class="fusion-none-v56">융합 가능한 다른 몬스터가 없습니다.</span>'}</div></div>`;
    const captureMenu=battleCaptureMenuV45(selectedEnemy,run,b,me,selectedMon);
    const partyMenu=`<div class="party-command-v51 party-command-v52"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>SWITCH</small><b>교대할 몬스터</b></div></div><div class="party-switch-v51 party-switch-v52">${(me.bench||[]).map(u=>`<button data-action="switch-direct" data-active="${esc(selectedMon?.instanceId||'')}" data-bench="${esc(u.instanceId)}"><img src="${esc(monsterDisplaySprite(u))}" alt="" decoding="async"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · HP ${u.hp}/${u.maxHp}</small></div></button>`).join('')||'<span>대기 몬스터가 없습니다.</span>'}</div></div>`;
    const infoMenu=`<div class="info-command-v51"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>BATTLE INFO</small><b>전투 정보</b></div></div><div class="battle-info-v51"><div><small>내 몬스터</small><b>${esc(selectedMon?.name||'-')}</b><span>${esc(selectedMon?.element||'')}</span></div><div><small>상대</small><b>${esc(selectedEnemy?.name||'-')}</b><span>${esc(elementWeakTo(selectedEnemy?.element||'').slice(0,3).join(' · ')||'약점 없음')}</span></div><button data-action="open-evolution" data-monster="${esc(selectedMon?.instanceId||'')}" ${!selectedMon?'disabled':''}>진화 / 융합</button><button data-action="run-info">RUN 정보</button></div></div>`;
    const command=menu==='moves'?moveMenu:menu==='capture'?captureMenu:menu==='fusion'?fusionMenu:menu==='party'?partyMenu:menu==='info'?infoMenu:rootMenu;
    ...
    const actionHint=ready.length>1?`${ready.length}마리 행동 남음`:ready.length===1?`${ready[0].name}의 차례`:'적의 행동을 기다리는 중';
    const prompt=menu==='root'?'무엇을 할까?':menu==='moves'?'어떤 기술을 사용할까?':menu==='fusion'?'누구와 융합할까?':menu==='party'?'누구와 교대할까?':menu==='capture'?'어떤 봉인구를 사용할까?':menu==='run'?'도주할까?':'전투 정보';
    setScreen('battle',`<section class="monster-battle-v51 monster-battle-v52 mode-${mode}"><div class="battle-bg-v51 battle-bg-v52" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="battle-atmos-v51 battle-atmos-v52"></div><header class="battle-top-v51 battle-top-v52"><div class="battle-title-v52"><small>${esc(b.encounterType==='boss'?'BOSS':b.encounterType==='rival'?'RIFT HUNTER':mode==='double'?'DOUBLE BATTLE':'WILD BATTLE')} · ${r.floor}F</small><b>${esc(b.encounterLabel||r.biome.name)}</b></div><div class="party-mini-v51 party-mini-v52">${partyMini}</div><div class="battle-core-v52"><span class="battle-money-v53">G ${Number(run.gold||0).toLocaleString('ko-KR')}</span><span class="battle-wave-resource-v56">WAVE ${String(((Number(r.floor||1)-1)%10)+1).padStart(2,'0')}/10</span></div><div class="battle-tools-v52"><button data-action="battle-menu" data-menu="info">i</button><button data-action="toggle-combat-log">LOG</button></div></header><main class="battle-arena-v51 battle-arena-v52"><section class="player-field-v51 player-field-v52">${monActors}</section><section class="enemy-field-v51 enemy-field-v52">${enemyActors}</section></main><footer class="command-box-v51 command-box-v52 command-box-v57"><div class="battle-dialog-v52 battle-dialog-v57" role="status" aria-live="polite"><small>${esc(actionHint)}</small><b>${esc(prompt)}</b><span>${selectedMon?`${esc(selectedMon.name)} · Lv.${selectedMon.level}`:''}</span></div><div class="command-content-v51 command-content-v52">${me.ended?'<div class="turn-wait-v51"><span>…</span><b>적 턴 진행 중</b></div>':command}</div></footer><aside class="battle-log-drawer-v31 ${state.combatLogOpen?'open':''}"><div><b>LOG</b><button data-action="toggle-combat-log">×</button></div>${b.log.slice(-10).reverse().map(x=>`<p>${esc(x.text)}</p>`).join('')}</aside></section>`);
  }

  function fieldServiceHtml(r,run){
    const rw=r.reward,costs=rw?.serviceCosts;if(!costs)return'';const used=rw.servicesBy?.[state.profileId]||{},damaged=run.hp<run.maxHp||(run.monsters||[]).some(m=>m.hp>0&&m.hp<m.maxHp),ko=(run.monsters||[]).some(m=>m.hp<=0),ppNeedsService=(run.monsters||[]).some(m=>m.majorStatus||(m.moves||[]).some(mv=>Number(mv.pp??mv.maxPp??0)<Number(mv.maxPp||mv.ppMax||mv.pp||1)));
    const svc=(id,icon,name,desc,enabled=true)=>`<button data-action="field-service" data-service="${id}" ${used[id]||run.gold<Number(costs[id]||0)||!enabled?'disabled':''}><span>${icon}</span><div><b>${name}</b><small>${desc}</small></div><strong>${used[id]?'완료':`${costs[id]}G`}</strong></button>`;
    return `<div class="field-service-v51"><header><small>FIELD SUPPORT</small><b>정비소</b><span>필요할 때만 이용하세요.</span></header>${svc('mend','♥','응급 회복','CORE + 생존 몬스터 회복',damaged)}${svc('revive','✚','부활 캡슐','쓰러진 몬스터 1체 부활',ko)}${svc('charge','PP','PP · 상태 정비','모든 몬스터 PP 약 35% 회복 · 주요 상태이상 치료',ppNeedsService)}</div>`;
  }
  function showFieldServiceMenuV51(){const r=state.room,run=r?.runState?.[state.profileId];if(!r||!run)return;modal(`<div class="service-modal-v51">${fieldServiceHtml(r,run)}<button class="cta" data-action="modal-close">닫기</button></div>`);}
  function postBattlePartySummary(r,run){
    const rows=r?.reward?.expBy?.[state.profileId]||[];if(!rows.length)return'';const grade=r.reward?.gradeBy?.[state.profileId]||'';
    return `<section class="postbattle-v51"><header><div><small>WAVE CLEAR</small><b>경험치 획득</b></div>${grade?`<strong>${esc(grade)}</strong>`:''}</header><div>${rows.map(x=>{const mon=(run.monsters||[]).find(m=>m.instanceId===x.instanceId),sprite=x.sprite||monsterDisplaySprite(mon),p=Math.max(0,Math.min(100,Math.round(Number(x.xp||0)/Math.max(1,Number(x.xpNeed||1))*100)));return `<article class="${x.leveled?'leveled':''}"><img src="${esc(sprite||'')}" alt=""><div><b>${esc(mon?.name||x.name||'MONSTER')}</b><small>Lv.${x.fromLevel}${x.fromLevel!==x.toLevel?` → ${x.toLevel}`:''} · EXP +${x.xpGained}</small><i><em style="width:${p}%"></em></i></div>${x.leveled?'<strong>LEVEL UP</strong>':''}</article>`;}).join('')}</div></section>`;
  }
  ...
  function renderReward(r){
    ...
    if(skillOffer&&!skillDone){
      ...
      actions=`<div class="reward-actions-v51"><button data-action="skip-moves-all">남은 기술 건너뛰기 → 보상으로</button></div>`;
    }else if(rewriteOffer&&!rewriteDone){
      const target=(run.monsters||[]).find(m=>m.instanceId===rewriteOffer.instanceId);step='RIFT REWRITE';
      body=`<section class="rewrite-v51"><header><img src="${esc(rewriteOffer.monsterSprite||monsterDisplaySprite(target)||'')}" alt=""><div><small>BOSS CLEAR</small><b>${esc(rewriteOffer.moveName)}</b><span>기술 개조 하나를 선택하세요.</span></div></header><div>${(rewriteOffer.options||[]).map(x=>`<button data-action="rewrite-pick" data-rewrite-id="${esc(x.id)}"><span>${esc(x.icon||'✦')}</span><div><b>${esc(x.name)}</b><small>${esc(shortLine(x.desc,48))}</small></div></button>`).join('')}</div></section>`;
      actions=`<div class="reward-actions-v51"><button data-action="skip-rewrite">유지하기</button></div>`;
    }else if(rw.camp&&!campDone){
      step='CAMP';
      body=`<section class="reward-simple-v51 camp-choice-v58"><header><small>CHECKPOINT · WAVE 05</small><b>파티 정비</b><span>한 가지 행동을 고르면 바로 다음 보상 단계로 이동합니다.</span></header><div class="three-choice-v51"><button data-action="camp-rest"><span>♥</span><b>회복</b><small>생존 몬스터 HP 30% 회복</small></button><button data-action="camp-pp"><span>PP</span><b>기술 정비</b><small>전체 기술 PP 50% 회복 · 상태이상 치료</small></button><button data-action="camp-meditate"><span>◇</span><b>명상</b><small>균열 파편 +2 · 런 골드 40G</small></button></div></section>`;
      ...
      step='CHOOSE ONE';const preRewardShop=rw.shop?`<section class="pre-reward-shop-v58"><header><small>SHOP</small><b>전투 정비</b><span>보상을 고르기 전에도 필요한 회복·PP·장착 아이템을 구매할 수 있습니다.</span></header>${merchantHtmlCompact(r)}</section>`:'';body=`${preRewardShop}<section class="reward-simple-v51 free-drop-v58"><header><small>${r.mode==='journey'?'FIELD DROP':'RIFT DROP'}</small><b>무료 보상 하나를 선택하세요</b><span>보상 선택 뒤에는 다음 웨이브 준비 단계로 이어집니다.</span></header><div class="reward-options-v51 reward-count-${Math.min(6,Math.max(1,options.length))}">${options.map(rewardChoiceTileV43).join('')}</div></section>`;
      actions=`<div class="reward-actions-v51 secondary"><button data-action="reward-salvage">분해</button><button data-action="reward-reroll">재굴림 ${nextRerollCost(r)}G</button>${run.fragments>0?'<button data-action="reward-reroll-fragment">◇ 1</button>':''}</div>`;
    }else if(rw.shop&&afterCore){
      step='SHOP';body=merchantHtmlCompact(r);actions=`<div class="reward-actions-v51"><button data-action="open-purge" class="v56-hidden" hidden>덱 정제</button><button class="next-v51" data-action="reward-continue">${esc(nextEncounterPreview(r))}</button></div>`;
    }else{
      ...
      actions=`<div class="reward-actions-v51">${rw.kind==='battle'&&rw.serviceCosts?'<button data-action="open-field-service">정비</button>':''}<button class="next-v51" data-action="reward-continue">${continued?'대기 중':r.finalClearPending?'클리어':'다음 전투'}</button></div>`;
    }
    const rewardLead=step==='NEW MOVE'?'새 기술을 확인하자.':step==='TECH DISC'?'새로운 기술을 배울 기회다.':step==='RIFT REWRITE'?'기술에 새로운 변화를 부여할 수 있다.':step==='CAMP'?'잠시 숨을 고르고 파티를 정비하자.':step==='RELIC'?'희귀한 유물 하나를 가져갈 수 있다.':step==='CHOOSE ONE'?'전투에서 승리했다! 보상 하나를 고르자.':step==='SHOP'?'다음 전투를 위해 필요한 것을 준비하자.':'준비가 끝났다. 다음 조우가 기다리고 있다.';
    setScreen('reward',`<section class="run-stage-v31 reward-stage-v31 reward-v51 reward-v57 reward-v58"><div class="run-bg-v31" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="run-shade-v31"></div>${compactRunHud(r,run)}<main class="reward-center-v51 reward-center-v57"><div class="reward-card-v51 reward-card-v57"><aside class="reward-dialog-v57"><small>WAVE ${String(((Number(r.floor||1)-1)%10)+1).padStart(2,'0')} CLEAR</small><b>${esc(rewardLead)}</b><span>G ${Number(run.gold||0).toLocaleString('ko-KR')} · ${esc(r.biome?.name||'EXPEDITION')}</span><i></i></aside><section class="reward-content-v57"><div class="reward-step-v51">${esc(step)}</div>${expPanel}${body}${actions}${['READY','SHOP'].includes(step)&&!continued?`<div class="reward-auto-v54"><i></i><span>${step==='SHOP'?'상점을 사용하지 않으면 잠시 후 다음 전투로 이동합니다.':'잠시 후 자동으로 다음 전투로 이동합니다.'}</span></div>`:''}</section></div></main></section>`);
  ...
  function renderEnd(r){const clear=r.status==='cleared';setScreen('end',`<section class="event-screen"><div class="event-card"><div class="event-sigil">${clear?'♛':'☒'}</div><div class="section-label">${clear?'50F DUNGEON CLEAR':'EXPEDITION ENDED'}</div><h1>${esc(r.reward?.title||'원정 종료')}</h1><p>${esc(r.reward?.text||'')}</p><div class="stat-grid">${r.players.map(p=>{const run=r.runState[p.id];return `<div class="stat-box"><b>${esc(p.nickname)}</b><small>도달 ${r.floor}F · PARTY ${(run?.monsters||[]).length} · 아이템 ${Object.keys(run?.items||{}).length}</small></div>`;}).join('')}</div><div class="modal-actions" style="justify-content:center"><button class="cta mint" data-action="finish-run">홈으로 돌아가기</button></div></div></section>`);}
  ...
  function renderCollectionInner(){
    ...
    const grid=view.map(c=>{const count=state.profile.collection[c.id]||0,inDeck=state.deckDraft.includes(c.id);return `<article class="collection-card-v34 spell-v40 ${count?'owned':'unowned'} ${inDeck?'selected':''} ${c.type==='spell'?'spell-row-v43':''}"><div class="collection-art-v34">${collectionArtHtml(c)}</div><div class="collection-copy-v34"><b class="rarity-${c.rarity}" title="${esc(tacticDisplayNameV55(c))}">${esc(tacticDisplayNameV55(c))}</b><small>${c.cost}E · ${esc(cardRole(c).label)} · ${esc(c.element)}</small><div>${cardEffectRows(c,3).map(x=>`<span><i>${esc(x.icon)}</i>${esc(x.value||x.label)}</span>`).join('')}</div></div>${count?`<button class="deck-add-v34" data-action="${inDeck?'deck-remove':'deck-add'}" data-card-id="${esc(c.id)}">${inDeck?'제외':'편성'}</button>`:'<em>미획득</em>'}</article>`;}).join('');
    setScreen('collection',`<section class="loadout-page-v34 spellbook-v40"><header class="loadout-head-v34"><button class="back-btn" data-action="${state.prepMode?'prep-return':'home'}">← ${state.prepMode?'파티':'홈'}</button><div><small>SPELL GRIMOIRE</small><h1>공명 지령 덱</h1><p>몬스터는 파티에서 직접 싸우고, 카드는 공격·방어·강화·진화를 만드는 전술입니다.</p></div><div class="loadout-total-v34"><b>${state.deckDraft.length}</b><span>/${max}</span></div></header><div class="spell-deck-strip-v40">${state.deckDraft.map(id=>{const c=byId(state.meta.cards,id);return c?`<button data-action="deck-remove" data-card-id="${esc(id)}">${stripCardThumbHtml(c)}</button>`:'';}).join('')}</div><div class="collection-toolbar-v34"><div class="filter-row">${[['all','전체'],['gene','인자술'],['attack','공격'],['support','지원'],['common','일반'],['rare','희귀'],['ultra','초희귀'],['legendary','전설'],['mythic','신화']].map(([k,n])=>`<button class="filter-btn ${state.collectionFilter===k?'active':''}" data-action="filter" data-filter="${k}">${n}</button>`).join('')}</div><input class="search-input" id="collectionSearch" value="${esc(state.collectionSearch)}" placeholder="전술 검색"></div><div class="collection-grid-v34 spell-grid-v40">${grid}</div><div class="loadout-save-v34"><div><b>${min}~${max}종</b><small>현재 ${state.deckDraft.length}종</small></div><button class="primary" data-action="deck-save" ${state.deckDraft.length<min||state.deckDraft.length>max?'disabled':''}>공명 지령 덱 저장</button></div><div class="pagination"><button class="cta" data-action="page-prev" ${state.collectionPage===0?'disabled':''}>←</button><span>${state.collectionPage+1}/${pages}</span><button class="cta" data-action="page-next" ${state.collectionPage>=pages-1?'disabled':''}>→</button></div></section>`);
  }
  function addDeckCard(cardId){const c=byId(state.meta.cards,cardId);if(!c||c.type!=='spell'||!state.profile.collection?.[cardId])return;if(state.deckDraft.includes(cardId))return;const max=state.meta.spellRules?.max||12;if(state.deckDraft.length>=max)return toast(`공명 지령 덱은 최대 ${max}종입니다.`,'error');state.deckDraft.push(cardId);renderCollectionInner();sound('card');}
  ...
  async function resumeRoom(){if(state.resumeBusy)return;state.resumeBusy=true;const btn=$('[data-action="resume-room"]');if(btn){btn.disabled=true;btn.classList.add('loading');}try{let room=null;const candidate=state.roomId||state.profile?.activeRoomId||'';if(candidate){try{const d=await api(`/api/room/${candidate}`);room=d.room?.players?.some(p=>p.id===state.profileId)?d.room:null;}catch{room=null;}}if(!room)room=await recoverRoomSessionV532({connect:false,notify:false});if(!room){localStorage.removeItem('riftdeck.roomId');state.roomId='';state.room=null;toast('이어갈 수 있는 원정이 없습니다. 새 원정을 시작할 수 있습니다.','error');return renderHome();}state.room=room;state.roomId=room.id;if(state.profile)state.profile.activeRoomId=room.id;localStorage.setItem('riftdeck.roomId',room.id);state.lastFxSeq=Number(room.seq||0);connectStream(room.id);renderRoom();toast(`${room.floor||0}F 원정을 이어갑니다.`,'good');}catch(e){toast(`이어하기 실패 · ${e.message||'서버 연결 오류'}`,'error');}finally{state.resumeBusy=false;if(btn&&document.body.contains(btn)){btn.disabled=false;btn.classList.remove('loading');}}}
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='battle-run'){if(state.busy)return;const me=state.room?.battle?.party?.find(p=>p.playerId===state.profileId),u=me?.units?.find(x=>x.instanceId===state.selectedMonster)||me?.units?.find(x=>x.hp>0&&!x.acted)||me?.units?.[0];if(!u)return;const d=await roomPost('run-battle',{instanceId:u.instanceId});state.battleMenu='root';toast(d.result.success?'도주 성공 · 보상 없이 다음 웨이브로 이동합니다.':`도주 실패 · 성공률 ${Math.round(Number(d.result.chance||0)*100)}%`,d.result.success?'good':'error');if(d.result.success){showCenterBanner('ESCAPED','보상 없이 다음 웨이브로 이동','turn');return renderRoom();}await waitForCombatCinematicV54();return renderRoom();}
      ...
      if(action==='fusion-partner')return openFusionBuilderV56(el.dataset.primary,el.dataset.secondary);
      if(action==='fusion-toggle-move'){const d=state.fusionDraft;if(!d)return;const id=el.dataset.moveId,all=allMyBattleMonstersV56(),a=all.find(x=>x.instanceId===d.primaryId),b=all.find(x=>x.instanceId===d.secondaryId),need=Math.min(4,fusionMovePoolV56(a,b).length),set=new Set(d.moves||[]);if(set.has(id))set.delete(id);else if(set.size<need)set.add(id);else return toast(`기술은 ${need}개만 선택할 수 있습니다.`,'error');d.moves=[...set];return renderFusionBuilderV56();}
      if(action==='fusion-confirm'){const d=state.fusionDraft;if(!d)return;closeModal();const out=await roomPost('monster-fuse',{primaryId:d.primaryId,secondaryId:d.secondaryId,moveIds:d.moves||[]});state.fusionDraft=null;state.battleMenu='root';await waitForCombatCinematicV54();await loadProfile();return renderRoom();}
      if(action==='fusion-pick')return openFusionBuilderV56(el.dataset.primary,el.dataset.secondary);
      ...
      if(action==='equip-held-item'){const monsterId=el.dataset.monster,itemId=el.dataset.item||null;const d=await roomPost('equip-held',{instanceId:monsterId,itemId});toast(itemId?`${d.result.item?.name||'아이템'} 장착 완료.`:'장착 아이템을 해제했습니다.','good');return showRunInfo();}
      if(action==='open-forge')return showForgeMenu();
      if(action==='open-purge')return showPurgeMenu();
      if(action==='open-field-service'){clearTimeout(state.rewardAutoTimer);state.rewardAutoTimer=0;return showFieldServiceMenuV51();}
      if(action==='play-card'){
      ...
      if(action==='skip-moves-all'){await roomPost('skip-moves-all',{});return renderRoom();}
      ...
      if(action==='field-service'){const d=await roomPost('service',{serviceId:el.dataset.service});toast(d.result.label,'good');sound('reward');closeModal();return renderRoom();}
      if(action==='reward-continue'){clearTimeout(state.rewardAutoTimer);state.rewardAutoTimer=0;return roomPost('continue',{});}
...
// Backward-compatible UI marker: SKILL DISC
