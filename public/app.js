...
(() => {
  ...
  async function cardCastWindup(cardEl,card,targetEl){
    ...
    let target=targetEl;
    if(mode==='summon')target=$('.unit-actor-v31:not(.filled)')||$('.ally-units-v31')||$('.battle-field-v31');
  ...
  function effectUi(op,value=0){
    const m={
      ...
      nextAttack:['▲','다음 공격','buff'],buffUnits:['♟','유닛 강화','buff'],thorns:['✦','반격','buff'],halveEnemyNext:['◐','피해 반감','guard'],energyDebt:['ϟ','에너지 부채','debuff'],cleanseWeak:['✧','약화 해제','heal']
    };
  ...
  function captureThrowFx(button,card,seal='basic'){
    if(!fxOn()||!button)return Promise.resolve();const r=button.getBoundingClientRect(),root=fxRoot(),orb=document.createElement('div');orb.className=`capture-orb-flight-v34 seal-${seal}`;orb.style.left=`${r.left+r.width/2}px`;orb.style.top=`${r.top+r.height/2}px`;orb.innerHTML='<i></i><b></b>';root.appendChild(orb);const target=$('.hunt-card-v32'),t=target?rectCenter(target):{x:innerWidth/2,y:innerHeight/2};const dx=t.x-(r.left+r.width/2),dy=t.y-(r.top+r.height/2);orb.style.setProperty('--dx',`${dx}px`);orb.style.setProperty('--dy',`${dy}px`);sound('card');setTimeout(()=>{target?.classList.add('capture-lock-v34');screenShake('soft');},300);setTimeout(()=>orb.remove(),720);return wait(520);
  }
  async function captureResultFx(card,success,escaped,newDiscovery){
    if(!fxOn())return;const root=document.createElement('div');root.className=`capture-cinematic-v34 ${success?'success':'fail'} rarity-${card?.rarity||'common'}`;root.innerHTML=`<div class="capture-space-v34"><i></i><i></i><i></i><div class="capture-card-cinema-v34"><img src="${esc(card?.art||'')}" alt=""><span>${esc(elementGlyph(card?.element||''))}</span></div><div class="capture-copy-v34"><small>${success?(newDiscovery?'NEW ARCHIVE LINK':'ARCHIVE LINKED'):(escaped?'ECHO LOST':'SEAL RESISTED')}</small><b>${esc(card?.name||'CARD')}</b>${success?'<em>던전 편성 가능</em>':'<em>다음 봉인구를 준비하세요</em>'}</div></div>`;document.body.appendChild(root);sound(success?'win':'error');if(success)screenShake('medium');else screenShake('soft');await wait(success?1050:700);root.remove();
  }
  ...
  function featureChips(){return `<div class="feature-strip"><span class="feature-chip">CARD HUNT</span><span class="feature-chip">315 CARDS</span><span class="feature-chip">120 ITEMS</span><span class="feature-chip">205 MONSTERS</span><span class="feature-chip">50 FLOORS</span><span class="feature-chip">1–4 CO-OP</span></div>`;}
  ...
  function renderBattle(r){
    ...
    const enemies=alive.map((e,i)=>{const it=intentCompact(e.intent);return `<button class="enemy-actor-v31 archetype-${enemyArchetypeName(e.name)} ${state.selectedEnemy===e.uid?'selected':''} ${b.tier==='boss'?'boss':''} ${e.enraged?'phase2':''} intent-${esc(e.intent?.type||'attack')} ${e.block?'has-shield-v33':''}" data-action="select-enemy" data-enemy="${e.uid}" title="${esc(e.intent?.text||'')}"><div class="enemy-intent-v31"><span>${esc(it.icon)}</span>${it.value?`<b>${esc(it.value)}</b>`:''}</div><div class="enemy-sprite-stage-v31"><img class="enemy-sprite" src="${esc(e.sprite)}" alt=""><i class="enemy-shadow-v31"></i><i class="target-ring-v31"></i>${e.block?`<i class="shield-shell-v33 enemy-shield-v33"><b>⬢ ${e.block}</b></i>`:''}</div><div class="enemy-name-v31"><b>${esc(e.name)}</b>${e.enraged?'<em>II</em>':''}${matchupBadge(e.element)}</div><div class="enemy-hpbar-v31"><i style="width:${pct(e.hp,e.maxHp)}%"></i></div><div class="enemy-sub-v31"><span>${e.hp}/${e.maxHp}</span>${e.block?`<span class="enemy-block-number-v34">⬢ ${e.block}</span>`:''}${enemyWeakBadge(e.element)}<span class="break-dot-v31 ${e.broken?'broken':''}"><i style="width:${pct(e.stagger,e.staggerMax)}%"></i></span></div></button>`;}).join('');
    const units=[0,1,2].map(i=>{const u=me.units[i];return u?`<div class="unit-actor-v31 filled element-${elementClass(u.element||byId(state.meta.cards,u.cardId)?.element)}" data-card-id="${esc(u.cardId)}"><button class="unit-recall-v34" data-action="recall-unit" data-instance-id="${esc(u.instanceId)}" title="전열 회수 · 턴당 1회">↩</button><div class="unit-sprite-v31"><img src="${esc(byId(state.meta.cards,u.cardId)?.art||'')}" alt=""></div><b>${esc(shortLine(u.name,12))}</b><span>${esc(elementGlyph(u.element||''))} ATK ${u.power}</span></div>`:`<div class="unit-actor-v31 empty"><i>+</i><small>UNIT</small></div>`;}).join('');
  ...
  function deckCounts(){const unit=state.deckDraft.filter(id=>byId(state.meta.cards,id)?.type==='unit').length,spell=state.deckDraft.filter(id=>byId(state.meta.cards,id)?.type==='spell').length;return{unit,spell,total:unit+spell};}
  function loadoutSlotHtml(id,index,type){const c=id?byId(state.meta.cards,id):null;if(!c)return `<div class="loadout-slot-v34 empty ${type}"><span>${index+1}</span><i>${type==='unit'?'♟':'✦'}</i></div>`;return `<div class="loadout-slot-v34 ${type} rarity-${c.rarity}" title="${esc(c.text)}"><img src="${esc(c.art)}" alt=""><div><b>${esc(shortLine(c.name,11))}</b><small>${esc(elementGlyph(c.element))} ${esc(c.element)} · ${c.cost}E</small></div><button data-action="deck-remove" data-card-id="${esc(c.id)}">×</button></div>`;}
  function renderCollectionInner(){
    const q=state.collectionSearch.trim().toLowerCase();let cards=state.meta.cards.filter(c=>(state.collectionFilter==='all'||state.collectionFilter===c.rarity||state.collectionFilter===c.type)&&(!q||c.name.toLowerCase().includes(q)||c.element.toLowerCase().includes(q)||c.text.toLowerCase().includes(q)));const pageSize=36,pages=Math.max(1,Math.ceil(cards.length/pageSize));state.collectionPage=Math.max(0,Math.min(state.collectionPage,pages-1));const view=cards.slice(state.collectionPage*pageSize,(state.collectionPage+1)*pageSize),counts=deckCounts(),unitIds=state.deckDraft.filter(id=>byId(state.meta.cards,id)?.type==='unit'),spellIds=state.deckDraft.filter(id=>byId(state.meta.cards,id)?.type==='spell');
    const collection=view.map(c=>{const count=state.profile.collection[c.id]||0,inDeck=state.deckDraft.includes(c.id);return `<article class="collection-card-v34 ${count?'owned':'unowned'} ${inDeck?'selected':''}" data-card-id="${esc(c.id)}"><div class="collection-art-v34"><img src="${esc(c.art)}" loading="lazy" alt=""><span class="element-${elementClass(c.element)}">${esc(elementGlyph(c.element))}</span></div><div class="collection-copy-v34"><b class="rarity-${c.rarity}">${esc(c.name)}</b><small>${c.type==='unit'?'UNIT':'SPELL'} · ${esc(c.element)} · ${c.cost}E</small><div>${cardEffectRows(c,2).map(x=>`<span><i>${esc(x.icon)}</i>${esc(x.value||x.label)}</span>`).join('')}</div></div>${count?`<button class="deck-add-v34" data-action="${inDeck?'deck-remove':'deck-add'}" data-card-id="${esc(c.id)}">${inDeck?'제외':'편성'}</button>`:`<em>여행에서 획득</em>`}</article>`;}).join('');
    setScreen('collection',`<section class="loadout-page-v34"><header class="loadout-head-v34"><button class="back-btn" data-action="home">← 홈</button><div><small>EXPEDITION LOADOUT</small><h1>원정 덱 편성</h1><p>여행에서 봉인한 카드와 복각 카드만 사용. 유닛 코어 + 스펠 북을 조합해 던전에 가져갑니다.</p></div><div class="loadout-total-v34"><b>${counts.total}</b><span>/16</span></div></header><main class="loadout-layout-v34"><aside class="party-loadout-v34"><div class="loadout-group-v34"><header><b>UNIT PARTY</b><span>${counts.unit}/6</span></header><div class="unit-slots-v34">${Array.from({length:6},(_,i)=>loadoutSlotHtml(unitIds[i],i,'unit')).join('')}</div></div><div class="loadout-group-v34"><header><b>SPELL BOOK</b><span>${counts.spell}/10</span></header><div class="spell-slots-v34">${Array.from({length:10},(_,i)=>loadoutSlotHtml(spellIds[i],i,'spell')).join('')}</div></div><div class="loadout-save-v34"><div><b>최소 8종</b><small>유닛 최대 6 · 스펠 최대 10</small></div><button data-action="deck-reset">되돌리기</button><button class="primary" data-action="deck-save" ${counts.total<8?'disabled':''}>이 덱으로 출발</button></div></aside><section class="owned-library-v34"><div class="collection-toolbar-v34"><div class="filter-row">${[['all','전체'],['unit','유닛'],['spell','스펠'],['common','일반'],['rare','희귀'],['ultra','초희귀'],['legendary','전설'],['mythic','신화']].map(([k,n])=>`<button class="filter-btn ${state.collectionFilter===k?'active':''}" data-action="filter" data-filter="${k}">${n}</button>`).join('')}</div><input class="search-input" id="collectionSearch" value="${esc(state.collectionSearch)}" placeholder="카드 검색"></div><div class="collection-grid-v34">${collection}</div><div class="pagination"><button class="cta" data-action="page-prev" ${state.collectionPage===0?'disabled':''}>←</button><span>${state.collectionPage+1}/${pages} · 보유 ${state.profile.ownedCount}/${state.profile.totalCards}</span><button class="cta" data-action="page-next" ${state.collectionPage>=pages-1?'disabled':''}>→</button></div></section></main></section>`);
  }
  function addDeckCard(cardId){const c=byId(state.meta.cards,cardId);if(!c||!state.profile.collection?.[cardId])return;if(state.deckDraft.includes(cardId))return;const counts=deckCounts();if(counts.total>=16)return toast('원정 덱은 최대 16종입니다.','error');if(c.type==='unit'&&counts.unit>=6)return toast('유닛 코어는 최대 6종입니다.','error');if(c.type==='spell'&&counts.spell>=10)return toast('스펠 북은 최대 10종입니다.','error');state.deckDraft.push(cardId);renderCollectionInner();sound('card');}
  ...
  async function saveDeck(){const counts=deckCounts();if(counts.total<8)return toast('원정 덱은 최소 8종이 필요합니다.','error');try{const d=await api('/api/deck',{body:{profileId:state.profileId,nickname:state.profile.nickname,deck:state.deckDraft}});state.profile=d.profile;state.deckDraft=[...d.profile.deck];updateTopbar();toast('원정 덱 저장 완료. 던전에 바로 사용할 수 있습니다.','good');sound('win');renderCollectionInner();}catch(e){toast(e.message,'error');}}
