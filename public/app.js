...
(() => {
  ...
  function compactRunHud(r,run){
    const hp=Math.max(0,Number(run?.hp||0)),max=Math.max(1,Number(run?.maxHp||1));
    return `<div class="run-hud-v27"><div class="run-floor-v27"><strong>${r.floor}</strong><span>F</span><div><b>${esc(r.biome.name)}</b><small>${r.mode==='dungeon'?difficultyKo(r.difficulty):'JOURNEY'}</small></div></div><div class="run-hp-v27"><span>HP ${hp}/${max}</span><i><em style="width:${pct(hp,max)}%"></em></i></div><div class="run-pills-v27"><span>G ${run.gold||0}</span><span>◇ ${run.fragments||0}</span><span>✦ ${(run.relics||[]).length}</span><button data-action="run-info">RUN</button></div></div>`;
  }
  ...
  function renderRoute(r){
    ...
    const content=!contractReady?`<div class="decision-card-v27"><div class="decision-title-v27"><small>RUN CONTRACT</small><h1>이번 원정의 방향을 고르세요.</h1><p>한 번만 고르면 됩니다. 이후에는 경로와 전투에 집중합니다.</p></div><div class="contract-grid-v27">${offers.map(c=>`<button data-action="contract-pick" data-contract-id="${esc(c.id)}"><span>${esc(c.icon||'◇')}</span><b>${esc(c.name)}</b><p>${esc(c.desc)}</p><small>${esc(c.detail)}</small></button>`).join('')}</div></div>`:`<div class="decision-card-v27"><div class="decision-title-v27"><small>PATH SELECT · PRESSURE ×${pressure}</small><h1>다음 길을 선택하세요.</h1><p>${esc(r.biome.desc)}</p></div><div class="path-grid-v27">${r.route.map((n,i)=>`<button class="path-choice-v27 kind-${esc(n.kind)} ${voted===n.id?'selected':''}" data-action="route-vote" data-node="${esc(n.id)}"><span class="path-icon-v27">${esc(n.icon)}</span><div><small>0${i+1}</small><b>${esc(n.label)}</b><p>${esc(n.desc)}</p></div><em>${n.votes.length}/${r.players.length}</em></button>`).join('')}</div><div class="decision-foot-v27">${voted?'선택 완료 · 다른 원정대원을 기다리는 중':'세 경로 중 하나를 선택하세요.'}</div></div>`;
  ...
  function renderBattle(r){
    ...
    setScreen('battle',`<section class="battle-screen battle-stage-v27"><div class="battle-backdrop" style="background-image:url('${esc(r.biome.background)}')"></div><div class="battle-vignette-v27"></div><header class="battle-hud-v27"><div class="battle-party-v27">${party}</div><div class="battle-label-v27"><small>${b.tier==='boss'?'BOSS':b.tier==='elite'?'ELITE':'ENCOUNTER'} · ${esc(r.biome.short)}</small><b>${r.floor}F · TURN ${b.turn}</b></div><div class="battle-tools-v27"><button data-action="run-info">RUN</button><button data-action="toggle-combat-log">LOG</button></div></header><main class="battle-field-v27"><div class="ally-zone-v27"><div class="player-nameplate-v27"><b>${esc(me.nickname)}</b><span>HP ${me.hp}/${me.maxHp} · BLOCK ${me.block}</span><i><em style="width:${pct(me.hp,me.maxHp)}%"></em></i></div><div class="unit-board">${units}</div></div><div class="enemy-zone-v27">${enemies}</div></main><section class="battle-dock-v27"><div class="energy-core-v27"><small>ENERGY</small><b>${me.energy}</b><span>/ ${me.maxEnergy}</span><div class="chain-mini-v27"><em>CHAIN ×${chain.count||0}</em>${[1,2,3,4,5].map(n=>`<i class="${n<=chain.count?'on':''}"></i>`).join('')}</div></div><div class="hand-wrap hand-dock-v26 hand-dock-v27"><div class="hand">${hand}</div><div class="hand-hint">숫자키 1~9 · 우클릭/길게 눌러 상세</div></div><div class="turn-box-v27"><div class="selected-intent-v27"><small>NEXT</small><b>${alive.length?alive.find(e=>e.uid===state.selectedEnemy)?.intent?.icon||alive[0].intent.icon:''} ${alive.length?esc(alive.find(e=>e.uid===state.selectedEnemy)?.intent?.text||alive[0].intent.text):'CLEAR'}</b></div>${me.ended?'<div class="waiting">동료 대기 중...</div>':`<button class="end-turn-v27" data-action="end-turn"><small>END TURN</small><b>E</b></button>`}</div></section><aside class="battle-log-drawer-v26 battle-log-drawer-v27 ${state.combatLogOpen?'open':''}"><div><b>COMBAT LOG</b><button data-action="toggle-combat-log">닫기</button></div>${b.log.slice(-18).reverse().map(x=>`<p>${esc(x.text)}</p>`).join('')}</aside></section>`);
  }
  ...
  function renderReward(r){
    ...
    if(rw.camp&&!campDone){
      ...
      step='CARD REWARD';title=rw.title||'보상 선택';sub='필요한 카드/아이템 하나만 고르세요. 필요 없으면 분해해도 됩니다.';
      body=`<div class="reward-options-v27">${options.map(o=>o.type==='card'?cardHtml(o.card,{action:'reward-pick',rewardId:o.id}):itemHtml(o.item,{action:'reward-pick',rewardId:o.id})).join('')}</div>`;
