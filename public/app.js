...
(() => {
  ...
  function renderExpeditionPrep(mode){
    state.prepMode=mode;state.monsterDraft=[...(state.profile.monsterParty||[])];state.deckDraft=[...(state.profile.deck||[])];state.current='prep';
    const rules=state.meta.monsterRules||{maxSlots:6,pointBudget:10},cost=monsterDraftCost();const owned=state.meta.monsters.filter(m=>Number(state.profile.monsters?.[m.id]||0)>0).sort((a,b)=>Number(a.pointCost)-Number(b.pointCost)||a.name.localeCompare(b.name,'ko'));
  ...
  function renderBattle(r){
    ...
    const enemies=alive.map(e=>{const it=intentCompact(e.intent);return `<button class="enemy-actor-v31 v40 archetype-${enemyArchetypeName(e.name)} ${state.selectedEnemy===e.uid?'selected':''} ${b.tier==='boss'?'boss':''}" data-action="select-enemy" data-enemy="${e.uid}"><div class="enemy-intent-v31"><span>${esc(it.icon)}</span>${it.value?`<b>${esc(it.value)}</b>`:''}</div><div class="enemy-sprite-stage-v31"><img class="enemy-sprite" src="${esc(e.sprite)}" alt=""><i class="enemy-shadow-v31"></i><i class="target-ring-v31"></i>${e.block?`<i class="shield-shell-v33 enemy-shield-v33"><b>⬢ ${e.block}</b></i>`:''}</div><div class="enemy-name-v31"><b>${esc(e.name)}</b>${matchupBadge(e.element)}</div><div class="enemy-hpbar-v31"><i style="width:${pct(e.hp,e.maxHp)}%"></i></div><div class="enemy-sub-v31"><span>${e.hp}/${e.maxHp}</span>${e.block?`<span class="enemy-block-number-v34">⬢ ${e.block}</span>`:''}${enemyWeakBadge(e.element)}<span class="break-dot-v31 ${e.broken?'broken':''}"><i style="width:${pct(e.stagger,e.staggerMax)}%"></i></span></div></button>`;}).join('');
    const monsters=me.units.map(u=>`<button class="monster-actor-v40 filled archetype-${esc(u.archetype||'construct')} element-${elementClass(u.element)} form-${monsterFormName(u).toLowerCase().replace(/\s+/g,'-')} ${state.selectedMonster===u.instanceId?'selected':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}"><div class="monster-sprite-v40">${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt=""><i></i></div><div class="monster-head-v40"><b>${esc(shortLine(u.name,16))}</b><span>Lv.${u.level} · ATK ${u.power}</span></div><div class="monster-form-chip-v40">${esc(monsterFormName(u))}</div><div class="monster-hp-v40"><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i><b>${u.hp}/${u.maxHp}</b>${u.block?`<strong>⬢ ${u.block}</strong>`:''}</div><div class="monster-gauges-v40">${monsterGauge('⌬',u.gene||0,8,'gene')}${monsterGauge('✦',u.resonance||0,6,'res')}${monsterGauge('◆',u.rift||0,5,'rift')}</div></button>`).join('');
    const bench=(me.bench||[]).map(u=>`<button data-action="select-bench" data-monster="${esc(u.instanceId)}" title="${esc(u.name)}"><img src="${esc(monsterDisplaySprite(u))}"><span>${esc(shortLine(u.name,9))}</span></button>`).join('');
    const controls=selectedMon?`<div class="evolve-controls-v40"><button data-action="monster-evolve" data-mode="evolve" data-monster="${esc(selectedMon.instanceId)}" ${selectedMon.evolved||selectedMon.level<3||selectedMon.gene<3?'disabled':''} title="Lv.3 + GENE 3">▲<small>진화</small></button><button data-action="open-fusion" data-monster="${esc(selectedMon.instanceId)}" ${selectedMon.gene<2||!(me.units.length+me.bench.length>1)?'disabled':''} title="두 몬스터 GENE 2">∞<small>융합</small></button><button data-action="monster-evolve" data-mode="resonance" data-monster="${esc(selectedMon.instanceId)}" ${selectedMon.resonanceTurns>0||selectedMon.resonance<4?'disabled':''} title="RESONANCE 4 · 3턴 초월">✦<small>공명</small></button><button data-action="monster-evolve" data-mode="rift" data-monster="${esc(selectedMon.instanceId)}" ${selectedMon.abyssBloom||selectedMon.rift<3||selectedMon.hp/selectedMon.maxHp>.6?'disabled':''} title="HP 60%↓ + RIFT 3 · 위험 초월">◆<small>개화</small></button>${(me.bench||[]).length?`<button data-action="open-switch" data-monster="${esc(selectedMon.instanceId)}" ${me.switchesUsed>=1?'disabled':''}>↔<small>교대</small></button>`:''}</div>`:'';
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='journey-create')return renderExpeditionPrep('journey');
      if(action==='dungeon-lobby')return renderExpeditionPrep('dungeon');
      ...
      if(action==='play-card'){
        ...
        const allyTarget=card.cardClass==='gene'?$(`[data-monster="${CSS.escape(state.selectedMonster||'')}"]`):null;await cardCastWindup(el,card,allyTarget||target);const d=await roomPost('play',{handIndex:index,targetUid:state.selectedEnemy,targetMonsterId:state.selectedMonster});if(d.room?.status==='reward')sound('win');return;
      }
