...
(() => {
  ...
  function featureChips(){return `<div class="feature-strip"><span class="feature-chip">315 CARDS</span><span class="feature-chip">22 RELICS</span><span class="feature-chip">50 FLOORS</span><span class="feature-chip">RUN CONTRACTS</span><span class="feature-chip">CARD UPGRADE</span><span class="feature-chip">REWARD SALVAGE</span><span class="feature-chip">ABYSS PRESSURE</span><span class="feature-chip">1–4 CO-OP</span></div>`;}
  function renderAuth(){
  ...
  function renderRoute(r){const mine=r.runState[state.profileId];const voted=r.route?.find(n=>n.votes.includes(state.profileId))?.id;const contractReady=!!r.contractClaims?.[state.profileId];setScreen('route',`${expeditionHeader(r)}<section class="route-page route-page-upgraded">${partyStrip(r)}${contractPanel(r)}${threatHtml(r)}${briefingHtml(r)}<div class="route-main-grid"><div><div class="route-nodes">${r.route.map(n=>`<button class="route-node ${voted===n.id?'voted':''}" data-action="route-vote" data-node="${esc(n.id)}" ${contractReady?'':'disabled'}><span class="vote-count">${n.votes.length}/${r.players.length}</span><div class="node-icon"><span>${esc(n.icon)}</span></div><b>${esc(n.label)}</b><p>${esc(n.desc)}</p><small>${n.kind==='elite'?'위험도 높음 · 유물 선택권':n.kind==='rest'?'회복 vs 강화 vs 명상':n.kind==='merchant'?'구매 + 덱 정제':n.kind==='treasure'?'골드 + 유물':n.kind==='event'?'선택형 리스크/리턴':'안정적인 빌드 성장'}</small></button>`).join('')}</div><p class="small-note route-note">${contractReady?'전원이 투표하면 가장 많은 표를 받은 경로로 이동합니다.':'먼저 위의 원정 서약을 선택해야 경로에 투표할 수 있습니다.'}</p></div><aside class="route-side-stack">${runItemRack(mine)}${feedPanel(r,6)}</aside></div></section>`);}
  function renderBattle(r){
    ...
    const nextType=chain.lastType?(chain.lastType==='unit'?'SPELL':'UNIT'):'UNIT / SPELL';
    setScreen('battle',`<section class="battle-screen battle-screen-upgraded"><div class="battle-backdrop" style="background-image:url('${esc(r.biome.background)}')"></div><div class="battle-vignette"></div><div class="battle-hud"><div class="battle-top">${b.party.map(pc=>`<div class="combatant-chip ${pc.playerId===state.profileId?'me':''}"><b>${esc(pc.nickname)}${pc.down?' · DOWN':''}</b><small>T${b.turn}${pc.ended?' · READY':''}</small><div class="hpbar"><i style="width:${pct(pc.hp,pc.maxHp)}%"></i></div><small>HP ${pc.hp}/${pc.maxHp} · <span class="energy-mini">⚡${pc.energy}/${pc.maxEnergy}</span> · 유닛 ${pc.units.length}/3${pc.chain?.count?` · CHAIN ×${pc.chain.count}`:''}</small></div>`).join('')}</div><div class="enemy-zone">${alive.map(e=>`<button class="enemy-card ${state.selectedEnemy===e.uid?'selected':''} ${b.tier==='boss'?'boss':''} ${e.enraged?'phase2':''}" data-action="select-enemy" data-enemy="${e.uid}">${state.selectedEnemy===e.uid?'<span class="target-lock">TARGET</span>':''}<div class="enemy-intent"><span>${esc(e.intent.icon)}</span><b>${esc(e.intent.text)}</b>${e.enraged?'<em>PHASE II</em>':''}</div><img class="enemy-sprite" src="${esc(e.sprite)}" alt=""><div class="enemy-name"><b>${esc(e.name)}</b>${e.enraged?'<span class="phase-badge">ENRAGED</span>':''}<div class="enemy-hp"><i style="width:${pct(e.hp,e.maxHp)}%"></i></div><div class="enemy-stats">HP ${e.hp}/${e.maxHp}${e.block?` · BLOCK ${e.block}`:''}${e.debuffs?.vulnerable?` · 취약 ${e.debuffs.vulnerable}`:''}${e.debuffs?.weak?` · 약화 ${e.debuffs.weak}`:''}${e.debuffs?.burn?` · 화상 ${e.debuffs.burn}`:''}</div></div></button>`).join('')}<div class="unit-board">${[0,1,2].map(i=>{const u=me.units[i];return u?`<div class="unit-slot filled"><img src="${esc(byId(state.meta.cards,u.cardId)?.art||'')}" alt=""><b>${esc(u.name)} · ATK ${u.power}</b></div>`:'<div class="unit-slot"><span class="small-note">UNIT SLOT</span></div>';}).join('')}</div></div><div class="battle-bottom"><div class="player-panel"><h3>${esc(me.nickname)}</h3><div class="big-hp">HP <b>${me.hp}/${me.maxHp}</b> · BLOCK ${me.block}</div><div class="hpbar"><i style="width:${pct(me.hp,me.maxHp)}%"></i></div><div class="energy">⚡ ${me.energy} / ${me.maxEnergy}</div><div class="chain-meter ${chain.count>=3?'hot':''}"><div class="chain-head"><b>TACTICAL CHAIN</b><span>×${chain.count||0}</span></div><div class="chain-pips">${[1,2,3,4,5].map(n=>`<i class="${n<=chain.count?'on':''} ${n===3?'flow':''} ${n===5?'over':''}"></i>`).join('')}</div><small>다음 권장: ${nextType} · 3연쇄 드로우+방어 · 5연쇄 OVERDRIVE</small></div><div class="pile-stats"><span>DRAW<br>${me.drawPile.length}</span><span>HAND<br>${me.hand.length}</span><span>DISCARD<br>${me.discard.length}</span></div><div class="battle-economy">G ${run.gold} · ◇${run.fragments||0} · RELIC ${(run.relics||[]).length}</div></div><div class="hand-wrap"><div class="hand">${me.hand.map((cid,i)=>{const c=runCardView(byId(state.meta.cards,cid),run);const can=!me.ended&&!me.down&&b.phase==='players'&&me.energy>=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(c.type==='spell'&&me.buffs.spellDiscount>0?1:0)))&&!(c.type==='unit'&&me.units.length>=3);return cardHtml(c,{action:'play-card',index:i,disabled:!can});}).join('')}</div><div class="hand-hint">강화 카드에는 +/++ 표시 · 우클릭/길게 누르기 = 상세 보기 · 숫자키 1~9 = 카드 사용</div></div><div class="battle-actions"><div class="objective-card"><div class="section-label">TACTICAL OBJECTIVE</div><b>${b.tier==='boss'?'BOSS':b.tier==='elite'?'ELITE':'COMBAT'} · ${r.floor}F</b><p>${b.tier==='boss'?'보스는 HP 50% 이하에서 PHASE II로 전환됩니다. 체인을 무리하게 늘리기보다 적 의도에 맞춰 끊을 타이밍도 계산하세요.':'노피격과 빠른 턴 클리어는 전투 등급과 추가 보상으로 연결됩니다.'}</p><small>심연 압력 ×${(r.threatTokens||[]).length} · 정예/보스 승리 시 유물 선택권</small></div><div class="section-label">ENEMY INTENT</div><div class="intent-help">적의 다음 행동을 먼저 읽으세요. UNIT↔SPELL을 교차 사용하면 TACTICAL CHAIN이 오르며 3/5연쇄에서 강력한 보너스가 발동합니다.</div><div class="combat-log">${b.log.slice(-10).map(x=>`<p>${esc(x.text)}</p>`).join('')}</div>${me.ended?'<div class="waiting">다른 원정대원의 턴 종료를 기다리는 중...</div>':`<button class="cta end-turn" data-action="end-turn">턴 종료 · E</button>`}</div></div></div></section>`);
  }
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='contract-pick')return roomPost('contract',{contractId:el.dataset.contractId});
