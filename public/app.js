...
(() => {
  ...
  function showCardInspect(cardId){
    const card=byId(state.meta?.cards,cardId);if(!card)return;
    ...
    modal(`<div class="card-inspect"><div class="section-label">CARD ANALYSIS</div><div class="card-inspect-grid"><div class="inspect-card-shell">${cardHtml(card)}</div><div class="inspect-copy"><h2 class="rarity-${card.rarity}">${esc(card.name)}</h2><div class="inspect-tags"><span>${esc(rarityKo(card.rarity))}</span><span>${card.type==='unit'?'UNIT':'SPELL'}</span><span>${esc(card.element)}</span><span>ENERGY ${card.cost}</span></div><p>${esc(card.text)}</p>${stats?`<strong>${esc(stats)}</strong>`:''}<small>전투 중에는 카드 번호키 1~9로도 사용할 수 있습니다. 적을 먼저 선택하면 해당 대상을 우선 공격합니다.</small></div></div><div class="modal-actions"><button class="cta mint" data-action="modal-close">닫기 · ESC</button></div></div>`);
  }
  ...
  function featureChips(){return `<div class="feature-strip"><span class="feature-chip">315 CARDS</span><span class="feature-chip">50 FLOORS</span><span class="feature-chip">1–4 CO-OP</span><span class="feature-chip">3 DIFFICULTIES</span><span class="feature-chip">CLOUD ACCOUNT</span><span class="feature-chip">TACTICAL CHAIN</span><span class="feature-chip">BOSS PHASE II</span><span class="feature-chip">IMPACT VFX</span></div>`;}
  function renderAuth(){
