(()=>{
const KX_COMPANY_BUILD='6.5.1-DEFENSE-GRADE-ROSTER';
...
let state={
  ...
  companyAvailable:true,companyMode:'REMOTE',companyRpcMode:'AUTO',companyError:'',companyRegion:'국내',companyNotice:'',companySection:'dashboard',companyAnalysisId:null,companyAnalysis:null,companyMetric:'valuation',companySearch:'',companyMediaRegion:'ALL',companyMediaTone:'PROMOTE',companyMediaTargetId:null,companyMediaSearch:'',companyOpsTab:'products',companyKickoff:null,companyKickoffLast:null,companyTalentGradeFilter:'ALL',
...
async function companyDefenseV640(action,budget){
  return rpc('kx_company_defense_v640',{p_action:String(action||'').toUpperCase(),p_budget:Number(budget||0)},true);
}
...
function localTick(d){
  ...
  if(c){
    ...
    if(!d.control_case&&d.world.cycle_no>3&&Math.random()<.07){const candidates=d.companies.filter(x=>x.is_bot&&localRivalryReason(d,x));if(candidates.length){const a=candidates[Math.floor(Math.random()*candidates.length)],reason=localRivalryReason(d,a),stake=3+Math.random()*5;d.control_case={id:Date.now(),status:'ACTIVE',stage:'WATCH',started_cycle:d.world.cycle_no,deadline_cycle:d.world.cycle_no+12,cycles_left:12,attacker_company_id:a.id,attacker_name:a.name,attacker_ticker:a.ticker,attacker_country:a.home_country,attacker_type:'BOT',attacker_style:a.ai_style,trigger_reason:reason,stake,counter_stake:0,used_rights_issue:false,used_poison_pill:false};localEvent(d,'TAKEOVER','경쟁사 지분 매집 포착',`${a.name}이 ${reason}을 계기로 우리 회사 지분 매집을 시작했습니다.`)}}
...
function companyStakeAgainstMe(){
  ...
  const incoming=[...(state.company?.incoming_holdings||[])].filter(h=>Number(h?.stake||0)>0);
  // A live server takeover case must always remain actionable. Filtering it out made the UI show
  // a high risk number while hiding every defense control.
  if(live&&String(live.status||'ACTIVE').toUpperCase()!=='DEFENDED'){
    ...
    return Math.max(0,Number(live.aggregate_stake||0),Number(live.stake||0),Number(holder?.stake||0),direct);
  }
  // Without a server takeover case, only holdings with an actual rivalry trigger are treated as
  // hostile. Ordinary outside shareholders are not shown as a takeover attack.
  const hostile=incoming.filter(h=>companyRivalryReason(Number(h?.holder_company_id||0)));
  return hostile.reduce((sum,h)=>sum+Math.max(0,Number(h?.stake||0)),0);
}
...
function activeTakeoverThreat(){
  ...
  if(live&&String(live.status||'ACTIVE').toUpperCase()!=='DEFENDED'){
    ...
    const liveStake=Math.max(Number(live.stake||0),Number(holder?.stake||0));
    const aggregate=Math.max(Number(live.aggregate_stake||0),liveStake);
    return {...live,synthetic:false,rivalry_reason:rivalry?.label||'서버에서 감지된 진행 중 경영권 공격',stake:liveStake,aggregate_stake:aggregate,counter_stake:Math.max(Number(live.counter_stake||0),Number(actualCounter?.stake||0))};
  }
  const qualified=incoming.map(h=>({h,reason:companyRivalryReason(Number(h?.holder_company_id||0))})).filter(x=>x.reason);
  if(!qualified.length)return null;
  const top=qualified[0].h,reason=qualified[0].reason;
  const aggregate=qualified.reduce((sum,x)=>sum+Math.max(0,Number(x.h?.stake||0)),0);
  const attackerId=Number(top?.holder_company_id||0),counter=myHoldings.find(h=>Number(h.target_company_id)===attackerId);
  return {synthetic:true,attacker_company_id:attackerId||null,attacker_name:top?.holder_name||'최대 외부주주',attacker_ticker:top?.holder_ticker||'',attacker_country:top?.holder_country||top?.country||'',attacker_type:top?.holder_type||'외부 주주',rivalry_reason:reason?.label||'직접 경쟁 관계',stake:Math.max(0,Number(top?.stake||0)),aggregate_stake:aggregate,stage:localTakeoverStage(Math.max(0,Number(top?.stake||0))),cycles_left:null,counter_stake:Number(counter?.stake||0),used_poison_pill:false,used_rights_issue:false};
}
  ...
  try{
    ...
    try{
      ...
      try{
        ...
        if(talent?.ok){state.company.recruit_pool=Array.isArray(talent.candidates)?talent.candidates:[];state.company.talents=Array.isArray(talent.my_talents)?talent.my_talents:[];state.company.poach_targets=Array.isArray(talent.poach_targets)?talent.poach_targets:[];state.company.talent_offers=Array.isArray(talent.inbound_offers)?talent.inbound_offers:[];state.company.talent_summary=talent.summary||{};state.company.talent_day=Number(talent.day_no||1);state.company.talent_hired_today=Number(talent.hired_today||0);state.company.talent_daily_limit=Number(talent.daily_limit||5);state.company.talent_available=true;state.company.talent_error='';}
...
function renderTakeoverCrisis(my){
  ...
  const budgetDefault=Math.max(100000000,Math.round(marketCap*.01/10000000)*10000000);
  ...
  const actions=[
    ...
    ['NEGOTIATE','지분 매각 협상','프리미엄 협상','지분 직접 감소',.82],
  ...
  return `<section id="takeoverDefenseCenter" class="takeover-crisis compact-defense">
    ...
    <div class="takeover-defense-grid">${actions.map(a=>{let preview='';if(a[0]==='BUYBACK'||a[0]==='NEGOTIATE'){const d=Math.min(stake,budgetDefault/marketCap*100*a[4]);preview=`기본예산 기준 약 ${d.toFixed(2)}%p 감소`;}else if(a[0]==='COUNTER_TAKEOVER'){preview=`기본예산 기준 상대 지분 약 ${(budgetDefault/attackerCap*100).toFixed(2)}% 확보`;}else if(a[0]==='RIGHTS_ISSUE'){preview='조달 규모에 따라 지분 희석';}else if(a[0]==='WHITE_KNIGHT'){preview='예산이 클수록 방어력 증가';}else preview='추가매입 비용·효율 악화';return `<button type="button" class="takeover-defense-btn" data-company-defense="${a[0]}"><small>${a[2]}</small><b>${a[1]}</b><span>${a[3]}</span><em>${preview}</em></button>`}).join('')}</div>
  </section>`;
}
...
async function executeTakeoverDefense(action,triggerEl=null){
  ...
  const labels={BUYBACK:'자사주 매입',NEGOTIATE:'지분 매각 협상',WHITE_KNIGHT:'백기사 확보',POISON_PILL:'포이즌필',RIGHTS_ISSUE:'긴급 유상증자',COUNTER_TAKEOVER:'역인수·맞지분'};
  ...
  if(normalized!=='RIGHTS_ISSUE'&&amount>cash){state.companyNotice=`법인현금 부족 · 보유 ${formatKrwSmart(cash)} / 필요 ${formatKrwSmart(amount)}`;renderTerminal(true);return;}
  const beforeStake=Number(control.stake||0),beforeCounter=Number(control.counter_stake||0);
  if(!confirm(`${labels[normalized]} 실행\n\n예산 ${formatKrwSmart(amount)}\n공격자 지분 ${beforeStake.toFixed(2)}%`))return;
  ...
  try{
    ...
    const after=Number(d.attacker_stake_after??beforeStake),counterAfter=Number(d.counter_stake_after??beforeCounter);
    state.companyNotice=`${d.message||labels[normalized]} · 공격자 ${beforeStake.toFixed(2)}% → ${after.toFixed(2)}%${normalized==='COUNTER_TAKEOVER'?` · 맞지분 ${beforeCounter.toFixed(2)}% → ${counterAfter.toFixed(2)}%`:''}`;
...
function renderEmployeeQuickRoster(my){
  ...
  return `<section class="employee-roster-quick grade-roster"><div class="employee-roster-head"><div><small>EMPLOYEE ROSTER</small><h3>재직 핵심인재</h3><p>등급별로 인재를 골라 보고, 같은 화면에서 연수·해고까지 처리합니다. 아래에 별도의 중복 재직 카드 목록은 표시하지 않습니다.</p></div><span>${rows.length}/${talents.length}명</span></div><div class="employee-grade-tabs"><button type="button" data-talent-grade-filter="ALL" class="${filter==='ALL'?'on':''}">전체 <b>${talents.length}</b></button>${grades.map(g=>`<button type="button" data-talent-grade-filter="${escapeHtml(g)}" class="${filter===g?'on':''}">${escapeHtml(g)} <b>${counts[g]}</b></button>`).join('')}</div><div class="employee-roster-table"><div class="employee-roster-row header"><span>직원</span><span>부서</span><span>역량 / 잠재력</span><span>월급</span><span>자동운영</span><span>관리</span></div>${rows.map(t=>`<div class="employee-roster-row"><span class="employee-name">${talentGradeBadge(t)}<b>${escapeHtml(t.name)}</b><small>${escapeHtml(t.specialty||'-')} · Lv.${Number(t.career_level||1)}</small></span><span>${escapeHtml(talentDepartmentShort(t.department))}</span><span><b>${Number(t.skill_score||0).toFixed(1)}</b><small>잠재력 ${Number(t.potential||50).toFixed(0)}</small></span><span>${formatKrwSmart(t.monthly_salary||0)}</span><span>+${talentAutopilotContribution(t).toFixed(1)}</span><span class="employee-roster-actions"><button type="button" data-talent-train="${t.id}" data-training-type="BALANCED">종합 연수</button><button type="button" data-talent-train="${t.id}" data-training-type="SPECIALTY">전문 연수</button><button type="button" class="employee-fire-btn" data-talent-fire="${t.id}">해고</button></span></div>`).join('')}</div></section>`;
}
...
function renderTalentMarket(my){
  ...
  return `<section class="talent-market talent-v640">
    <div class="talent-command-head"><div><small>PEOPLE & TALENT · DAY ${day}</small><h2>핵심인재실</h2><span>하루 기본 채용 + 유료 면접 + 스카우트로 인재를 확보하세요. 좋은 인재일수록 회사 자동 운영력이 올라갑니다.</span></div><div><b>${talents.length}명</b><span>오늘 채용 ${hired}/${limit}</span></div></div>
...
function renderMediaDesk(my){
  ...
  return `<section class="corp-section media-desk-section clean-newsroom-management">
    <div class="company-section-head"><div><small>NEWS / IR</small><h2>언론 대응</h2></div></div>
    ...
    <div class="media-target-shell compact"><div><small>현재 기사 대상</small><b class="${Number(target.id)===threatId?'danger-text':''}">${escapeHtml(target?.name||'')}</b><span>${Number(target.id)===threatId?'내 경영권을 위협 중인 회사':companyOperatorType(target)==='PLAYER'&&Number(target.id)!==Number(my.id)?'실제 유저 회사':Number(target.id)===Number(my.id)?'내 회사':'BOT 회사'}</span></div><label><span>대상 선택</span><select id="companyMediaTarget">${options}</select></label></div>
    ...
    <div class="auto-outlet-grid">${filtered.map(o=>`<article class="auto-outlet-card"><div><small>${o.kind}</small><h3>${o.name}</h3></div><dl><div><dt>비용</dt><dd>${formatKrwSmart(o.cost*(tone==='CRITICAL'?1.15:1))}</dd></div><div><dt>도달</dt><dd>${o.reach}</dd></div></dl><button type="button" data-company-media="${o.key}" data-media-api-code="${o.apiCode}" data-media-cost="${o.cost}" data-media-name="${o.name}" data-media-region="${o.region}">${tone==='CRITICAL'?'비판 기사 발행':tone==='NEUTRAL'?'사실 기사 발행':'홍보 기사 발행'}</button></article>`).join('')}</div>
...
function renderOperationsWorkspace(my){
  ...
  const tabs=[['products','제품·사업'],['projects','프로젝트'],['supply','공급망'],['finance','재무'],['portfolio','법인투자'],['global','해외시장']];
  let body='';
...
function buildChairmanProposals(my){
  ...
  if(threat){
    ...
    if(stake>=20)push({id:`defense-negotiate-${Number(threat.attacker_company_id||0)}-${Math.floor(cycle/10)}`,kind:'DEFENSE_NEGOTIATE',priority:'critical',department:'FINANCE',officer:crisisOfficer,title:'공격자 지분 매각 협상안',summary:'프리미엄을 지급해 공격자가 보유한 우리 회사 지분을 직접 되사는 협상안입니다.',budget:Math.max(100000000,Math.min(cash*.18,valuation*.012)),impact:'공격자 지분 직접 감소 · 현금 지출'});
    if(stake>=35)push({id:`defense-white-${Number(threat.attacker_company_id||0)}-${Math.floor(cycle/12)}`,kind:'DEFENSE_WHITE_KNIGHT',priority:'critical',department:'MANAGEMENT',officer:mgmt?.top||crisisOfficer,title:'백기사 우호지분 확보안',summary:'경영지원팀이 우호 주주를 끌어들여 의결권 방어력을 높이자고 제안했습니다.',budget:Math.max(100000000,Math.min(cash*.14,valuation*.01)),impact:'방어력 대폭 강화 · 우호 의결권 확보'});
    if(stake>=45)push({id:`defense-rights-${Number(threat.attacker_company_id||0)}-${Math.floor(cycle/12)}`,kind:'DEFENSE_RIGHTS_ISSUE',priority:'critical',department:'FINANCE',officer:crisisOfficer,title:'긴급 유상증자 희석안',summary:'경영권선이 가까워 재무팀이 신주 발행으로 공격자 지분을 희석하는 비상안을 상신했습니다.',budget:Math.max(100000000,Math.min(Math.max(cash*.18,valuation*.012),valuation*.025)),impact:'공격자 지분 희석 · 신규 현금 조달 · 기존 주주 희석'});
    if(Number(threat.attacker_company_id||0)>0&&cash>120000000)push({id:`defense-counter-${Number(threat.attacker_company_id)}-${Math.floor(cycle/14)}`,kind:'DEFENSE_COUNTER',priority:'warn',department:'MANAGEMENT',officer:mgmt?.top||crisisOfficer,title:'역인수·맞지분 확보안',summary:'상대 회사 지분을 역으로 매입해 협상력을 확보하자는 제안입니다.',budget:Math.max(120000000,Math.min(cash*.18,valuation*.012)),impact:'상대 회사 맞지분 확보 · 협상 압력'});
  }
...
function renderCompanySubnav(my){
  ...
  const items=[
    ['dashboard','경영 홈'],['operations','사업 운영'],['people','직원',offers?`이직제안 ${offers}`:`핵심인재 ${(state.company?.talents||[]).length}명`],['competition','투자·M&A',threat>=15?`위험 ${threat.toFixed(1)}%`:null],['risk','뉴스·리스크']
  ];
...
function renderTerminal(preserve=false){
  ...
  app.innerHTML=`<div class="terminal management-first-terminal">
    <header class="top management-topbar">
      <div class="brand"><div class="kxlogo">KX</div><strong>KX CORPORATE</strong><span class="online-mode-chip ${state.companyAvailable===false?'offline':'online'}">${state.companyAvailable===false?'ONLINE 연결 필요':'ONLINE · LIVE 6.4.5'}</span></div>
      ${topNav()}
...
<button class="tutorial-btn" id="tutorialBtn">튜토리얼</button><button class="logout" id="logout">로그아웃</button>
    ...
    <nav class="mobile-nav management-mobile-nav">
      ...
      <button data-main-tab="company" data-company-section-nav="people" class="${state.tab==='company'&&state.companySection==='people'?'on':''}">직원</button>
...
function bind(){
  ...
  document.querySelectorAll('[data-chairman-approve]').forEach(b=>b.onclick=async()=>{
    ...
    try{
      ...
      else if(p.kind==='DEFENSE_NEGOTIATE'){result=await companyDefenseV640('NEGOTIATE',p.budget);if(result?.ok===false)throw new Error(result.message||'지분 협상 실패');state.companyNotice=result?.message||'공격자 지분 매각 협상을 집행했습니다.';await loadCompanyLayer(false,true);renderTerminal(true);}
      else if(p.kind==='DEFENSE_WHITE_KNIGHT'){result=await companyDefenseV640('WHITE_KNIGHT',p.budget);if(result?.ok===false)throw new Error(result.message||'백기사 확보 실패');state.companyNotice=result?.message||'백기사 우호지분 확보안을 집행했습니다.';await loadCompanyLayer(false,true);renderTerminal(true);}
      else if(p.kind==='DEFENSE_RIGHTS_ISSUE'){result=await companyDefenseV640('RIGHTS_ISSUE',p.budget);if(result?.ok===false)throw new Error(result.message||'긴급 유상증자 실패');state.companyNotice=result?.message||'긴급 유상증자로 공격자 지분을 희석했습니다.';await loadCompanyLayer(false,true);renderTerminal(true);}
  ...
  document.querySelectorAll('[data-company-media]').forEach(b=>b.onclick=async()=>{
    ...
    try{
      b.disabled=true;const oldLabel=b.textContent;b.textContent='발행 중…';const result=await companyMediaV640(outlet,targetId,tone);if(result?.ok===false)throw new Error(result?.message||'기사를 처리하지 못했습니다.');
    ...
    }catch(err){state.companyNotice='언론 처리 실패: '+err.message;alert(state.companyNotice);renderTerminal(true);}
  });
...
실패하더라도 헤드헌터 착수비가 발생합니다.`))return;
  ...
  document.querySelectorAll('[data-talent-fire]').forEach(b=>b.onclick=async()=>{
    ...
    try{b.disabled=true;b.textContent='처리 중…';const d=await companyTalentTerminateV645(id);if(!d?.ok)throw new Error(d?.message||'해고 처리 실패');state.companyNotice=d.message||`${t.name}의 퇴직 처리가 완료되었습니다.`;playCompanySfx('alert');await loadCompanyLayer(false,true);renderTerminal(true);}catch(err){state.companyNotice='해고 실패: '+err.message;alert(state.companyNotice);renderTerminal(true);}
  });
  ...
  document.querySelectorAll('[data-talent-train]').forEach(b=>b.onclick=async()=>{
    ...
    if(!confirm(`${t.name}에게 ${type==='SPECIALTY'?'전문':'종합'} 연수를 진행할까요?\n잠재력이 높아도 성장 성공은 보장되지 않습니다.`))return;
    try{b.disabled=true;b.textContent='연수 중…';const d=await companyTalentApi('TRAIN',{p_talent_id:id,p_training_type:type});if(!d?.ok)throw new Error(d?.message||'연수 실패');state.companyNotice=d.message;playCompanySfx(d.grew?'success':'alert');await loadCompanyLayer(false,true);renderTerminal(true);}catch(err){state.companyNotice='연수 처리 실패: '+err.message;alert(state.companyNotice);renderTerminal(true);}
  });
