...
(() => {
  ...
  function acceptRoomUpdate(next,{initial=false}={}){
    ...
    if(leavingBattle){
      ...
      const delay=Math.min(14000,Math.max(620,choreography+160));
    ...
    if(!initial)setTimeout(()=>{const choreography=playRoomEvents(next,after);setTimeout(()=>playCombatDelta(prev,next),Math.min(14000,Math.max(140,choreography+100)));},changed?260:0);
  }
  ...
  async function playNamedMonsterMoveV45(payload,enemySide=false){
    if(!payload)return;const actor=enemySide?$(`[data-enemy="${CSS.escape(payload.enemyUid||'')}"]`):$(`[data-monster="${CSS.escape(payload.instanceId||'')}"]`),target=enemySide?(payload.targetMonsterId?$(`[data-monster="${CSS.escape(payload.targetMonsterId)}"]`):$('.player-field-v44')):$(`[data-enemy="${CSS.escape(payload.targetUid||'')}"]`);if(!actor||!target)return;
  ...
  function queueAutoEncounter(r,node){
    const claim=r.contractClaims?.[state.profileId];if(!claim||!node||node.votes?.includes(state.profileId))return;
    const key=`${r.id}:${r.floor}:${node.id}`;if(state.autoEncounterKey===key)return;state.autoEncounterKey=key;
    setTimeout(()=>{const now=state.room,current=now?.route?.find(n=>n.id===node.id);if(!now||now.id!==r.id||now.floor!==r.floor||now.status!=='route'||!current||current.votes?.includes(state.profileId)){return;}if(state.busy){state.autoEncounterKey='';return queueAutoEncounter(now,current);}roomPost('vote',{nodeId:current.id}).catch(()=>{state.autoEncounterKey='';});},r.floor===1?900:620);
  }
  ...
  function renderRoute(r){
    const run=r.runState[state.profileId],nodes=r.route||[],pressure=(r.threatTokens||[]).length,claim=r.contractClaims?.[state.profileId];
    const fork=nodes.length>1||nodes.some(n=>n.biomeChoice),votedNode=nodes.find(n=>n.votes?.includes(state.profileId));
    const contract=!claim?contractPanel(r):`<div class="contract-mini-v42"><span>STARTER CORE</span><b>${esc((r.contractOffers?.[state.profileId]||[]).find(x=>x.id===claim)?.name||'무서약')}</b></div>`;
    let content='';
    if(fork){
      content=`<div class="biome-fork-v42">${waveTracker(r,'NEXT BIOME')}<div class="biome-fork-copy-v42"><small>BIOME FORK</small><h1>다음 10웨이브를 고르세요</h1><p>수호자를 쓰러뜨린 뒤에는 길이 갈라집니다. 바이옴마다 등장 몬스터와 보스가 달라집니다.</p></div><div class="biome-fork-grid-v42">${nodes.map((node,i)=>{const selected=votedNode?.id===node.id;return `<button class="biome-path-v42 ${selected?'selected':''}" data-action="route-vote" data-node="${esc(node.id)}" ${!claim?'disabled':''}><span>${node.icon||'⌁'}</span><small>${esc(node.kicker||`PATH ${i+1}`)}</small><b>${esc(node.biomeName||node.label)}</b><em>${esc(node.desc||'')}</em><strong>${selected?'선택됨':`${node.votes?.length||0}표`}</strong></button>`;}).join('')}</div></div>`;
    }else{
      const node=nodes[0],voted=node?.votes?.includes(state.profileId),kind=node?.kind||'combat',tag=kind==='boss'?'BIOME BOSS':kind==='elite'?'WARDEN BATTLE':kind==='rival'?'RIFT HUNTER':'WILD ENCOUNTER';
      content=`<div class="encounter-gate-v42 kind-${esc(kind)}">${waveTracker(r,'BIOME RUN')}<div class="encounter-core-v42">${routeGraphic(kind)}<small>${esc(node?.kicker||tag)}</small><h1>${esc(node?.label||'야생 조우')}</h1><p>${esc(node?.desc||'다음 몬스터 조우를 시작합니다.')}</p><div class="encounter-tags-v42"><span>${r.floor}F</span><span>${esc(r.biome.name)}</span><span>LV CAP ${r.levelCap||'-'}</span>${pressure?`<span>PRESSURE ×${pressure}</span>`:''}</div><button class="encounter-enter-v42 ${voted?'waiting':''}" data-action="route-vote" data-node="${esc(node?.id||'')}" ${!claim||voted?'disabled':''}>${!claim?'먼저 STARTER CORE 선택':voted?'동료 준비 대기 중…':'조우 시작'}</button></div></div>`;
    }
    setScreen('route',`<section class="run-stage-v31 route-stage-v31 route-v42"><div class="run-bg-v31" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="ambient-grid-v31"></div><div class="run-shade-v31"></div>${compactRunHud(r,run)}<main class="decision-center-v31 encounter-decision-v42">${contract}${content}</main><div class="floor-track-v31"><i style="width:${r.maxFloor?pct(r.floor,r.maxFloor):Math.min(100,(r.floor%50)*2)}%"></i><span>${r.floor}/${r.maxFloor||'∞'}</span></div></section>`);
    if(claim&&!fork&&nodes[0]&&!nodes[0].votes?.includes(state.profileId))queueAutoEncounter(r,nodes[0]);
  }
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map((u,i)=>`<button class="active-mon-v41 active-mon-v44 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''} archetype-${esc(u.archetype||'beast')}" data-action="select-monster" data-monster="${esc(u.instanceId)}"><div class="mon-status-v41 mon-status-v44"><div><b>${esc(u.name)}</b><small>Lv.${u.level}${r.levelCap?` / LEVEL CAP ${r.levelCap}`:''} · ${esc(u.element)}${u.secondaryElement?`/${esc(u.secondaryElement)}`:''}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i>${u.block?`<strong>⬢ ${u.block}</strong>`:''}</div><div class="mon-sprite-v41 mon-sprite-v44"><i class="mon-shadow-v41"></i>${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt=""><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v41">DONE</em>':''}</button>`).join('');
    const enemyActors=alive.map((e,i)=>{const it=intentCompact(e.intent);return `<button class="enemy-mon-v41 enemy-mon-v44 ${state.selectedEnemy===e.uid?'selected':''} archetype-${esc(e.archetype||enemyArchetypeName(e.name))} ${e.tier==='boss'?'is-boss-v50':''} ${e.broken?'broken':''}" data-action="select-enemy" data-enemy="${esc(e.uid)}"><div class="enemy-status-v41 enemy-status-v44"><div><b>${esc(e.name)}</b><small>${esc(e.element||'')} ${enemyWeakBadge(e.element)}</small></div><span>${e.hp}/${e.maxHp}</span><i><em style="width:${pct(e.hp,e.maxHp)}%"></em></i>${e.block?`<strong>⬢ ${e.block}</strong>`:''}</div><div class="enemy-sprite-v41 enemy-sprite-v44"><i class="enemy-shadow-v41"></i><img class="enemy-sprite" src="${esc(e.sprite)}" alt=""><span class="target-ring-v41"></span></div><div class="enemy-intent-mini-v41 enemy-intent-v44"><small>NEXT</small><b>${esc(it.icon)} ${esc(it.value||'')}</b></div></button>`;}).join('');
    const rootMenu=`<div class="root-command-v41 root-command-v44 root-command-v45 root-command-v47"><button class="fight" data-action="battle-menu" data-menu="moves"><span>⚔</span><div><b>기술</b><small>${selectedMon?.acted?'행동 완료':'4개 기술 · 숙련 진화'}</small></div><em>FIGHT</em></button><button class="spell" data-action="battle-menu" data-menu="spells"><span>✦</span><div><b>스펠</b><small>전술 ${me.tacticsUsed||0}/${me.tacticLimit||2}</small></div><em>TACTIC</em></button><button class="party" data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length?'disabled':''}><span>↔</span><div><b>교대</b><small>${(me.bench||[]).length}마리 대기</small></div><em>SWITCH</em></button><button class="capture" data-action="battle-menu" data-menu="capture" ${!['wild','boss'].includes(b.encounterType)?'disabled':''}><span>◉</span><div><b>봉인</b><small>${['wild','boss'].includes(b.encounterType)?`HP ${Math.round((Number(selectedEnemy?.hp||0)/Math.max(1,Number(selectedEnemy?.maxHp||1)))*100)}% · 낮을수록 유리`:'이 전투에서는 불가'}</small></div><em>CAPTURE</em></button></div>`;
    const moves=selectedMon?.moves||[];
    const moveMenu=`<div class="move-command-v41 move-command-v44 move-command-v47"><div class="subcommand-head-v41 subcommand-head-v47"><button data-action="battle-menu" data-menu="root">←</button><b>${esc(selectedMon?.name||'MONSTER')} · 기술</b><span>${selectedMon?.acted?'행동 완료':'1~4 키로 즉시 사용 · 숙련도에 따라 기술 진화'}</span></div><div class="move-grid-v41 move-grid-v44 move-grid-v47">${[0,1,2,3].map(i=>{const mv=moves[i];if(!mv)return`<button class="move-btn-v47 empty" disabled><em>0${i+1}</em><b>EMPTY</b></button>`;const can=moveUsable(mv,selectedMon,me),mastery=moveMasteryUi(mv),family=namedMoveFamily({skill:mv.name,element:mv.element||selectedMon.element,move:mv});return `<button class="move-btn-v41 move-btn-v44 move-btn-v45 move-btn-v47 family-${family} kind-${esc(mv.kind||'attack')} element-${elementClass(mv.element||selectedMon.element)}" data-action="move-use" data-move-id="${esc(mv.id)}" ${can?'':'disabled'}><em>0${i+1}</em><div class="move-top-v47"><span class="move-element-v47">${elementGlyph(mv.element||selectedMon.element)} ${esc(mv.element||selectedMon.element)}</span><strong>${Number(mv.cooldownRemaining||0)>0?`CD ${mv.cooldownRemaining}`:'READY'}</strong></div><div class="move-name-v47"><i>${namedSkillGlyphV47(family)}</i><b>${esc(mv.name)}</b>${mastery.label?`<mark>${mastery.label}</mark>`:''}</div><p>${esc(mv.summary||moveKindKo(mv))}</p><div class="move-stats-v47">${moveDetailBadgesV47(mv)}</div><u class="move-mastery-v44 move-mastery-v47"><i style="width:${mastery.p}%"></i><small>${mastery.label?`숙련 ${mastery.label}`:`숙련 ${mastery.uses}/${mastery.next}`}</small></u></button>`;}).join('')}</div></div>`;
    const spellMenu=`<div class="spell-command-v41 spell-command-v44"><div class="subcommand-head-v41"><button data-action="battle-menu" data-menu="root">←</button><b>전술 스펠</b><span>턴당 최대 ${me.tacticLimit||2}회 · ${me.tacticsUsed||0}/${me.tacticLimit||2}</span></div><div class="spell-list-v44">${me.hand.map((cid,i)=>{const c=runCardView(byId(state.meta.cards,cid),run),cost=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(me.buffs.spellDiscount>0?1:0))),can=!me.ended&&b.phase==='players'&&Number(me.tacticsUsed||0)<Number(me.tacticLimit||2)&&me.energy>=cost;return spellCommandTileV44(c,i,can,cost);}).join('')}</div></div>`;
    const captureMenu=battleCaptureMenuV45(selectedEnemy,run,b,me,selectedMon);
    const partyMenu=`<div class="party-command-v41 party-command-v44"><div class="subcommand-head-v41"><button data-action="battle-menu" data-menu="root">←</button><b>몬스터 교대</b><span>교대도 행동을 소비합니다.</span></div><div class="party-switch-grid-v41">${(me.bench||[]).map(u=>`<button data-action="switch-direct" data-active="${esc(selectedMon?.instanceId||'')}" data-bench="${esc(u.instanceId)}"><img src="${esc(monsterDisplaySprite(u))}"><div><b>${esc(u.name)}</b><small>Lv.${u.level}${r.levelCap?` / ${r.levelCap}`:''} · ${esc(u.element)}</small><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i><span>${u.hp}/${u.maxHp}</span></div></button>`).join('')||'<span>대기 몬스터가 없습니다.</span>'}</div></div>`;
    const infoMenu=`<div class="info-command-v41 info-command-v44"><div class="subcommand-head-v41"><button data-action="battle-menu" data-menu="root">←</button><b>전투 정보</b><span>필요한 정보만 확인</span></div><div class="battle-info-grid-v41"><div><small>내 몬스터</small><b>${esc(selectedMon?.name||'-')}</b><span>${matchupBadge(selectedMon?.element||'')}</span></div><div><small>적 약점</small><b>${esc(selectedEnemy?.name||'-')}</b><span>${enemyWeakBadge(selectedEnemy?.element||'')}</span></div><button data-action="open-evolution" data-monster="${esc(selectedMon?.instanceId||'')}" ${!selectedMon?'disabled':''}><span>✦</span><b>진화 / 융합</b><small>조건 확인</small></button><button data-action="run-info"><span>◈</span><b>RUN</b><small>유물 · 보물</small></button></div></div>`;
    ...
    const partyMini=[...(me.units||[]),...(me.bench||[])].slice(0,6).map(u=>`<span class="${u.hp<=0?'ko':state.selectedMonster===u.instanceId?'on':''}" title="${esc(u.name)}"><img src="${esc(monsterDisplaySprite(u))}"><i style="width:${pct(u.hp,u.maxHp)}%"></i></span>`).join('');
    setScreen('battle',`<section class="monster-battle-v41 monster-battle-v44 mode-${mode}"><div class="battle-bg-v41" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="battle-atmos-v41 battle-atmos-v44"></div><header class="battle-top-v41 battle-top-v42 battle-top-v44"><div><small>${esc(b.encounterType==='warden'?'WARDEN BATTLE':b.encounterType==='boss'?'BIOME BOSS':b.encounterType==='rival'?'RIFT HUNTER':mode==='double'?'DOUBLE WILD':'WILD ENCOUNTER')} · ${r.floor}F</small><b>${esc(b.encounterLabel||r.biome.name)}</b></div>${waveTracker(r,'WAVE')}<div class="party-mini-v44">${partyMini}</div><div class="core-mini-v41"><span>CORE</span><b>${me.hp}/${me.maxHp}</b>${me.block?`<strong>⬢ ${me.block}</strong>`:''}</div><div><button data-action="run-info">RUN</button><button data-action="toggle-combat-log">LOG</button></div></header><main class="battle-arena-v41 battle-arena-v44"><section class="enemy-field-v41 enemy-field-v44"><small class="side-kicker-v44">ENEMY</small>${enemyActors}</section><section class="player-field-v41 player-field-v44"><small class="side-kicker-v44">PARTY</small>${monActors}</section></main><footer class="command-box-v41 command-box-v44 command-box-v47"><div class="battle-resource-v47"><span>⚡ <b>${me.energy}</b>/${me.maxEnergy}</span><small>${esc(selectedMon?.name||'MONSTER')} · Lv.${selectedMon?.level||'-'}</small><i><em style="width:${pct(selectedMon?.hp||0,selectedMon?.maxHp||1)}%"></em></i></div><div class="command-content-v41">${me.ended?'<div class="turn-wait-v41"><span>…</span><b>상대 행동을 기다리는 중</b></div>':command}</div><div class="battle-side-actions-v47"><button data-action="battle-menu" data-menu="info">정보 <b>I</b></button><button class="skip-turn-v41 skip-turn-v44" data-action="end-turn" ${me.ended?'disabled':''}>턴 종료 <b>E</b></button></div></footer><aside class="battle-log-drawer-v31 ${state.combatLogOpen?'open':''}"><div><b>LOG</b><button data-action="toggle-combat-log">×</button></div>${b.log.slice(-10).reverse().map(x=>`<p>${esc(x.text)}</p>`).join('')}</aside></section>`);
  }
  ...
  function fieldServiceHtml(r,run){
    ...
    const svc=(id,icon,name,desc,enabled=true)=>`<button data-action="field-service" data-service="${id}" ${used[id]||run.gold<Number(costs[id]||0)||!enabled?'disabled':''}><span>${icon}</span><b>${name}</b><small>${desc}</small><strong>${used[id]?'DONE':`${costs[id]}G`}</strong></button>`;
    return `<div class="field-service-v42"><div class="field-service-title-v42"><span>FIELD SUPPORT</span><b>웨이브 정비소</b><small>골드는 보상 재굴림뿐 아니라 다음 조우를 준비하는 자원입니다.</small></div><div class="field-service-grid-v42">${svc('mend','♥','응급 회복','CORE 15% · 생존 몬스터 22%',damaged)}${svc('revive','✚','부활 캡슐','쓰러진 몬스터 1체 HP 40%',ko)}${svc('charge','✦','아르카나 충전',`다음 전투 에너지/시작패 +1 · 충전 ${charge}/2`,charge<2)}</div></div>`;
  }
  function postBattlePartySummary(r,run){
    ...
    return `<section class="postbattle-summary-v43 postbattle-summary-v44"><header><div><small>WAVE ${String(((Number(r.floor||1)-1)%10)+1).padStart(2,'0')} CLEAR</small><b>파티 경험치</b></div>${grade?`<strong>${esc(grade)}</strong>`:''}</header><div class="party-exp-v43 party-exp-v44">${rows.map((x,idx)=>{const mon=(run.monsters||[]).find(m=>m.instanceId===x.instanceId),sprite=x.sprite||monsterDisplaySprite(mon),pctv=Math.max(0,Math.min(100,Math.round(Number(x.xp||0)/Math.max(1,Number(x.xpNeed||1))*100))),gain=x.statGain||{};return `<article class="exp-mon-v43 exp-mon-v44 ${x.leveled?'leveled':''} ${x.partyRole==='ko'?'ko':''}" style="--delay:${idx*90}ms"><div class="exp-sprite-v43"><img src="${esc(sprite||'')}" alt="">${x.leveled?'<i>LEVEL UP</i>':''}</div><div class="exp-copy-v43"><b>${esc(mon?.name||x.name||'MONSTER')}</b><span>Lv.${x.fromLevel}${x.fromLevel!==x.toLevel?` <em>→ ${x.toLevel}</em>`:''}</span><div><i style="--xp:${pctv}%;width:${pctv}%"></i></div><small>EXP +${x.xpGained} · ${Math.round(x.xp||0)}/${Math.round(x.xpNeed||0)}${x.capped?` · CAP ${x.levelCap}`:''}</small>${x.leveled?`<p><span>HP +${Number(gain.hp||0)}</span><span>ATK +${Number(gain.power||0)}</span>${x.newMoves?.length?`<b>NEW · ${esc(x.newMoves.map(m=>m.name).join(', '))}</b>`:''}</p>`:''}</div></article>`;}).join('')}</div></section>`;
  }

  function rewardChoiceTileV43(o){
    if(o.type==='card'){const c=o.card;return `<button class="modifier-option-v43 rarity-${esc(c.rarity)}" data-action="reward-pick" data-reward-id="${esc(o.id)}" title="${esc(c.text||'')}"><span class="modifier-art-v43"><img src="${esc(c.art||'')}" alt=""><i>${esc(elementGlyph(c.element))}</i></span><b>${esc(c.name)}</b><small>SPELL SIGIL · ${esc(c.element)} · ${c.cost}E</small><em>${esc(cardKeywords(c,2).join(' · ')||cardRole(c).label)}</em></button>`;}
    const it=o.item;return `<button class="modifier-option-v43 rarity-${esc(it.rarity)}" data-action="reward-pick" data-reward-id="${esc(o.id)}" title="${esc(it.text||'')}"><span class="modifier-art-v43"><img src="${esc(it.art||'')}" alt=""><i>${esc(it.icon||'◆')}</i></span><b>${esc(it.name)}</b><small>${esc(it.category||rarityKo(it.rarity))}</small><em>${esc(shortLine(it.text||'',34))}</em></button>`;
  }
  function renderReward(r){
    ...
    const rewriteOffer=rw.rewriteOffers?.[pid],rewriteDone=!rewriteOffer||!!rw.rewriteClaims?.[pid];
    const capturePending=false;const afterCore=campDone&&skillDone&&rewriteDone&&(!relicOpts.length||!!relicClaim)&&rewardDone;let step='REWARD',body='',actions='';
    ...
    if(skillOffer&&!skillDone){
      ...
      body=`<section class="learn-offer-v44"><div class="learn-mon-v44"><img src="${esc(skillOffer.monsterSprite||'')}" alt=""><div><small>${esc(skillOffer.reason||'BATTLE REWARD')}</small><b>${esc(skillOffer.monsterName)}</b><span>${isLevel?'레벨업으로 새로운 기술을 떠올렸습니다.':'강적에게서 새로운 기술 데이터를 획득했습니다.'}</span></div></div><div class="learn-move-options-v44 ${moves.length===1?'single':''}">${moves.map(m=>`<button data-action="open-learn-move" data-move-id="${esc(m.id)}" class="kind-${esc(m.kind||'attack')} element-${elementClass(m.element||'')}"><span>${moveKindIcon(m)}</span><div><small>${isLevel?'LEVEL MOVE':'TECH DISC'}</small><b>${esc(m.name)}</b><em>${esc(movePowerLine(m))}</em><p>${esc(m.summary||moveKindKo(m))}</p></div><strong>${m.accuracy||100}%<i>CD ${m.cooldown||0}</i></strong></button>`).join('')}</div></section>`;
      actions=`<div class="reward-actions-v31 reward-actions-v43"><button data-action="skip-move">${isLevel?'이번에는 배우지 않기':'기술 디스크 보류'}</button></div>`;
    }
    else if(rewriteOffer&&!rewriteDone){
      const target=(run.monsters||[]).find(m=>m.instanceId===rewriteOffer.instanceId),move=(target?.moves||[]).find(m=>m.id===rewriteOffer.moveId);
      step='RIFT REWRITE';
      body=`<section class="rewrite-reward-v42"><header><small>BOSS CLEAR MUTATION</small><b>많이 쓴 기술 하나가 균열에 물들었습니다.</b><span>단순 수치 강화가 아니라, 기술의 사용법 자체를 바꾸는 개조를 하나 각인합니다.</span></header><div class="rewrite-target-v42"><div class="rewrite-mon-v42"><img src="${esc(rewriteOffer.monsterSprite||monsterDisplaySprite(target)||'')}" alt=""><div><small>${esc(rewriteOffer.monsterName)} · 사용 ${rewriteOffer.uses||0}회</small><b>${esc(rewriteOffer.moveName)}</b><span>${esc(movePowerLine(move||{name:rewriteOffer.moveName,kind:rewriteOffer.moveKind}))}</span></div></div><i>→</i><strong>?</strong></div><div class="rewrite-options-v42">${(rewriteOffer.options||[]).map(x=>`<button class="rewrite-option-v42 kind-${esc(rewriteOffer.moveKind||'attack')}" data-action="rewrite-pick" data-rewrite-id="${esc(x.id)}"><span>${esc(x.icon||'✦')}</span><small>MOVE MUTATION</small><b>${esc(x.name)}</b><em>${esc(x.desc)}</em></button>`).join('')}</div></section>`;
      actions=`<div class="reward-actions-v31 reward-actions-v43"><button data-action="skip-rewrite">개조하지 않고 유지</button></div>`;
    }
    ...
    else if(!rewardDone){step=r.mode==='journey'?'FIELD SUPPLY':'CHOOSE ONE';body=`<section class="modifier-pick-v43"><header><small>${r.mode==='journey'?'FIELD SUPPLY':'RIFT DROP'}</small><b>보상 하나를 선택하세요</b><span>전투 흐름은 유지하고, 필요한 강화만 집어 듭니다.</span></header><div class="modifier-options-v43 reward-options-v31">${options.map(rewardChoiceTileV43).join('')}</div></section>`;actions=`<div class="reward-actions-v31 reward-actions-v43"><button data-action="reward-salvage">건너뛰고 분해</button><button data-action="reward-reroll">↻ 재굴림 ${nextRerollCost(r)}G</button>${run.fragments>0?'<button data-action="reward-reroll-fragment">◇ 파편 1</button>':''}</div>`;}
    else if(rw.shop&&afterCore){step='FIELD SHOP';body=merchantHtmlCompact(r);actions=`<div class="reward-actions-v31"><button data-action="open-purge">덱 정제</button><button class="next-button-v31" data-action="reward-continue">${esc(nextEncounterPreview(r))}</button></div>`;}
    else{step='READY';const grade=rw.gradeBy?.[pid];body=`<div class="reward-ready-v31">${waveTracker(r,'CLEARED')}<span>✓</span><b>${claim?esc(claim.label):'준비 완료'}</b>${grade?`<em>${grade}</em>`:''}<small>${r.finalClearPending?'FINAL CLEAR':esc(nextEncounterPreview(r))}</small></div>`;actions=`<div class="reward-actions-v31"><button class="next-button-v31" data-action="reward-continue">${continued?'WAIT':r.finalClearPending?'CLEAR':'NEXT WAVE'}</button></div>`;}
    const support=rw.kind==='battle'&&!capturePending&&skillDone&&rewriteDone?fieldServiceHtml(r,run):'';
    setScreen('reward',`<section class="run-stage-v31 reward-stage-v31 reward-v41 reward-v42 reward-v43"><div class="run-bg-v31" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="ambient-grid-v31"></div><div class="run-shade-v31"></div>${compactRunHud(r,run)}<main class="reward-center-v31 reward-center-v43"><div class="reward-focus-v31 reward-focus-v43"><div class="reward-step-v31">${esc(step)}</div>${expPanel}${body}${support}${actions}</div></main></section>`);
  }
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='battle-capture'){if(state.busy)return;const me=state.room?.battle?.party?.find(p=>p.playerId===state.profileId),u=me?.units?.find(x=>x.instanceId===state.selectedMonster)||me?.units?.find(x=>!x.acted)||me?.units?.[0],enemy=state.room?.battle?.enemies?.find(e=>e.uid===state.selectedEnemy);if(!u||!enemy)return;await captureThrowFx(el,enemy,el.dataset.seal);const d=await roomPost('capture',{sealType:el.dataset.seal,enemyUid:enemy.uid,instanceId:u.instanceId});await captureResultFx(d.result.monster,d.result.success,false,d.result.newDiscovery);await loadProfile();state.battleMenu='root';toast(d.result.success?`봉인 성공 · ${d.result.monster.name} 획득!`:`봉인 실패 · 성공률 ${Math.round(Number(d.result.chance||0)*100)}%`,d.result.success?'good':'error');return renderRoom();}
      ...
      if(action==='field-service'){const d=await roomPost('service',{serviceId:el.dataset.service});toast(d.result.label,'good');sound('reward');return renderRoom();}
