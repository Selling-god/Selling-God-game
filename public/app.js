...
(() => {
  ...
  function acceptRoomUpdate(next,{initial=false}={}){
    ...
    if(leavingBattle){
      ...
      const hasFinisher=(next?.feed||[]).some(ev=>Number(ev.seq)>Number(after||0)&&['monster-move','monster-burst','enemy-attack'].includes(ev.type));const delay=fxOn()?Math.min(45000,Math.max(hasFinisher?4600:2600,Number(choreography||0)+900)):160;
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>{const held=heldItemClient(u);return `<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${held?` <i class="battle-held-v53" title="${esc(held.text||'')}">${esc(held.icon||'◆')} ${esc(held.name)}</i>`:''}${playerStatusBadgeV56(u)}${statStageBadgesV56(u)}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i>${u.fusionSprite&&!u.fused?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`;}).join('');
