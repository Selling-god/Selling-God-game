...
(() => {
  ...
  function renderRoute(r){
    ...
    if(!claim){
      ...
      // Normal floors are server-auto-entered in v5.2. This is only a short
      // multiplayer wait/fallback screen and never auto-POSTs /vote.
      const node=nodes[0];
      decision=`<div class="route-loading-v52"><span>✦</span><small>${esc(node?.kicker||'NEXT BATTLE')}</small><b>${esc(node?.label||'전투 준비')}</b><p>동료 준비가 끝나면 자동으로 전투가 시작됩니다.</p>${node&&!node.votes?.includes(state.profileId)?`<button data-action="route-vote" data-node="${esc(node.id)}">전투 시작</button>`:''}</div>`;
    }
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>`<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i>${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`).join('');
