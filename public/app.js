...
(() => {
  ...
  function contractPanel(r){
    ...
    if(claim){const c=offers.find(x=>x.id===claim);return `<div class="starter-picked-v51"><span>${esc(c?.icon||'◇')}</span><div><small>RUN TRAIT</small><b>${esc(c?.name||claim)}</b></div></div>`;}
    return `<section class="starter-choice-v51"><header><small>RUN TRAIT</small><b>이번 원정 특성 하나를 고르세요</b><span>동료와 함께할 원정 특성 하나를 선택하세요.</span></header><div class="starter-grid-v51">${offers.map((c,i)=>`<button data-action="contract-pick" data-contract-id="${esc(c.id)}"><em>${i+1}</em><span>${esc(c.icon||'◇')}</span><div><b>${esc(c.name)}</b><small>${esc(shortLine(c.detail||c.desc,42))}</small></div></button>`).join('')}</div></section>`;
  }
  ...
  function renderRoute(r){
    ...
    const contract=!claim?contractPanel(r):'';
    let content='';
    if(fork){
    ...
    // v5.1: the opening trait choice is its own clean screen. Do not stack the route card behind it.
    const decision=!claim?contract:content;
    setScreen('route',`<section class="run-stage-v31 route-stage-v31 route-v42 route-v51"><div class="run-bg-v31" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="ambient-grid-v31"></div><div class="run-shade-v31"></div>${compactRunHud(r,run)}<main class="decision-center-v31 encounter-decision-v42 encounter-decision-v51">${decision}</main><div class="floor-track-v31"><i style="width:${r.maxFloor?pct(r.floor,r.maxFloor):Math.min(100,(r.floor%50)*2)}%"></i><span>${r.floor}/${r.maxFloor||'∞'}</span></div></section>`);
    if(!claim)return;
    if(!fork&&nodes[0]&&!nodes[0].votes?.includes(state.profileId))queueAutoEncounter(r,nodes[0]);
  }
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>`<button class="active-mon-v41 active-mon-v44 active-mon-v51 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v41 mon-status-v44 mon-status-v51"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${u.secondaryElement?`/${esc(u.secondaryElement)}`:''}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v41 mon-sprite-v44 mon-sprite-v51"><i class="mon-shadow-v41"></i>${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`).join('');
    const enemyActors=alive.map(e=>{const it=intentCompact(e.intent);return `<button class="enemy-mon-v41 enemy-mon-v44 enemy-mon-v51 ${state.selectedEnemy===e.uid?'selected':''} ${e.tier==='boss'?'is-boss-v50':''}" data-action="select-enemy" data-enemy="${esc(e.uid)}"><div class="enemy-status-v41 enemy-status-v44 enemy-status-v51"><div><b>${esc(e.name)}</b><small>${esc(e.element||'')} ${enemyWeakBadge(e.element)}</small></div><span>${e.hp}/${e.maxHp}</span><i><em style="width:${pct(e.hp,e.maxHp)}%"></em></i></div><div class="enemy-sprite-v41 enemy-sprite-v44 enemy-sprite-v51"><i class="enemy-shadow-v41"></i><img class="enemy-sprite" src="${esc(e.sprite)}" alt="${esc(e.name)}"><span class="target-ring-v41"></span></div><div class="enemy-intent-mini-v41 enemy-intent-v44 enemy-intent-v51"><small>NEXT</small><b>${esc(it.icon)} ${esc(it.value||'')}</b></div></button>`;}).join('');
    const rootMenu=`<div class="root-command-v51"><button class="fight" data-action="battle-menu" data-menu="moves" ${selectedMon?.acted?'disabled':''}><span>⚔</span><b>기술</b><small>${selectedMon?.acted?'행동 완료':'공격 기술 선택'}</small></button><button class="spell" data-action="battle-menu" data-menu="spells"><span>✦</span><b>스펠</b><small>${me.tacticsUsed||0}/${me.tacticLimit||2}</small></button><button class="party" data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length||selectedMon?.acted?'disabled':''}><span>↔</span><b>교대</b><small>${(me.bench||[]).length} 대기</small></button><button class="capture" data-action="battle-menu" data-menu="capture" ${!['wild','boss'].includes(b.encounterType)||selectedMon?.acted?'disabled':''}><span>◉</span><b>봉인</b><small>${selectedEnemy?`HP ${Math.round(Number(selectedEnemy.hp||0)/Math.max(1,Number(selectedEnemy.maxHp||1))*100)}%`:'대상 없음'}</small></button></div>`;
    const moves=selectedMon?.moves||[];
    const moveMenu=`<div class="move-command-v51"><div class="subcommand-head-v51"><button data-action="battle-menu" data-menu="root">←</button><div><small>${esc(selectedMon?.name||'MONSTER')}</small><b>기술을 선택하세요</b></div></div><div class="move-grid-v51">${[0,1,2,3].map(i=>{const mv=moves[i];if(!mv)return `<button class="move-btn-v51 empty" disabled><em>${i+1}</em><b>비어 있음</b></button>`;const can=moveUsable(mv,selectedMon,me),power=movePowerLine(mv);return `<button class="move-btn-v51 element-${elementClass(mv.element||selectedMon.element)}" data-action="move-use" data-move-id="${esc(mv.id)}" ${can?'':'disabled'}><em>${i+1}</em><span>${namedSkillGlyphV47(namedMoveFamily({skill:mv.name,element:mv.element||selectedMon.element,move:mv}))}</span><div><b>${esc(mv.name)}</b><small>${esc(power||moveKindKo(mv))}</small></div><strong>${Number(mv.cooldownRemaining||0)>0?`CD ${mv.cooldownRemaining}`:`${mv.accuracy||100}%`}</strong></button>`;}).join('')}</div></div>`;
    const spellMenu=`<div class="spell-command-v51"><div class="subcommand-head-v51"><button data-action="battle-menu" data-menu="root">←</button><div><small>TACTIC SPELL</small><b>스펠을 선택하세요 · ${me.tacticsUsed||0}/${me.tacticLimit||2}</b></div></div><div class="spell-list-v51">${me.hand.map((cid,i)=>{const c=runCardView(byId(state.meta.cards,cid),run),cost=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(me.buffs.spellDiscount>0?1:0))),can=!me.ended&&b.phase==='players'&&Number(me.tacticsUsed||0)<Number(me.tacticLimit||2)&&me.energy>=cost;return `<button data-action="play-card" data-index="${i}" ${can?'':'disabled'}><em>${i+1}</em><span>${spellSigilIcon(c)}</span><div><b>${esc(c.name)}</b><small>${esc(cardKeywords(c,2).join(' · ')||c.element)}</small></div><strong>${cost}E</strong></button>`;}).join('')}</div></div>`;
    const captureMenu=battleCaptureMenuV45(selectedEnemy,run,b,me,selectedMon);
    const partyMenu=`<div class="party-command-v51"><div class="subcommand-head-v51"><button data-action="battle-menu" data-menu="root">←</button><div><small>SWITCH</small><b>교대할 몬스터</b></div></div><div class="party-switch-v51">${(me.bench||[]).map(u=>`<button data-action="switch-direct" data-active="${esc(selectedMon?.instanceId||'')}" data-bench="${esc(u.instanceId)}"><img src="${esc(monsterDisplaySprite(u))}" alt=""><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)} · HP ${u.hp}/${u.maxHp}</small></div></button>`).join('')||'<span>대기 몬스터가 없습니다.</span>'}</div></div>`;
    const infoMenu=`<div class="info-command-v51"><div class="subcommand-head-v51"><button data-action="battle-menu" data-menu="root">←</button><div><small>BATTLE INFO</small><b>간단 정보</b></div></div><div class="battle-info-v51"><div><small>내 몬스터</small><b>${esc(selectedMon?.name||'-')}</b><span>${esc(selectedMon?.element||'')}</span></div><div><small>대상</small><b>${esc(selectedEnemy?.name||'-')}</b><span>${esc(elementWeakTo(selectedEnemy?.element||'').slice(0,3).join(' · ')||'약점 없음')}</span></div><button data-action="open-evolution" data-monster="${esc(selectedMon?.instanceId||'')}" ${!selectedMon?'disabled':''}>진화 / 융합</button><button data-action="run-info">RUN 정보</button></div></div>`;
    ...
    const actionHint=ready.length>1?`${ready.length}마리 행동 남음`:ready.length===1?`${ready[0].name} 행동`:'적 턴 자동 진행';
    setScreen('battle',`<section class="monster-battle-v41 monster-battle-v44 monster-battle-v51 mode-${mode}"><div class="battle-bg-v41 battle-bg-v51" style="background-image:url('${esc(sceneBg(r))}')"></div><div class="battle-atmos-v41 battle-atmos-v44 battle-atmos-v51"></div><header class="battle-top-v41 battle-top-v42 battle-top-v44 battle-top-v51"><div><small>${esc(b.encounterType==='boss'?'BIOME BOSS':b.encounterType==='rival'?'RIFT HUNTER':mode==='double'?'DOUBLE WILD':'WILD ENCOUNTER')} · ${r.floor}F</small><b>${esc(b.encounterLabel||r.biome.name)}</b></div>${waveTracker(r,'WAVE')}<div class="party-mini-v44 party-mini-v51">${partyMini}</div><div class="core-mini-v41"><span>CORE</span><b>${me.hp}/${me.maxHp}</b></div><div><button data-action="run-info">RUN</button><button data-action="toggle-combat-log">LOG</button></div></header><main class="battle-arena-v41 battle-arena-v44 battle-arena-v51"><section class="player-field-v41 player-field-v44 player-field-v51"><small class="side-kicker-v51">PARTY</small>${monActors}</section><section class="enemy-field-v41 enemy-field-v44 enemy-field-v51"><small class="side-kicker-v51">ENEMY</small>${enemyActors}</section></main><footer class="command-box-v41 command-box-v44 command-box-v47 command-box-v51"><div class="battle-resource-v51"><span>⚡ <b>${me.energy}</b>/${me.maxEnergy}</span><small>${esc(actionHint)}</small></div><div class="command-content-v41 command-content-v51">${me.ended?'<div class="turn-wait-v51"><span>…</span><b>적 턴 진행 중</b></div>':command}</div><div class="battle-side-v51"><button data-action="battle-menu" data-menu="info">정보</button><small>모든 출전 몬스터가 행동하면<br>적 턴이 자동으로 시작됩니다.</small></div></footer><aside class="battle-log-drawer-v31 ${state.combatLogOpen?'open':''}"><div><b>LOG</b><button data-action="toggle-combat-log">×</button></div>${b.log.slice(-10).reverse().map(x=>`<p>${esc(x.text)}</p>`).join('')}</aside></section>`);
  }
