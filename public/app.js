...
(() => {
  ...
  async function playNamedMonsterMoveV45(payload,enemySide=false){
    ...
    setTimeout(async()=>{
      ...
      else if(family==='wave'||family==='cyclone'||family==='music'){await waveFx(actor,target,card);namedSkillOverlayV45(target,family,payload);}
      ...
      else if(['slash','claw','bite','rush'].includes(family)){const hits=family==='claw'?3:family==='bite'?2:(variant%3)+1;for(let i=0;i<hits;i++)setTimeout(()=>slashAt(target,payload.element||'강철',heavy&&i===0),i*64);namedSkillOverlayV45(target,family,payload);}
      ...
      if(!['heal','guard'].includes(family)&&Number(payload.damage||0)>0)popNumber(target,`-${payload.damage}`,'damage'+(enemySide?' player':''));if(heavy)screenShake('hit');soundElement(payload.element||'공허',heavy);
    },family==='beam'?210:145);setTimeout(()=>actor.classList.remove('attacking-v40'),820);
  }
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>{const held=heldItemClient(u);return `<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${held?` <i class="battle-held-v53" title="${esc(held.text||'')}">${esc(held.icon||'◆')} ${esc(held.name)}</i>`:''}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i>${u.fusionSprite?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`;}).join('');
    const enemyActors=alive.map(e=>{const it=intentCompact(e.intent);return `<button class="enemy-mon-v41 enemy-mon-v44 enemy-mon-v51 enemy-mon-v52 ${state.selectedEnemy===e.uid?'selected':''} ${e.tier==='boss'?'is-boss-v50':''}" data-action="select-enemy" data-enemy="${esc(e.uid)}"><div class="enemy-status-v51 enemy-status-v52"><div><b>${esc(e.name)}</b><small>${esc(e.element||'')} ${enemyWeakBadge(e.element)}</small></div><span>${e.hp}/${e.maxHp}</span><i><em style="width:${pct(e.hp,e.maxHp)}%"></em></i></div><div class="enemy-sprite-v51 enemy-sprite-v52"><i class="enemy-shadow-v41"></i><img class="enemy-sprite" src="${esc(e.sprite)}" alt="${esc(e.name)}" decoding="async"><span class="target-ring-v41"></span></div><div class="enemy-intent-v51 enemy-intent-v52"><small>NEXT</small><b>${esc(it.icon)} ${esc(it.value||'')}</b></div></button>`;}).join('');

    const rootMenu=`<div class="root-command-v51 root-command-v52"><button class="fight" data-action="battle-menu" data-menu="moves" ${selectedMon?.acted?'disabled':''}><span>⚔</span><b>기술</b><small>몬스터 기술</small></button><button class="spell" data-action="battle-menu" data-menu="spells"><span>✦</span><b>스펠</b><small>${me.tacticsUsed||0}/${me.tacticLimit||2}</small></button><button class="party" data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length||selectedMon?.acted?'disabled':''}><span>↔</span><b>교대</b><small>${(me.bench||[]).length} 대기</small></button><button class="capture" data-action="battle-menu" data-menu="capture" ${!['wild','boss'].includes(b.encounterType)||selectedMon?.acted?'disabled':''}><span>◉</span><b>봉인</b><small>${selectedEnemy?`${Math.round(Number(selectedEnemy.hp||0)/Math.max(1,Number(selectedEnemy.maxHp||1))*100)}% HP`:'대상 없음'}</small></button></div>`;
    const moves=selectedMon?.moves||[];
    const moveMenu=`<div class="move-command-v51 move-command-v52"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>${esc(selectedMon?.name||'MONSTER')}</small><b>기술을 선택하세요</b></div></div><div class="move-grid-v51 move-grid-v52">${[0,1,2,3].map(i=>{const mv=moves[i];if(!mv)return `<button class="move-btn-v51 move-btn-v52 empty" disabled><em>${i+1}</em><b>비어 있음</b></button>`;const can=moveUsable(mv,selectedMon,me),power=movePowerLine(mv);return `<button class="move-btn-v51 move-btn-v52 element-${elementClass(mv.element||selectedMon.element)}" data-action="move-use" data-move-id="${esc(mv.id)}" ${can?'':'disabled'}><em>${i+1}</em><span>${namedSkillGlyphV47(namedMoveFamily({skill:mv.name,element:mv.element||selectedMon.element,move:mv}))}</span><div><b>${esc(mv.name)}</b><small>${esc(power||moveKindKo(mv))}</small></div><strong>${Number(mv.cooldownRemaining||0)>0?`CD ${mv.cooldownRemaining}`:`${mv.accuracy||100}%`}</strong></button>`;}).join('')}</div></div>`;
    const spellMenu=`<div class="spell-command-v51 spell-command-v52"><div class="subcommand-head-v51 subcommand-head-v52"><button data-action="battle-menu" data-menu="root">←</button><div><small>TACTIC SPELL</small><b>스펠을 선택하세요 · ${me.tacticsUsed||0}/${me.tacticLimit||2}</b></div></div><div class="spell-list-v51 spell-list-v52">${me.hand.map((cid,i)=>{const c=runCardView(byId(state.meta.cards,cid),run),cost=Math.max(0,c.cost-(me.buffs.anyDiscount>0?1:(me.buffs.spellDiscount>0?1:0))),can=!me.ended&&b.phase==='players'&&Number(me.tacticsUsed||0)<Number(me.tacticLimit||2)&&me.energy>=cost;return `<button data-action="play-card" data-index="${i}" ${can?'':'disabled'}><em>${i+1}</em><span>${spellSigilIcon(c)}</span><div><b>${esc(c.name)}</b><small>${esc(cardKeywords(c,2).join(' · ')||c.element)}</small></div><strong>${cost}E</strong></button>`;}).join('')}</div></div>`;
    ...
    const prompt=menu==='root'?'무엇을 할까?':menu==='moves'?'어떤 기술을 사용할까?':menu==='spells'?'어떤 스펠을 사용할까?':menu==='party'?'누구와 교대할까?':menu==='capture'?'어떤 봉인구를 사용할까?':'전투 정보';
  ...
  function fieldServiceHtml(r,run){
    ...
    return `<div class="field-service-v51"><header><small>FIELD SUPPORT</small><b>정비소</b><span>필요할 때만 이용하세요.</span></header>${svc('mend','♥','응급 회복','CORE + 생존 몬스터 회복',damaged)}${svc('revive','✚','부활 캡슐','쓰러진 몬스터 1체 부활',ko)}${svc('charge','✦','아르카나 충전',`다음 전투 시작 강화 · ${charge}/2`,charge<2)}</div>`;
  }
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='move-use'){if(state.busy)return;const me=state.room?.battle?.party?.find(p=>p.playerId===state.profileId),u=me?.units?.find(x=>x.instanceId===state.selectedMonster)||me?.units?.find(x=>x.hp>0&&!x.acted)||me?.units?.[0];if(!u)return;const d=await roomPost('move',{instanceId:u.instanceId,moveId:el.dataset.moveId,targetUid:state.selectedEnemy});if(d.room?.status==='reward'){state.battleMenu='root';sound('win');return;}const nextMe=d.room?.battle?.party?.find(p=>p.playerId===state.profileId),next=nextMe?.units?.find(x=>x.hp>0&&!x.acted);if(next){state.selectedMonster=next.instanceId;state.battleMenu='moves';showCenterBanner(next.name,'다음 몬스터의 기술을 선택하세요','turn');renderBattle(d.room);}else state.battleMenu='root';return;}
      ...
      if(action==='open-field-service')return showFieldServiceMenuV51();
      if(action==='play-card'){
