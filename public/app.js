...
(() => {
  ...
  function renderRoute(r){
    const run=r.runState[state.profileId],node=r.route?.[0],voted=node?.votes?.includes(state.profileId),pressure=(r.threatTokens||[]).length,claim=r.contractClaims?.[state.profileId];
    const kind=node?.kind||'combat',tag=kind==='boss'?'BIOME BOSS':kind==='elite'?'WARDEN BATTLE':'WILD ENCOUNTER';
    ...
    const content=`<div class="encounter-gate-v42 kind-${esc(kind)}">${waveTracker(r,'BIOME RUN')}<div class="encounter-core-v42">${routeGraphic(kind)}<small>${esc(node?.kicker||tag)}</small><h1>${esc(node?.label||'야생 조우')}</h1><p>${esc(node?.desc||'다음 몬스터 조우를 시작합니다.')}</p><div class="encounter-tags-v42"><span>${r.floor}F</span><span>${esc(r.biome.name)}</span>${pressure?`<span>PRESSURE ×${pressure}</span>`:''}</div><button class="encounter-enter-v42 ${voted?'waiting':''}" data-action="route-vote" data-node="${esc(node?.id||'')}" ${!claim||voted?'disabled':''}>${!claim?'먼저 STARTER CORE 선택':voted?'동료 준비 대기 중…':'조우 시작'}</button></div></div>`;
    ...
    if(claim&&node&&!voted)queueAutoEncounter(r,node);
  }
  ...
  function renderReward(r){
    ...
    const support=rw.kind==='battle'&&!capturePending&&!skillOffer?fieldServiceHtml(r,run):'';
