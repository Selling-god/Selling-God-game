...
(() => {
  ...
  const BATTLE_SPEED_V73={slow:1.82,normal:1.48,fast:1.00};
  ...
  function itemMetaV80(it){if(!it)return'';const bits=[];const h=it.held||null;if(h?.kind==='evolutionCatalyst')bits.push('진화 촉매 · 대상 몬스터 전용');else if(h)bits.push(h.consume?'1회 자동 발동':'몬스터 장착');if(h?.kind==='xpBoost')bits.push(`EXP +${Math.round(Number(h.xpPct||0)*100)}%`);if(h?.kind==='lowHpHeal')bits.push(`HP ${Math.round(Number(h.threshold||.5)*100)}%↓ 자동 회복`);if(h?.kind==='statusCure')bits.push('상태이상 자동 회복');if(h?.kind==='lowPpRestore')bits.push('PP 부족 시 자동 회복');return bits.join(' · ');}
  function rewardEquipEligibleV82(item,mon){if(!item?.held||!mon)return false;const ids=Array.isArray(item.held.speciesIds)?item.held.speciesIds:[];return item.held.kind!=='evolutionCatalyst'||!ids.length||ids.includes(mon.speciesId);}
  function showRewardEquipPickerV82(rewardId){
    const r=state.room,rw=r?.reward,run=r?.runState?.[state.profileId],opt=(rw?.playerOptions?.[state.profileId]||[]).find(x=>x.id===rewardId),it=opt?.item||byId(state.meta.items,opt?.itemId);if(!r||!run||!opt||!it?.held)return toast('장착할 수 있는 보상 아이템을 찾지 못했습니다.','error');
  ...
  function rewardTokenV64(o){
    ...
    const it=o.item;
    const meta=itemMetaV80(it);return `<button class="afterbattle-token-v64 reward-token-v64 reward-card-v71 rarity-${esc(it.rarity)} ${it.held?'held-reward-v80':''} ${it.category==='진화재료'?'evo-material-reward-v80':''}" data-action="reward-pick" data-reward-id="${esc(o.id)}" data-reward-focus-v64="1" data-focus-name="${esc(it.name)}" data-focus-meta="${esc(itemMetaV80(it)||it.category||rarityKo(it.rarity))} · 무료 보상" data-focus-desc="${esc(it.text||'')}">${rewardArtV71(it)}<div class="reward-copy-v71"><b>${esc(it.name)}</b>${meta?`<i>${esc(meta)}</i>`:''}<em>${esc(it.text||'효과 정보 없음')}</em></div><small>FREE</small></button>`;
  }
  ...
  function renderReward(r){
    ...
    let step='REWARD',body='',actions='',normalShop=false,focusName='전투가 끝났다.',focusMeta='WAVE CLEAR',focusDesc='상점 정비 후 무료 장착 아이템 하나를 고르고, 이어서 장착할 몬스터를 선택하세요.';
