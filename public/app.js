...
(() => {
  ...
  function monsterMiniCardHtml(mon,{ownedCount=0,featured=false}={}){
    if(!mon)return '';
    const tag=featured?'<span class="pickup-tag">PICK UP</span>':'';
    return `<article class="monster-mini-card rarity-${esc(mon.rarity||mon.tier||'common')} ${featured?'featured':''}">${tag}<div class="monster-mini-top"><span class="monster-mini-type">${esc(rarityKo(mon.rarity||mon.tier||'common'))}</span><span class="monster-mini-element">${esc(mon.element||'')}</span></div><div class="monster-mini-art"><img src="${esc(mon.sprite||'')}" alt=""></div><b>${esc(mon.name)}</b><small>${esc(mon.evolutionItemName?`진화 재료: ${mon.evolutionItemName}`:mon.skill||'전투 몬스터')}</small>${ownedCount?`<em>보유 ×${ownedCount}</em>`:''}</article>`;
  }
  ...
  function showEvolutionMenu(instanceId){const room=state.room,me=room?.battle?.party?.find(p=>p.playerId===state.profileId),u=allMyBattleMonstersV56().find(x=>x.instanceId===instanceId),run=room?.runState?.[state.profileId]||{};if(!u)return;const req=monsterById(u.speciesId);const canNatural=!u.evolved&&u.level>=(u.evolutionLevel||8);const materialId=u.evolutionItemId||req?.evolutionItemId||null;const materialName=u.evolutionItemName||req?.evolutionItemName||'';const materialCount=materialId?Number(run.items?.[materialId]||0):0;const canMaterial=!u.evolved&&materialId&&materialCount>0;modalRoot.innerHTML=`<div class="modal"><div class="modal-card"><div class="section-label">MONSTER EVOLUTION</div><h3>${esc(u.name)} 진화</h3><p>현재 레벨 ${u.level} · 필요 레벨 ${u.evolutionLevel||8}${materialId?` · 재료 ${esc(materialName)} ${materialCount}개 보유`:''}</p><div class="modal-actions"><button class="cta" data-action="monster-evolve" data-mode="natural" data-monster="${u.instanceId}" ${canNatural?'':'disabled'}>일반 진화</button><button class="cta gold" data-action="monster-evolve" data-mode="accelerate" data-monster="${u.instanceId}" ${(me?.gene||0)>=2?'':'disabled'}>유전자 촉진 (2)</button>${materialId?`<button class="cta mint" data-action="monster-evolve" data-mode="material" data-monster="${u.instanceId}" ${canMaterial?'':'disabled'}>${esc(materialName)} 사용</button>`:''}<button class="ghost" data-action="close-modal">닫기</button></div></div></div>`;openModal();}
  ...
  function renderBattle(r){
    ...
    const monActors=me.units.map(u=>{const held=heldItemClient(u);return `<button class="active-mon-v41 active-mon-v44 active-mon-v51 active-mon-v52 ${state.selectedMonster===u.instanceId?'selected':''} ${u.acted?'acted':''}" data-action="select-monster" data-monster="${esc(u.instanceId)}" ${u.hp<=0?'disabled':''}><div class="mon-status-v51 mon-status-v52"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)}${held?` <i class="battle-held-v53" title="${esc(held.text||'')}">${esc(held.icon||'◆')} ${esc(held.name)}</i>`:''}${playerStatusBadgeV56(u)}${statStageBadgesV56(u)}</small></div><span>${u.hp}/${u.maxHp}</span><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><div class="mon-sprite-v51 mon-sprite-v52"><i class="mon-shadow-v41"></i>${u.fusionSprite&&!u.fused?`<img class="fusion-ghost-v40" src="${esc(u.fusionSprite)}" alt="">`:''}<img src="${esc(monsterDisplaySprite(u))}" alt="${esc(u.name)}" decoding="async" data-hires-v72="1"><span class="mon-ring-v41"></span></div>${u.acted?'<em class="acted-mark-v51">완료</em>':''}</button>`;}).join('');
  ...
  function rewardTokenV64(o){
    ...
    const it=o.item;
    return `<button class="afterbattle-token-v64 reward-token-v64 reward-card-v71 rarity-${esc(it.rarity)}" data-action="reward-pick" data-reward-id="${esc(o.id)}" data-reward-focus-v64="1" data-focus-name="${esc(it.name)}" data-focus-meta="${esc(it.category||rarityKo(it.rarity))} · 무료 보상" data-focus-desc="${esc(it.text||'')}">${rewardArtV71(it)}<div class="reward-copy-v71"><b>${esc(it.name)}</b><em>${esc(it.text||'효과 정보 없음')}</em></div><small>FREE</small></button>`;
  }
  function merchantTokenV64(r,x){
    ...
    return `<button class="afterbattle-token-v64 shop-token-v64 reward-card-v71 rarity-${esc(obj.rarity)}" data-action="buy" data-item="${esc(x.id)}" data-type="${esc(x.type)}" data-reward-focus-v64="1" data-focus-name="${esc(obj.name)}" data-focus-meta="${esc(x.type==='card'?'RIFT COMMAND':obj.category||'SHOP')} · ${Number(x.price).toLocaleString('ko-KR')}G" data-focus-desc="${esc(obj.text||'')}" ${disabled?'disabled':''}>${rewardArtV71(obj)}<div class="reward-copy-v71"><b>${esc(obj.name)}</b><em>${esc(obj.text||'효과 정보 없음')}</em></div><small>${bought?'SOLD':`${Number(x.price).toLocaleString('ko-KR')}G`}</small></button>`;
  }
  ...
  function renderGacha(){const b=state.meta.banner,mb=state.meta.monsterBanner||{},featured=b.featured.map(id=>byId(state.meta.cards,id)).filter(Boolean),pickup=mb.featured||null,archiveRates=Object.entries(mb.rates||{}).reverse();setScreen('gacha',`<section class="gacha-page dual-gacha-page"><div class="gacha-hero"><div class="page-head"><button class="back-btn" data-action="home">← 홈</button><div><div class="section-label">ARCHIVE HUB</div><h1>소환 아카이브</h1><p>카드 복각 소환과 5일마다 바뀌는 전설 몬스터 픽업을 한곳에서 관리합니다.</p></div></div><div class="gacha-dual-grid"><div class="gacha-banner"><div class="gacha-copy"><div class="section-label">LIMITED CARD BANNER</div><h1>${esc(b.name)}</h1><p>${esc(b.subtitle)}. 높은 등급이 나왔을 때 픽업 카드가 등장할 확률이 상승합니다. 중복 카드는 잔광으로 환급됩니다.</p><div class="rate-table">${Object.entries(b.rates).reverse().map(([k,v])=>`<div class="rate-cell"><b class="rarity-${k}">${esc(rarityKo(k))}</b>${(v*100).toFixed(1)}%</div>`).join('')}</div><div class="pity">전설 천장 ${state.profile.pity.legendary}/${b.pity.legendary} · 신화 천장 ${state.profile.pity.mythic}/${b.pity.mythic}</div><div class="banner-actions"><button class="cta" data-action="gacha-pull" data-count="1">1회 · ◆${b.singleCost}</button><button class="cta gold" data-action="gacha-pull" data-count="10">10회 · ◆${b.tenCost}</button></div></div><div class="gacha-art">${featured.map(c=>cardHtml(c)).join('')}</div></div><div class="gacha-banner monster-banner-panel"><div class="gacha-copy"><div class="section-label">LEGEND PICK UP</div><h1>전설 몬스터 소환</h1><p>여행 중 봉인하던 몬스터를 프리즘으로 소환할 수 있습니다. 픽업 전설은 5일마다 변경됩니다.</p><div class="monster-banner-meta"><span>현재 프리즘 ◆${Number(state.profile.gems||0).toLocaleString('ko-KR')}</span><span>픽업 교체 ${mb.endsAt?new Date(mb.endsAt).toLocaleDateString('ko-KR'):'-'}</span><span>천장 ${state.profile.monsterArchivePity||0}/${mb.pityLegendary||10}</span></div><div class="rate-table">${archiveRates.map(([k,v])=>`<div class="rate-cell"><b class="rarity-${k}">${esc(rarityKo(k))}</b>${(v*100).toFixed(1)}%</div>`).join('')}</div><div class="banner-actions"><button class="cta mint" data-action="monster-archive-pull" data-count="1">1회 · ◆${mb.singleCost||180}</button><button class="cta gold" data-action="monster-archive-pull" data-count="10">10회 · ◆${mb.tenCost||1700}</button></div></div><div class="monster-pickup-showcase">${pickup?monsterMiniCardHtml(pickup,{ownedCount:Number(state.profile.monsters?.[pickup.id]||0),featured:true}):'<div class="small-note">픽업 정보 없음</div>'}</div></div></div></div></section>`);}
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='monster-archive-pull')return doMonsterArchivePull(el.dataset.count);
