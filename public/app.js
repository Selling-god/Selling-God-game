...
(() => {
  ...
  function enemyAttackEventFx(payload){
    ...
    const heavy=!!payload.heavy,baseStyle=payload.style||'beast',style=moveFxStyle(payload,baseStyle),fake=payloadFxCard(payload,heavy||Number(payload?.damage||0)>=18);
    attackLabelV40(actor,payload.skill||'ENEMY ATTACK','enemy');attackAfterimageV40(actor,-1);actor.classList.add('attacking-v40');
    try{actor.animate(motionFramesV40(baseStyle,-1,heavy),{duration:heavy?760:610,easing:'cubic-bezier(.14,.82,.16,1)'});}catch{}
    setTimeout(async()=>{
      if(style==='heal'){healBloomFx(actor,fake);signatureImpactFx(actor,fake,.9);}
      else if(style==='guard'){shieldGainFx(actor,Math.max(6,Number(payload?.damage||0)));signatureImpactFx(actor,fake,.92);}
      else if(style==='eruption'){await eruptionFx(target,fake);}
      else if(style==='beam'){await beamFx(actor,target,fake);}
      else if(style==='projectile'){await signatureProjectileFx(actor,target,fake);signatureImpactFx(target,fake,heavy?1.55:1.10);}
      else if(style==='wave'){await waveFx(actor,target,fake);}
      else {await meleeArcFx(target,fake,heavy||['assassin','knight','crab'].includes(baseStyle));}
      if(style!=='heal'&&style!=='guard')popNumber(target,`-${payload.damage||0}`,'damage player');screenShake(heavy?'heavy':'soft');
    },style==='beam'?220:200);
  ...
  function renderBattle(r){
    ...
    const rootMenu=`<div class="root-command-v41 root-command-v44"><button class="fight" data-action="battle-menu" data-menu="moves"><span>⚔</span><b>기술</b><small>${selectedMon?.acted?'행동 완료':'전투 기술 4개'}</small></button><button data-action="battle-menu" data-menu="spells"><span>✦</span><b>스펠</b><small>전술 ${me.tacticsUsed||0}/${me.tacticLimit||2}</small></button><button data-action="battle-menu" data-menu="party" ${!(me.bench||[]).length?'disabled':''}><span>↔</span><b>교대</b><small>${(me.bench||[]).length} 대기</small></button><button data-action="battle-menu" data-menu="info"><span>◎</span><b>정보</b><small>상성 · 진화</small></button></div>`;
    const moves=selectedMon?.moves||[];
    const moveMenu=`<div class="move-command-v41 move-command-v44"><div class="subcommand-head-v41"><button data-action="battle-menu" data-menu="root">←</button><b>${esc(selectedMon?.name||'MONSTER')} · 기술</b><span>${selectedMon?.acted?'행동 완료':'대상을 선택하고 기술 사용'}</span></div><div class="move-grid-v41 move-grid-v44">${[0,1,2,3].map(i=>{const mv=moves[i];if(!mv)return`<button disabled><em>0${i+1}</em><b>EMPTY</b></button>`;const can=moveUsable(mv,selectedMon,me),mastery=moveMasteryUi(mv);return `<button class="move-btn-v41 move-btn-v44 kind-${esc(mv.kind||'attack')} element-${elementClass(mv.element||selectedMon.element)}" data-action="move-use" data-move-id="${esc(mv.id)}" ${can?'':'disabled'}><em>0${i+1}</em><span>${moveKindIcon(mv)}</span><b>${esc(mv.name)}${mastery.label?` <i>${mastery.label}</i>`:''}</b><small>${esc(movePowerLine(mv))}</small><strong>${esc(moveStatus(mv,selectedMon))}</strong><u class="move-mastery-v44"><i style="width:${mastery.p}%"></i></u></button>`;}).join('')}</div></div>`;
  ...
  function renderReward(r){
    ...
    const capture=r.mode==='journey'&&r.capture&&!r.capture.escaped?r.capture:null,capturePending=!!capture&&!capture.attemptedBy?.includes(pid);const afterCore=campDone&&skillDone&&rewriteDone&&(!relicOpts.length||!!relicClaim)&&rewardDone;let step='REWARD',body='',actions='';
