...
(() => {
  ...
  function showMonsterPullV80(results){const top=[...results].sort((a,b)=>(state.meta.rarities[b.rarity]?.order||0)-(state.meta.rarities[a.rarity]?.order||0))[0];sound(['legendary','mythic'].includes(top?.rarity)?'gacha':'reward');modalRoot.innerHTML=`<div class="pull-result monster-pull-result-v80 monster-pull-result-v82"><div class="pull-result-inner"><div class="summon-orb"></div><div class="section-label">MONSTER ARCHIVE</div><h2>몬스터 소환 결과</h2><div class="monster-pull-grid-v80 monster-pull-grid-v82">${results.map(m=>monsterPullCardV80(m,state.meta.monsterPickup?.featuredId===m.id)).join('')}</div><button class="cta mint close-pull" data-action="close-pull">확인</button></div></div>`;}
