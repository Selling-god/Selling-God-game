...
(() => {
  ...
  const state = {
    ...
    lastBattleKey: '',
    autoEncounterKey: '',
    autoEncounterTimer: 0,
    encounterStarting: false,
    rewardConfirmTimer: 0,
    lastCastFx: null,
    battleEntranceKey: '',
    battleExitTimer: 0,
  ...
  function updateTopbar(){
    ...
    const coin=$('#coinCount');if(coin)coin.textContent=Number(state.profile.coins||0).toLocaleString('ko-KR');
    ...
    $('#versionLabel').textContent=`FUSEWILD v${state.meta?.version||'6'}`;
  }
  ...
  const BATTLE_SPEED_V73={slow:1.72,normal:1.34,fast:.92};
  const BATTLE_PACE_V57={turn:650,announce:850,move:980,signature:1220,result:780,status:900,blocked:760,win:1280,minor:650};
  ...
  function battleNarrateV57(text,{tone='normal',tag='BATTLE',sub='',instant=false}={}){
    ...
    if(instant||chars.length<2){b.textContent=chars.join('');return;}let i=0;const speed=Math.max(34,Math.min(58,Math.round((1180*battlePaceScaleV73())/Math.max(1,chars.length))));
  ...
  function canvasMoveFxV57(actor,target,family,payload,enemySide=false){
    if(!fxOn()||!actor||!target)return;const dpr=Math.min(1.35,window.devicePixelRatio||1),cv=document.createElement('canvas'),ctx=cv.getContext('2d');if(!ctx)return;cv.className=`combat-fx-canvas-v57 fx-${family} ${payload?.signature?'signature':''}`;cv.width=Math.round(innerWidth*dpr);cv.height=Math.round(innerHeight*dpr);cv.style.width=`${innerWidth}px`;cv.style.height=`${innerHeight}px`;document.body.appendChild(cv);ctx.scale(dpr,dpr);
    const A=rectCenter(actor),T=rectCenter(target),[c1,c2]=fxColorV57(payload?.element||payload?.move?.element||'공허'),rng=seededV57(payload?.fxSeed||`${family}:${payload?.skill}`),sig=!!payload?.signature||!!payload?.move?.signature,dur=sig?1650:family==='beam'||family==='meteor'?1350:1150,variant=Number(payload?.fxVariant||0)%8,parts=Array.from({length:sig?22:14},()=>({r:rng(),a:rng()*Math.PI*2,s:.5+rng()*1.2,x:rng(),y:rng()}));
  ...
  function playRoomEvents(next,afterSeq,{freshAction=false}={}){
    ...
    for(const ev of events){const p=ev.payload||{};
      ...
      else if(ev.type==='monster-faint')later(()=>{const t=p.instanceId?$(`[data-monster="${CSS.escape(p.instanceId)}"]`):null;if(t){t.classList.add('fainted-v70');stageDefeatVisualV62(t);}battleNarrateV57(`${p.monsterName||'몬스터'}은(는) 더 이상 싸울 수 없다!`,{tag:'FAINTED',tone:'bad'});screenShake('medium');},BATTLE_PACE_V57.status);
      else if(ev.type==='replacement-required'&&p.playerId===state.profileId)later(()=>{state.battleMenu='replacement';battleNarrateV57('다음에 출전할 몬스터를 선택하세요.',{tag:'CHOOSE NEXT',tone:'normal'});showCenterBanner('CHOOSE NEXT','다음 몬스터를 선택하세요','turn');if(state.room?.battle?.phase==='replacement')renderBattle(state.room);},BATTLE_PACE_V57.result);
      else if(ev.type==='monster-replacement')later(()=>{battleNarrateV57(`${p.monsterName||'몬스터'}, 나와줘!`,{tag:'GO!',tone:'good'});showCenterBanner(p.monsterName||'GO!','BATTLE CONTINUES','turn');},BATTLE_PACE_V57.status);
  ...
  async function playNamedMonsterMoveV45(payload,enemySide=false){
    ...
    const actorDur=signature?1180:tempo==='charge'?980:tempo==='multi'?900:790;try{actor.animate(namedActorFramesV47(family,dir,variant),{duration:actorDur,easing:'cubic-bezier(.14,.82,.16,1)'});}catch{}
    const windup=signature?410:tempo==='charge'?330:family==='beam'?290:230;
    setTimeout(async()=>{
      if(signature)monsterSignatureFxV532(['heal','guard'].includes(family)?actor:target,payload,'impact');
      if(family==='heal'){healBloomFx(actor,card);namedSkillOverlayV45(actor,family,payload);}
      else if(family==='guard'){shieldGainFx(actor,Number(mv.shield||8));namedSkillOverlayV45(actor,family,payload);}
    ...
    },windup);setTimeout(()=>actor.classList.remove('attacking-v40'),actorDur+260);
  }

  function organicMoveFxV71(actor,target,family,payload,enemySide=false){
    if(!fxOn()||!actor||!target)return;const root=fxRoot(),A=rectCenter(actor),T=rectCenter(target),el=document.createElement('div'),seed=skillHash(`${payload?.fxSeed||''}:${payload?.skill||''}:${family}`),sig=!!payload?.signature||!!payload?.move?.signature;
    el.className=`organic-move-fx-v71 family-${family} ${sig?'signature':''}`;el.style.setProperty('--sx',`${A.x}px`);el.style.setProperty('--sy',`${A.y}px`);el.style.setProperty('--tx',`${T.x}px`);el.style.setProperty('--ty',`${T.y}px`);el.style.setProperty('--dx',`${T.x-A.x}px`);el.style.setProperty('--dy',`${T.y-A.y}px`);
    const count=sig?24:16;let html='<b class="organic-core-v71"></b><strong class="organic-glyph-v71">'+namedSkillGlyphV47(family)+'</strong>';
    for(let i=0;i<count;i++){const a=((seed+i*47)%360),r=28+((seed>>>((i%4)*4))%74),d=220+((seed+i*83)%540),sz=5+((seed+i*29)%13);html+=`<i style="--a:${a}deg;--r:${r}px;--d:${d}ms;--sz:${sz}px;--i:${i}"></i>`;}
    html+='<em></em><span></span>';el.innerHTML=html;root.appendChild(el);requestAnimationFrame(()=>el.classList.add('go'));setTimeout(()=>el.remove(),sig?2100:1550);
  }
  function moveSpectacleV56(actor,target,family,payload,enemySide=false){
    ...
    el.innerHTML=`<span class="fx-glyph-v56">${glyph}</span>${Array.from({length:family==='bullet'?10:family==='meteor'?8:family==='spore'?12:6},(_,i)=>`<i style="--i:${i}"></i>`).join('')}<b></b><em></em>`;root.appendChild(el);setTimeout(()=>el.classList.add('go'),10);setTimeout(()=>el.remove(),payload?.signature?1900:1450);
  }
  ...
  function queueAutoEncounter(r,node){
    ...
    if(state.autoEncounterKey===key&&state.autoEncounterTimer)return;
    clearTimeout(state.autoEncounterTimer);state.autoEncounterKey=key;
    state.autoEncounterTimer=setTimeout(async()=>{
      state.autoEncounterTimer=0;
  ...
  function monsterDisplaySprite(u){if(!u)return'';if(u.fused)return fusionCompositeSpriteUrl(u);if(u.abyssBloom)return u.riftSprite||u.sprite;if(Number(u.resonanceTurns||0)>0)return u.resonanceSprite||u.sprite;if(u.evolved)return u.evolutionSprite||u.sprite;return u.sprite||u.baseSprite||'';}
  ...
  function scheduleRewardAutoAdvanceV54(){
    clearTimeout(state.rewardAutoTimer);state.rewardAutoTimer=0;
    // v6.0: never leave the reward/shop screen on a timer. The free reward is the commit action.
  }
  ...
  function playerStatusBadgeV56(u){const bits=[];if(u?.fused)bits.push(`<i class="player-major-status-v56 fusion-turns-v60" title="융합은 최대 5턴 유지되며 전투 종료 시 즉시 해제됩니다.">∞ ${Math.max(0,Number(u.fusionTurnsLeft||5))}T</i>`);const row=PLAYER_STATUS_META_V56[u?.majorStatus];if(row)bits.push(`<i class="player-major-status-v56 status-${esc(u.majorStatus)}" title="${esc(row[0])}">${esc(row[1])} ${esc(row[0])}</i>`);return bits.join('');}
  ...
  function openFusionBuilderV56(primaryId,secondaryId){
    const all=allMyBattleMonstersV56(),a=all.find(x=>x.instanceId===primaryId),b=all.find(x=>x.instanceId===secondaryId);if(!a||!b||a.instanceId===b.instanceId||a.fused||b.fused)return toast('융합 상태의 몬스터는 해제된 뒤 다시 융합할 수 있습니다.','error');
  ...
  function showEvolutionMenu(instanceId){
    ...
    const run=state.room?.runState?.[state.profileId]||{},species=monsterById(u.speciesId),evoLevel=Number(u.evolutionLevel||18),itemId=u.evolutionItemId||species?.evolutionItemId,itemName=u.evolutionItemName||species?.evolutionItemName||'',itemCount=itemId?Number(run.items?.[itemId]||0):0,itemOnly=Boolean(u.evolutionItemOnly||species?.evolutionItemOnly),naturalReady=!itemOnly&&Number(u.level||1)>=evoLevel,earlyReady=!itemOnly&&Number(u.level||1)>=3&&Number(u.gene||0)>=3,evoOk=!u.evolved&&(naturalReady||earlyReady),itemOk=!u.evolved&&!!itemId&&itemCount>0,resOk=Number(u.resonanceTurns||0)<=0&&u.resonance>=4,riftOk=!u.abyssBloom&&u.rift>=3&&u.hp/u.maxHp<=.6;
    const evoText=itemOnly?`${itemName}을(를) 사용해야 진화합니다. 현재 ${itemCount}개 보유 중입니다.`:`자연 진화는 Lv.${evoLevel}에 자동으로 발생합니다. GENE을 모으면 더 일찍 진화시킬 수도 있습니다.`;
    modal(`<div class="evolution-menu-v41 evolution-menu-v43 evo-material-v80"><div class="section-label">MONSTER EVOLUTION</div><div class="evo-hero-v41"><img src="${esc(monsterDisplaySprite(u))}"><div><small>${esc(u.element)} · Lv.${u.level}</small><h2>${esc(u.name)}</h2><p>${esc(evoText)}</p></div></div><div class="evo-grid-v41">${itemId?`<button class="material-evo-v80" data-action="monster-evolve" data-mode="item" data-monster="${esc(u.instanceId)}" ${itemOk?'':'disabled'}><span>✧</span><b>${esc(itemName)} 진화</b><small>보유 ${itemCount}개 · 사용 시 1개 소모</small></button>`:''}<button data-action="monster-evolve" data-mode="evolve" data-monster="${esc(u.instanceId)}" ${evoOk?'':'disabled'}><span>▲</span><b>${itemOnly?'재료 필요':naturalReady?'자연 진화':'조기 진화'}</b><small>${itemOnly?esc(itemName):naturalReady?`Lv.${evoLevel} 달성`:`Lv.${evoLevel} 자동 · 또는 GENE 3`}</small></button><button data-action="open-fusion" data-monster="${esc(u.instanceId)}" ${(me.units.length+me.bench.length>1)&&!u.acted&&!u.fused?'':'disabled'}><span>∞</span><b>턴 융합</b><small>5턴 유지 · 전투 종료 해제</small></button><button data-action="monster-evolve" data-mode="resonance" data-monster="${esc(u.instanceId)}" ${resOk?'':'disabled'}><span>✦</span><b>공명진화</b><small>RES 4 · 3턴</small></button><button data-action="monster-evolve" data-mode="rift" data-monster="${esc(u.instanceId)}" ${riftOk?'':'disabled'}><span>◆</span><b>균열개화</b><small>HP 60%↓ · RIFT 3</small></button></div><button class="cta" data-action="modal-close">닫기</button></div>`);
  }
  ...
  function showHeldItemMenuV53(instanceId){
    ...
    const choices=held.map(({item,n})=>{const free=Math.max(0,n-Number(equipped[item.id]||0)),on=current?.id===item.id,consume=!!item.held?.consume;return `<button class="held-choice-v53 ${on?'on':''}" data-action="equip-held-item" data-monster="${esc(mon.instanceId)}" data-item="${esc(item.id)}" ${free<1&&!on?'disabled':''}><span>${esc(item.icon||'◆')}</span><div><b>${esc(item.name)}</b><small>${esc(item.text)}</small><em>${consume?'1회 발동 후 소모':'지속 장착 효과'}</em></div><strong>${on?'장착 중':`사용 가능 ${free}`}</strong></button>`;}).join('');
  ...
  function renderRoute(r){
    ...
    if(!claim){
      ...
      const glyph=node?.encounterType==='boss'?'♛':node?.encounterType==='rival'?'⚔':node?.encounterType==='warden'?'◆':'◇';decision=`<div class="wave-bridge-v61 wave-bridge-v80"><div class="bridge-signal-v80"><i></i><strong>${glyph}</strong><i></i></div><small>${esc(scene)} · WAVE ${String(wave).padStart(2,'0')}/10</small><h1>${esc(nextKind)}</h1><b>${voted?'원정대 동기화 중':'다음 조우로 진입합니다'}</b><span>${voted?'다른 원정대원의 준비가 끝나면 함께 이동합니다.':'몬스터와 장착 아이템을 확인했습니다. 전투를 준비하세요.'}</span><div class="bridge-party-v80">${(run.monsters||[]).slice(0,6).map(m=>`<span class="${m.hp<=0?'ko':''}"><img src="${esc(monsterDisplaySprite(m))}" alt=""><i style="width:${pct(m.hp,m.maxHp)}%"></i></span>`).join('')}</div>${!voted?`<i class="bridge-progress-v80" style="--bridge-ms:${delay}ms"></i>`:''}</div>`;
  ...
  function renderBattle(r){
    ...
    const replacementMenu=`<div class="replacement-command-v70 replacement-command-v71"><div class="replacement-head-v70"><small>FAINTED · PARTY SELECT</small><b>${esc(pendingReplacement?.faintedName||'몬스터')} 대신 누가 나갈까?</b><span>다음 몬스터를 직접 선택하면 전투가 이어집니다.</span></div><div class="replacement-grid-v70">${(me.bench||[]).filter(u=>u.hp>0).map(u=>`<button data-action="replace-monster" data-bench="${esc(u.instanceId)}" data-slot="${Number(pendingReplacement?.slotIndex||0)}"><img src="${esc(monsterDisplaySprite(u))}" alt=""><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)} · HP ${u.hp}/${u.maxHp}</small><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><strong>출전</strong></button>`).join('')||'<p>출전 가능한 몬스터가 없습니다.</p>'}</div></div>`;
    const command=menu==='replacement'?replacementMenu:menu==='moves'?moveMenu:menu==='capture'?captureMenu:menu==='fusion'?fusionMenu:menu==='party'?partyMenu:menu==='info'?infoMenu:rootMenu;
  ...
  function rewardArtV71(obj){
    const art=obj?.art||'',glyph=esc(obj?.icon||'◆');
    return art?`<span class="reward-art-v71"><img src="${esc(art)}" alt="" data-img-fallback-v71="1"><i>${glyph}</i></span>`:`<span class="reward-art-v71 fallback"><i>${glyph}</i></span>`;
  }
  function itemMetaV80(it){if(!it)return'';const bits=[];if(it.category==='진화재료')bits.push('특수 진화');if(it.held)bits.push(it.held.consume?'1회용 장착':'몬스터 장착');if(it.held?.kind==='xpBoost')bits.push(`EXP +${Math.round(Number(it.held.xpPct||0)*100)}%`);return bits.join(' · ');}
  function rewardTokenV64(o){
    ...
    const it=o.item;
    const meta=itemMetaV80(it);return `<button class="afterbattle-token-v64 reward-token-v64 reward-card-v71 rarity-${esc(it.rarity)} ${it.held?'held-reward-v80':''} ${it.category==='진화재료'?'evo-material-reward-v80':''}" data-action="reward-pick" data-reward-id="${esc(o.id)}" data-reward-focus-v64="1" data-focus-name="${esc(it.name)}" data-focus-meta="${esc(it.category||rarityKo(it.rarity))} · 무료 보상" data-focus-desc="${esc(it.text||'')}">${rewardArtV71(it)}<div class="reward-copy-v71"><b>${esc(it.name)}</b>${meta?`<i>${esc(meta)}</i>`:''}<em>${esc(it.text||'효과 정보 없음')}</em></div><small>FREE</small></button>`;
  }
  function merchantTokenV64(r,x){
    const run=r.runState[state.profileId],bought=r.reward?.purchased?.[`${state.profileId}:${x.id}`],obj=x.type==='card'?x.card:x.item,disabled=bought||run.gold<x.price;
    const meta=x.type==='item'?itemMetaV80(obj):'';return `<button class="afterbattle-token-v64 shop-token-v64 reward-card-v71 rarity-${esc(obj.rarity)} ${obj.held?'held-reward-v80':''} ${obj.category==='진화재료'?'evo-material-reward-v80':''}" data-action="buy" data-item="${esc(x.id)}" data-type="${esc(x.type)}" data-reward-focus-v64="1" data-focus-name="${esc(obj.name)}" data-focus-meta="${esc(x.type==='card'?'RIFT COMMAND':obj.category||'SHOP')} · ${Number(x.price).toLocaleString('ko-KR')}G" data-focus-desc="${esc(obj.text||'')}" ${disabled?'disabled':''}>${rewardArtV71(obj)}<div class="reward-copy-v71"><b>${esc(obj.name)}</b>${meta?`<i>${esc(meta)}</i>`:''}<em>${esc(obj.text||'효과 정보 없음')}</em></div><small>${bought?'SOLD':`${Number(x.price).toLocaleString('ko-KR')}G`}</small></button>`;
  }
  function rewardLeadPanelV64(r,run){
    const mons=(run?.monsters||[]).slice(0,6),lead=mons.find(m=>m.hp>0)||mons[0];if(!lead)return'';
    const expRows=r?.reward?.expBy?.[state.profileId]||[],row=expRows.find(x=>x.instanceId===lead.instanceId),hp=pct(lead.hp,lead.maxHp),exp=row?Math.max(0,Math.min(100,Math.round(Number(row.xp||0)/Math.max(1,Number(row.xpNeed||1))*100))):0;
    return `<aside class="afterbattle-lead-v64"><div class="lead-art-v64"><img src="${esc(monsterDisplaySprite(lead))}" alt="${esc(lead.name)}"></div><div class="lead-plate-v64"><div><b>${esc(lead.name)}</b><strong>Lv.${lead.level}</strong></div><small>${esc(lead.element||'')} · HP ${lead.hp}/${lead.maxHp}${row?` · EXP +${row.xpGained}`:''}</small><i class="lead-hp-v64"><em style="width:${hp}%"></em></i>${row?`<i class="lead-exp-v64"><em style="width:${exp}%"></em></i>`:''}</div><div class="afterbattle-party-mini-v64">${mons.map(m=>`<span class="${m.hp<=0?'ko':''}" title="${esc(m.name)}"><img src="${esc(monsterDisplaySprite(m))}" alt=""><i style="width:${pct(m.hp,m.maxHp)}%"></i></span>`).join('')}</div></aside>`;
  }
  function updateRewardFocusV64(el){
  ...
  function renderReward(r){
    ...
    let step='REWARD',body='',actions='',normalShop=false,focusName='전투가 끝났다.',focusMeta='WAVE CLEAR',focusDesc='상점에서 필요한 정비를 한 뒤 무료 보상 하나를 선택하세요.';
    if(skillOffer&&!skillDone){
      const isLevel=skillOffer.source==='level',moves=skillOffer.moves||[];step=isLevel?'NEW MOVE':'TECH DISC';focusName='새 기술을 확인하세요.';focusMeta=step;focusDesc='배울 기술을 고르거나 현재 기술 구성을 유지할 수 있습니다.';
      body=`<section class="learn-offer-v51"><header><img src="${esc(skillOffer.monsterSprite||'')}" alt=""><div><small>${esc(skillOffer.reason||'BATTLE REWARD')}</small><b>${esc(skillOffer.monsterName)}</b><span>새 기술을 하나 확인하세요.</span></div></header><div>${moves.map(m=>`<button data-action="open-learn-move" data-move-id="${esc(m.id)}"><span>${moveKindIcon(m)}</span><div><b>${esc(m.name)}</b><small>${esc(movePowerLine(m))}</small></div><strong>${m.accuracy||100}%</strong></button>`).join('')}</div></section>`;actions=`<button data-action="skip-move">건너뛰기</button>`;
    }else if(rewriteOffer&&!rewriteDone){
      const target=(run.monsters||[]).find(m=>m.instanceId===rewriteOffer.instanceId);step='RIFT REWRITE';focusName='기술 개조를 선택하세요.';focusMeta=step;focusDesc='보스 클리어 보상입니다. 하나를 선택하거나 현재 기술을 유지할 수 있습니다.';
      body=`<section class="rewrite-v51"><header><img src="${esc(rewriteOffer.monsterSprite||monsterDisplaySprite(target)||'')}" alt=""><div><small>BOSS CLEAR</small><b>${esc(rewriteOffer.moveName)}</b><span>기술 개조 하나를 선택하세요.</span></div></header><div>${(rewriteOffer.options||[]).map(x=>`<button data-action="rewrite-pick" data-rewrite-id="${esc(x.id)}"><span>${esc(x.icon||'✦')}</span><div><b>${esc(x.name)}</b><small>${esc(shortLine(x.desc,48))}</small></div></button>`).join('')}</div></section>`;actions=`<button data-action="skip-rewrite">유지하기</button>`;
    }else if(rw.camp&&!campDone){
      step='CAMP';focusName='파티를 정비하세요.';focusMeta='CAMP';focusDesc='회복, PP·상태 정비, 명상 중 하나를 선택합니다.';
      body=`<section class="reward-simple-v51"><header><small>CAMP</small><b>CAMP MAINTENANCE</b></header><div class="three-choice-v51"><button data-action="camp-rest"><span>+</span><b>REST</b><small>MONSTER HP +30%</small></button><button data-action="camp-pp"><span>PP</span><b>PP / STATUS</b><small>PP +50% / CURE STATUS</small></button><button data-action="camp-meditate"><span>◇</span><b>MEDITATE</b><small>FRAGMENT +2 / 40G</small></button></div></section>`;
      ...
      step='SHOP + REWARD';normalShop=true;focusName='보상을 선택하세요.';focusMeta=`${esc(r.biome?.name||'EXPEDITION')} · WAVE ${String(((Number(r.floor||1)-1)%10)+1).padStart(2,'0')} CLEAR`;focusDesc='상점은 선택 사항입니다. 필요한 정비를 한 뒤 아래 무료 보상 중 하나를 선택하세요.';
      ...
      actions=`${rw.serviceCosts?'<button data-action="open-field-service">정비</button>':''}<button data-action="reward-reroll">재굴림 ${nextRerollCost(r)}G</button>${run.fragments>0?'<button data-action="reward-reroll-fragment">◇ 1</button>':''}<button data-action="reward-salvage">분해</button>`;
    }else{
      step='READY';const grade=rw.gradeBy?.[pid];focusName=claim?claim.label:'정비 완료';focusMeta=`WAVE CLEAR${grade?` · ${grade}`:''}`;focusDesc='선택한 보상을 확인한 뒤 다음 웨이브로 이동합니다.';body=rewardReadyPanelV80(r,run,claim,grade);actions=`${rw.kind==='battle'&&rw.serviceCosts?'<button data-action="open-field-service">정비</button>':''}<button class="next-v64" data-action="reward-continue">${continued?'대기 중':r.finalClearPending?'클리어':'바로 다음 웨이브'}</button>`;
    }
  ...
  function monsterPullCardV80(m,featured=false){if(!m)return'';const owned=Number(state.profile.monsters?.[m.id]||0);return `<article class="monster-pull-card-v80 rarity-${esc(m.rarity||m.tier)} ${featured?'featured':''}">${featured?'<span class="pickup-ribbon-v80">PICK UP</span>':''}<div class="monster-pull-art-v80"><img src="${esc(m.sprite)}" alt=""></div><small>${esc(rarityKo(m.rarity||m.tier))} · ${esc(m.element)}</small><b>${esc(m.name)}</b><p>${esc(m.passive?.text||m.skill||'고유 전투 능력')}</p><em>${owned?`보유 ×${owned}`:'미보유'}</em></article>`;}
  function renderGacha(){const b=state.meta.banner,p=state.meta.monsterPickup||{},featured=b.featured.map(id=>byId(state.meta.cards,id)).filter(Boolean),pickup=p.featured;setScreen('gacha',`<section class="gacha-page gacha-page-v80"><div class="gacha-hero"><div class="page-head"><button class="back-btn" data-action="home">← 홈</button><div><div class="section-label">ARCHIVE SUMMON</div><h1>소환 아카이브</h1><p>카드 복각 소환과 몬스터 픽업 소환을 분리했습니다. 몬스터 픽업은 전투에서 얻는 코인으로 뽑으며 5일마다 대상 전설이 교체됩니다.</p></div></div><div class="gacha-grid-v80"><section class="gacha-banner card-banner-v80"><div class="gacha-copy"><div class="section-label">CARD RERUN</div><h2>${esc(b.name)}</h2><p>${esc(b.subtitle)}</p><div class="rate-table">${Object.entries(b.rates).reverse().map(([k,v])=>`<div class="rate-cell"><b class="rarity-${k}">${esc(rarityKo(k))}</b>${(v*100).toFixed(1)}%</div>`).join('')}</div><div class="pity">전설 천장 ${state.profile.pity.legendary}/${b.pity.legendary} · 신화 천장 ${state.profile.pity.mythic}/${b.pity.mythic}</div><div class="banner-actions"><button class="cta" data-action="gacha-pull" data-count="1">1회 · ◆${b.singleCost}</button><button class="cta gold" data-action="gacha-pull" data-count="10">10회 · ◆${b.tenCost}</button></div></div><div class="gacha-art">${featured.map(c=>cardHtml(c)).join('')}</div></section><section class="gacha-banner monster-banner-v80"><div class="monster-banner-copy-v80"><div class="section-label">5-DAY LEGEND PICKUP</div><div class="pickup-timer-v80"><span>교체까지</span><b id="monsterPickupClock">--</b></div><h2>${pickup?esc(pickup.name):'전설 몬스터 픽업'}</h2><p>전설 등급 등장 시 픽업 대상 출현 확률이 상승합니다. 10회 소환의 마지막 1회는 희귀 이상을 보장합니다.</p><div class="coin-wallet-v80"><span>COIN</span><b>● ${Number(state.profile.coins||0).toLocaleString('ko-KR')}</b><small>전투 승리로 획득</small></div><div class="rate-table">${Object.entries(p.rates||{}).reverse().map(([k,v])=>`<div class="rate-cell"><b class="rarity-${k}">${esc(rarityKo(k))}</b>${(v*100).toFixed(1)}%</div>`).join('')}</div><div class="pity">전설 확정 ${Number(state.profile.monsterPickupPity||0)}/${p.pity||30}</div><div class="banner-actions"><button class="cta mint" data-action="monster-pickup-pull" data-count="1">1회 · ●${p.singleCost||500}</button><button class="cta gold" data-action="monster-pickup-pull" data-count="10">10회 · ●${p.tenCost||4500}</button></div></div><div class="monster-pickup-stage-v80">${pickup?monsterPullCardV80(pickup,true):''}<div class="pickup-aura-v80"></div></div></section></div></div></section>`);tickBanner();}
  ...
  document.addEventListener('error',e=>{const img=e.target;if(!(img instanceof HTMLImageElement)||!img.matches('[data-img-fallback-v71]'))return;img.classList.add('broken-v71');const wrap=img.closest('.reward-art-v71');if(wrap)wrap.classList.add('fallback');},true);
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='monster-pickup-pull')return doMonsterPickup(el.dataset.count);
