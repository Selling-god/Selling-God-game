...
(() => {
  ...
  function acceptRoomUpdate(next,{initial=false}={}){
    ...
    if(leavingBattle){
      ...
      const delay=Math.min(5600,Math.max(520,choreography+120));
    ...
    if(!initial)setTimeout(()=>{const choreography=playRoomEvents(next,after);setTimeout(()=>playCombatDelta(prev,next),Math.min(5600,Math.max(120,choreography+80)));},changed?260:0);
  }
  ...
  function moveStatus(move,u){if(!move)return'LOCK';if(Number(u?.level||1)<Number(move.unlockLevel||1))return`Lv.${move.unlockLevel}`;if(Number(move.cooldownRemaining||0)>0)return`CD ${move.cooldownRemaining}`;let acc=Number(move.accuracy||100);if(move.rewrite?.id==='overclock')acc=Math.max(50,acc-6);return `${move.rewrite?.icon?`${move.rewrite.icon} `:''}${acc}%`;}
  ...
  function showMoveTeachMenu(offer,moveId){
    ...
    modal(`<div class="move-teach-modal-v41 move-teach-modal-v43"><div class="section-label">${isLevel?'LEVEL-UP MOVE':'TECH DISC'}</div><div class="teach-head-v41"><img src="${esc(offer.monsterSprite||'')}"><div><small>${esc(offer.reason||'NEW MOVE')}</small><h2>${esc(offer.monsterName)}</h2><b>${moveKindIcon(move)} ${esc(move.name)}</b><span>${esc(movePowerLine(move))} · ${esc(move.element||'')}</span></div></div><p>${isLevel?`Lv.${offer.learnLevel||''} 성장으로 새 기술을 익힐 수 있습니다.`:'기술 디스크에서 선택한 기술입니다.'} 교체할 4개 기술 중 하나를 선택하세요.</p><div class="replace-moves-v41">${[0,1,2,3].map(i=>{const old=current[i];return `<button data-action="learn-move" data-move-id="${esc(move.id)}" data-replace-index="${i}"><em>0${i+1}</em><b>${old?esc(old.name):'EMPTY'}</b><small>${old?esc(movePowerLine(old)):'새 슬롯'}</small><span>→ ${esc(move.name)}</span></button>`;}).join('')}</div><button class="cta" data-action="modal-close">취소</button></div>`);
  }
  ...
  function postBattlePartySummary(r,run){
    ...
    return `<section class="postbattle-summary-v43"><header><div><small>WAVE ${String(((Number(r.floor||1)-1)%10)+1).padStart(2,'0')} CLEAR</small><b>전투 경험치 정산</b></div>${grade?`<strong>${esc(grade)}</strong>`:''}</header><div class="party-exp-v43">${rows.map(x=>{const mon=(run.monsters||[]).find(m=>m.instanceId===x.instanceId),sprite=x.sprite||monsterDisplaySprite(mon),pctv=Math.max(0,Math.min(100,Math.round(Number(x.xp||0)/Math.max(1,Number(x.xpNeed||1))*100)));return `<article class="exp-mon-v43 ${x.leveled?'leveled':''} ${x.partyRole==='ko'?'ko':''}"><div class="exp-sprite-v43"><img src="${esc(sprite||'')}" alt="">${x.leveled?'<i>LEVEL UP!</i>':''}</div><div class="exp-copy-v43"><b>${esc(mon?.name||x.name||'MONSTER')}</b><span>Lv.${x.toLevel}${x.fromLevel!==x.toLevel?` <em>${x.fromLevel}→${x.toLevel}</em>`:''}</span><div><i style="width:${pctv}%"></i></div><small>EXP +${x.xpGained} · ${Math.round(x.xp||0)}/${Math.round(x.xpNeed||0)}${x.capped?` · LEVEL CAP ${x.levelCap}`:''}${x.newMoves?.length?` · NEW ${esc(x.newMoves.map(m=>m.name).join(', '))}`:''}</small></div></article>`;}).join('')}</div></section>`;
  }
  ...
  function renderReward(r){
    ...
    else if(skillOffer&&!skillDone){const isLevel=skillOffer.source==='level';step=isLevel?'NEW MOVE':'TECH DISC';body=`<div class="skill-reward-v41 skill-reward-v43"><div class="skill-mon-v41"><img src="${esc(skillOffer.monsterSprite||'')}"><div><small>${esc(skillOffer.reason||'BATTLE REWARD')}</small><b>${esc(skillOffer.monsterName)}</b><span>${isLevel?'레벨업으로 새로운 기술을 떠올렸습니다.':'워든/보스가 남긴 기술 디스크입니다.'}</span></div></div><div class="skill-disc-grid-v41 ${isLevel?'single-v43':''}">${(skillOffer.moves||[]).map(m=>`<button data-action="open-learn-move" data-move-id="${esc(m.id)}" class="kind-${esc(m.kind||'attack')} element-${elementClass(m.element||'')}"><span>${moveKindIcon(m)}</span><b>${esc(m.name)}</b><small>${esc(movePowerLine(m))}</small><em>${m.accuracy||100}% · CD ${m.cooldown||0}</em></button>`).join('')}</div></div>`;actions=`<div class="reward-actions-v31"><button data-action="skip-move">${isLevel?'배우지 않기':'기존 기술 유지'}</button></div>`;}
