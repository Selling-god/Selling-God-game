...
(() => {
  ...
  const BATTLE_PACE_V57={turn:1350,announce:2100,move:1900,signature:2450,result:1700,status:1800,blocked:1950,win:2550,minor:1250};
  function battleNarrateV57(text,{tone='normal',tag='BATTLE',sub='',instant=false}={}){
    ...
    if(instant||chars.length<2){b.textContent=chars.join('');return;}let i=0;const speed=Math.max(28,Math.min(48,Math.round(1250/Math.max(1,chars.length))));
  ...
  function moveResultNarrationsV57(payload,enemySide=false){
    ...
    if(Number(payload?.targetHpAfter)===0&&Number(payload?.targetHpBefore)>0)out.push({text:`${targetName}은(는) 쓰러졌다!`,tone:enemySide?'bad':'victory',tag:'FAINT',faint:true});
  ...
  function playRoomEvents(next,afterSeq){
    ...
    for(const ev of events){const p=ev.payload||{};
      ...
      else if(ev.type==='turn')later(()=>{const turnNo=Number(p.turn||next.battle?.turn||0);battleNarrateV57(`턴 ${turnNo} — 행동을 선택하세요.`,{tag:'YOUR TURN',tone:'normal'});showCenterBanner(`TURN ${turnNo}`,'','turn');},BATTLE_PACE_V57.turn);
      ...
      else if(ev.type==='monster-hit'){/* hit feedback is serialized with enemy-attack/status phases. */}
      else if(ev.type==='replacement-required'&&p.playerId===state.profileId)later(()=>{battleNarrateV57(`${p.faintedName||'몬스터'} 대신 내보낼 몬스터를 선택하세요.`,{tag:'CHOOSE NEXT',tone:'normal'});showCenterBanner('CHOOSE NEXT','출전할 몬스터를 선택하세요','turn');},BATTLE_PACE_V57.result);
      else if(ev.type==='monster-switch')later(()=>{const targetName=p.monsterName||ev.message||'몬스터';battleNarrateV57(p.replacement?`${targetName}, 나와줘!`:(ev.message||`${targetName} 교대!`),{tag:p.replacement?'SEND OUT':'SWITCH',tone:'good'});if(p.replacement)showCenterBanner(targetName,'BATTLE IN','turn');},BATTLE_PACE_V57.result);
  ...
  function renderBattle(r){
    ...
    const pendingReplacement=b.phase==='replacement'&&(me.pendingReplacements||[]).length?me.pendingReplacements[0]:null;
    const waitingOnAlly=b.phase==='replacement'&&!pendingReplacement;
    ...
    const replacementMenu=pendingReplacement?`<div class="replacement-command-v63"><header><small>CHOOSE NEXT MONSTER</small><b>${esc(pendingReplacement.faintedName||'몬스터')} 대신 누구를 내보낼까?</b><span>교체 출전은 행동을 소비하지 않습니다. 새 몬스터가 나온 뒤 다음 턴의 명령을 선택합니다.</span></header><div class="replacement-grid-v63">${(me.bench||[]).filter(u=>u.hp>0).map(u=>`<button data-action="replacement-pick" data-bench="${esc(u.instanceId)}" data-slot="${Number(pendingReplacement.slotIndex||0)}"><img src="${esc(monsterDisplaySprite(u))}" alt="" decoding="async"><div><b>${esc(u.name)}</b><small>Lv.${u.level} · ${esc(u.element)} · HP ${u.hp}/${u.maxHp}</small><i><em style="width:${pct(u.hp,u.maxHp)}%"></em></i></div><strong>출전</strong></button>`).join('')||'<span class="replacement-none-v63">출전 가능한 몬스터가 없습니다.</span>'}</div></div>`:waitingOnAlly?`<div class="replacement-wait-v63"><span>…</span><b>동료가 교체 몬스터를 선택하고 있습니다</b><small>선택이 끝나면 전투가 계속됩니다.</small></div>`:'';
    const command=pendingReplacement||waitingOnAlly?replacementMenu:menu==='moves'?moveMenu:menu==='capture'?captureMenu:menu==='fusion'?fusionMenu:menu==='party'?partyMenu:menu==='info'?infoMenu:rootMenu;
  ...
  document.addEventListener('click',async e=>{
    ...
    try{
      ...
      if(action==='replacement-pick'){if(state.busy)return;const d=await roomPost('replacement',{benchId:el.dataset.bench,slotIndex:Number(el.dataset.slot||0)});state.selectedMonster=d.result?.instanceId||el.dataset.bench;state.battleMenu='root';await waitForCombatCinematicV54();return renderRoom();}
