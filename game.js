...
/* v40.40 ASYNC COMPANY GOVERNANCE */
let companyAsyncV4040={notifications:[],unread_count:0,companies:[]};
const policyLabelsV4040={stable:'안정 경영',growth:'성장 투자',dividend:'고배당 우선',defense:'위기 방어',expansion:'공격적 확장'};
function companyAsyncMapV4040(){return new Map((companyAsyncV4040.companies||[]).map(x=>[String(x.stock_id),x]))}
function dateLeftV4040(v){if(!v)return '-';const s=Math.max(0,Math.floor((new Date(v)-Date.now())/1000));const h=Math.floor(s/3600),m=Math.floor((s%3600)/60);return h?`${h}시간 ${m}분`:`${m}분`}

const _loadBusinessV4040=loadBusiness;
loadBusiness=async function(options={}){
  const host=document.getElementById('businessView');if(!host)return;
  if(!options.silent)host.innerHTML='<div class="business-loading"><span class="business-spinner"></span><b>비동기 주주 경영 정보 확인 중</b><small>총회·위기·블록딜·알림을 서버 시간으로 정산하고 있습니다.</small></div>';
  try{
    const [board,div,season,asyncData]=await Promise.all([
      rpcV4039('get_corporate_board_v4039'),rpcV4039('get_shareholder_dividend_status_v4039'),rpcV4039('get_shareholder_season_v4039'),rpcV4039('get_company_async_dashboard_v4040')
    ]);
    corporateBoardV4034=Array.isArray(board)?board:[];corporateDividendV4034=div||{companies:[],tax_rate:0,wealth:0};shareholderSeasonV4039=season||null;companyAsyncV4040=asyncData||{notifications:[],unread_count:0,companies:[]};
    renderCorporateBoardV4039();
  }catch(e){console.error(e);host.innerHTML=`<div class="business-error"><b>회사 시스템을 불러오지 못했습니다.</b><p>${esc(e.message||String(e))}</p><small>v40.40 추가 SQL 실행 여부를 확인하세요.</small><button onclick="loadBusiness()">다시 시도</button></div>`}
};
