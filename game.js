...
let authMode="login",currentUser=null,profile=null,inventory=[],stocks=[],holdings=[],collectibles=[],effects={},explore=null,auction=null,auctionChoices=[],sellerAuction=null,negotiation=null,job=null,selectedStock=null,toastTimer=null,realtime=null,negotiationSkills={},collectiblePage=1,decorationPage=1;
...
async function startSellerAuction(){const id=auctionSellItem.value,r=inventory.find(x=>x.id===id);if(!r)return toast('출품할 아이템을 선택하세요.');const{data,error}=await db.rpc('start_npc_seller_auction_v13',{p_user_item_id:id});if(error)return toast(error.message);sellerAuction={session:data.session_id,item:r,current:Number(data.start_price),step:0,maxSteps:Number(data.max_steps),log:[`시작가 ${money(data.start_price)}`]};renderSellerAuction();runSellerAuction()}
function renderSellerAuction(){const s=sellerAuction;if(!s)return;sellerAuctionHall.innerHTML=`<div class="seller-live"><img src="${itemImage(s.item.items.name,s.item.items.category)}"><div><p class="eyebrow">NPC COLLECTOR BATTLE</p><h2>${esc(s.item.items.name)}</h2><div class="seller-price">현재 입찰가 <b>${money(s.current)}</b></div><div class="collector-row"><span>🧐 감정가</span><span>🤑 수집가</span><span>😎 리셀러</span></div><div class="bid-log">${s.log.slice(-6).map(x=>`<p>${esc(x)}</p>`).join('')}</div></div></div>`}
function runSellerAuction(){const t=setInterval(async()=>{const s=sellerAuction;if(!s)return clearInterval(t);if(s.step>=s.maxSteps){clearInterval(t);const{data,error}=await db.rpc('finish_npc_seller_auction_v13',{p_session_id:s.session,p_final_price:s.current});if(error)return toast(error.message);toast(`판매 완료 ${money(data.final_price)}`);playSuccessSound();sellerAuction=null;await Promise.all([loadProfile(),loadInventory()]);fillAuctionSellItems();sellerAuctionHall.innerHTML='<div class="auction-finished">NPC 경매가 종료되었습니다.</div>';return}s.step++;const rarityWeight=rarityScore(s.item.items.rarity),cond=s.item.condition_score;const jump=.025+Math.random()*(.025+rarityWeight*.012+cond/2500);s.current=Math.round(s.current*(1+jump));s.log.push(`${['감정가','수집가','리셀러'][s.step%3]} +${money(Math.round(s.current*jump/(1+jump)))}`);renderSellerAuction()},1300)}
...
/* 소장품/집 */
async function drawCollectible(){const modal=document.getElementById('gachaModal');modal.classList.remove('hidden');modal.className='overlay gacha-spinning rarity-0';gachaRarity.textContent='두근두근...';gachaResultIcon.textContent='🎁';gachaResultName.textContent='캡슐 개봉 중';gachaResultName.className='';gachaResultEffect.textContent='빛이 강해집니다';playGachaBuild();await wait(1700);const{data,error}=await db.rpc('draw_collectible');if(error){closeGachaReveal();return toast(error.message)}const rank=rarityScore(data.rarity);modal.className=`overlay gacha-reveal rarity-${rank}`;gachaRarity.textContent=data.rarity;gachaResultIcon.textContent=data.icon||'✨';gachaResultName.textContent=data.name;gachaResultName.className=`rarity-text ${rarityClass(data.rarity)}`;gachaResultEffect.textContent=`${data.effect_name} +${data.effect_percent}%`;rank>=4?playJackpotSound():playSuccessSound();await Promise.all([loadProfile(),loadCollectibles()]);updateNetworth()}
function closeGachaReveal(){gachaModal.className='overlay hidden'}
function wait(ms){return new Promise(r=>setTimeout(r,ms))}
async function loadCollectibles(){const{data,error}=await db.from('user_collectibles').select(`id,is_equipped,is_placed,is_listed,collectibles(id,name,type,rarity,effect_code,effect_name,effect_percent,icon)`).eq('user_id',currentUser.id).order('acquired_at',{ascending:false});if(error)return toast(error.message);collectibles=data||[];const eq=collectibles.find(x=>x.is_equipped&&x.collectibles.type==='phone_case');equippedCase.innerHTML=eq?collectibleRow(eq):'<p class="muted">장착 케이스 없음</p>';renderCollectiblePages();applyPhoneCase(eq);fillCollectibleSelect()}
function renderCollectiblePages(){const list=[...collectibles],pageSize=6,total=Math.max(1,Math.ceil(list.length/pageSize));collectiblePage=Math.min(Math.max(1,collectiblePage),total);const start=(collectiblePage-1)*pageSize;collectibleInventory.innerHTML=list.slice(start,start+pageSize).map(collectibleRow).join('')||'<p class="muted">소장품 없음</p>';const info=document.getElementById('collectiblePageInfo');if(info)info.textContent=`${collectiblePage}P / ${total}P · 총 ${list.length}개`;const prev=document.getElementById('collectiblePrev'),next=document.getElementById('collectibleNext');if(prev)prev.disabled=collectiblePage<=1;if(next)next.disabled=collectiblePage>=total;}
function changeCollectiblePage(step){const total=Math.max(1,Math.ceil(collectibles.length/6));collectiblePage=Math.min(total,Math.max(1,collectiblePage+step));renderCollectiblePages();document.getElementById('collectibleInventory')?.scrollTo({top:0,behavior:'smooth'})}
