...
let authMode="login",currentUser=null,profile=null,inventory=[],stocks=[],holdings=[],collectibles=[],effects={},explore=null,auction=null,auctionChoices=[],sellerAuction=null,negotiation=null,job=null,selectedStock=null,toastTimer=null,realtime=null;
...
function rarityScore(r){return {'일반':0,'고급':1,'희귀':2,'진귀':3,'영웅':4,'보물':5,'전설':6}[r]??2}
...
/* 소장품/집 */
async function drawCollectible(){const modal=document.getElementById('gachaModal');modal.classList.remove('hidden');modal.className='overlay gacha-spinning';gachaRarity.textContent='두근두근...';gachaResultIcon.textContent='🎁';gachaResultName.textContent='캡슐 개봉 중';gachaResultEffect.textContent='빛이 강해집니다';playGachaBuild();await wait(1700);const{data,error}=await db.rpc('draw_collectible');if(error){closeGachaReveal();return toast(error.message)}const rank=rarityScore(data.rarity);modal.className=`overlay gacha-reveal rarity-${rank}`;gachaRarity.textContent=data.rarity;gachaResultIcon.textContent=data.icon||'✨';gachaResultName.textContent=data.name;gachaResultEffect.textContent=`${data.effect_name} +${data.effect_percent}%`;rank>=4?playJackpotSound():playSuccessSound();await Promise.all([loadProfile(),loadCollectibles()]);updateNetworth()}
function closeGachaReveal(){gachaModal.className='overlay hidden'}
function wait(ms){return new Promise(r=>setTimeout(r,ms))}
async function loadCollectibles(){const{data,error}=await db.from('user_collectibles').select(`id,is_equipped,is_placed,is_listed,collectibles(id,name,type,rarity,effect_code,effect_name,effect_percent,icon)`).eq('user_id',currentUser.id).order('acquired_at',{ascending:false});if(error)return toast(error.message);collectibles=data||[];const eq=collectibles.find(x=>x.is_equipped&&x.collectibles.type==='phone_case');equippedCase.innerHTML=eq?collectibleRow(eq):'<p class="muted">장착 케이스 없음</p>';collectibleInventory.innerHTML=collectibles.map(collectibleRow).join('')||'<p class="muted">소장품 없음</p>';applyPhoneCase(eq);fillCollectibleSelect()}
...
function applyPhoneCase(eq){const shell=document.querySelector('.phone-shell'),home=document.querySelector('.phone-home');if(!shell||!home)return;const name=eq?.collectibles?.name||'';shell.dataset.case=name;home.dataset.wallpaper=name;phoneOwner.textContent=profile?.nickname||'판매왕'}
